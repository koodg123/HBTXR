...
class SpatialTransform:
    ...
    def ellipse(self, xywht: list[float] | tuple[float, float, float, float, float]) -> list[float]:
        ...
        cov = rotation @ axes @ rotation.T
        linear = self.matrix[:2, :2].astype(np.float32)
        cov_t = linear @ cov @ linear.T
...
def crop_with_padding(image: np.ndarray, source_xywh: tuple[float, float, float, float]) -> np.ndarray:
    x, y, w, h = source_xywh
    x0 = int(np.floor(x))
    y0 = int(np.floor(y))
    x1 = int(np.ceil(x + w))
    y1 = int(np.ceil(y + h))
    out = np.zeros((max(1, y1 - y0), max(1, x1 - x0), *image.shape[2:]), dtype=image.dtype) if image.ndim == 3 else np.zeros((max(1, y1 - y0), max(1, x1 - x0)), dtype=image.dtype)
    src_x0 = max(0, x0)
    src_y0 = max(0, y0)
...
def resize_image(arr: np.ndarray, size_wh: tuple[int, int], *, mode: str) -> np.ndarray:
    image = Image.fromarray(arr)
    resample = Image.Resampling.BILINEAR if mode == "bilinear" else Image.Resampling.NEAREST
    return np.asarray(image.resize(size_wh, resample=resample))


def resize_tensor(arr: np.ndarray, size_wh: tuple[int, int], *, mode: str) -> np.ndarray:
...
def direct_transform(source_xywh: tuple[float, float, float, float], target_size_wh: tuple[int, int], *, policy: str) -> SpatialTransform:
    x, y, w, h = source_xywh
    sx = float(target_size_wh[0]) / max(float(w), 1.0)
    sy = float(target_size_wh[1]) / max(float(h), 1.0)
    matrix = np.asarray(
        [[sx, 0.0, -x * sx], [0.0, sy, -y * sy], [0.0, 0.0, 1.0]],
        dtype=np.float32,
    )
...
def letterbox_transform(
    ...
    *,
    ...
    matrix = np.asarray(
        [[scale, 0.0, pad_x - x * scale], [0.0, scale, pad_y - y * scale], [0.0, 0.0, 1.0]],
        dtype=np.float32,
    )
    return SpatialTransform(policy=policy, matrix=matrix, source_xywh=source_xywh, target_size_wh=target_size_wh, scale_xy=(scale, scale), pad_xy=(pad_x, pad_y))


def is_letterbox_policy(policy: str) -> bool:
    return str(policy).strip().lower() in {"letterbox_square", "sensor_full_letterbox"}
...
def build_transform(
    *,
    ...
    if resize_policy == "facet_square_direct":
        return direct_transform(roi_xywh, target_size_wh, policy=resize_policy)
    if resize_policy == "letterbox_square":
        return letterbox_transform(roi_xywh, target_size_wh, policy=resize_policy)
    if resize_policy == "sensor_full_square":
        full_xywh = (0.0, 0.0, float(source_shape_hw[1]), float(source_shape_hw[0]))
        return direct_transform(full_xywh, target_size_wh, policy=resize_policy)
    if resize_policy == "sensor_full_letterbox":
        full_xywh = (0.0, 0.0, float(source_shape_hw[1]), float(source_shape_hw[0]))
        return letterbox_transform(full_xywh, target_size_wh, policy=resize_policy)
...
def apply_transform_to_frame(frame: np.ndarray, transform: SpatialTransform) -> np.ndarray:
    cropped = crop_with_padding(frame, transform.source_xywh)
    if is_letterbox_policy(transform.policy):
        scale = transform.scale_xy[0]
        scaled_w = max(1, int(round(cropped.shape[1] * scale)))
        scaled_h = max(1, int(round(cropped.shape[0] * scale)))
        resized = resize_image(cropped, (scaled_w, scaled_h), mode="bilinear")
        ...
        return canvas
    return resize_image(cropped, transform.target_size_wh, mode="bilinear")


def apply_transform_to_mask(mask: np.ndarray, transform: SpatialTransform) -> np.ndarray:
    cropped = crop_with_padding(mask, transform.source_xywh)
    if is_letterbox_policy(transform.policy):
        scale = transform.scale_xy[0]
        scaled_w = max(1, int(round(cropped.shape[1] * scale)))
        scaled_h = max(1, int(round(cropped.shape[0] * scale)))
        resized = resize_image(cropped, (scaled_w, scaled_h), mode="nearest")
        canvas = np.zeros((transform.target_size_wh[1], transform.target_size_wh[0]), dtype=resized.dtype)
        pad_x = int(round(transform.pad_xy[0]))
        pad_y = int(round(transform.pad_xy[1]))
        canvas[pad_y:pad_y + resized.shape[0], pad_x:pad_x + resized.shape[1]] = resized
        return canvas
    return resize_image(cropped, transform.target_size_wh, mode="nearest")


def apply_transform_to_event(event: np.ndarray, transform: SpatialTransform) -> np.ndarray:
    cropped_channels = [crop_with_padding(event[channel], transform.source_xywh) for channel in range(event.shape[0])]
    ...
    if is_letterbox_policy(transform.policy):
        ...
        resized = resize_tensor(stacked, (scaled_w, scaled_h), mode="bilinear")
        ...
        return canvas
    return resize_tensor(stacked, transform.target_size_wh, mode="bilinear")


class SpatialTransformResolver:
    def __init__(
        self,
        *,
    ...
    def resolve_resize_policy(self, row: dict[str, Any]) -> str:
    ...
    def build(
        ...
        *,
        ...
        return build_transform(
            resize_policy=self.resolve_resize_policy(row),
            source_shape_hw=(sensor_size_wh[1], sensor_size_wh[0]),
            roi_xywh=roi_xywh,
            target_size_wh=self.input_size,
        )

    def apply_frame(self, frame: np.ndarray, transform: SpatialTransform) -> np.ndarray:
        return apply_transform_to_frame(frame, transform)

    def apply_mask(self, mask: np.ndarray, transform: SpatialTransform) -> np.ndarray:
        return apply_transform_to_mask(mask, transform)

    def apply_event(self, event: np.ndarray, transform: SpatialTransform) -> np.ndarray:
        return apply_transform_to_event(event, transform)
