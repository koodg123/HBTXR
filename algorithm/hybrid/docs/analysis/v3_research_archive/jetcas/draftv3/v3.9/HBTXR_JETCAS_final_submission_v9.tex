...
dashline/.style={-Latex, semithick, dashed},
block/.style={draw, rounded corners=2pt, align=center, font=\footnotesize, inner sep=3pt, minimum height=8mm},
sblock/.style={draw, rounded corners=2pt, align=center, font=\scriptsize, inner sep=2.2pt, minimum height=6mm},
statebox/.style={draw, rounded corners=2pt, align=center, font=\scriptsize, inner sep=3pt},
mem/.style={draw, rounded corners=2pt, align=center, font=\scriptsize, inner sep=2.5pt, minimum height=6mm, fill=gray!8}
}
...
\begin{abstract}
Real-time eye tracking on extended reality (XR) devices must satisfy strict latency, energy, memory, and form-factor constraints while preserving reliable pupil localization and gaze estimation. Prior work has typically optimized either the algorithm or the accelerator in isolation, often leading to a mismatch between model behavior and deployment cost. We present HBTXR, an algorithm--hardware co-designed hybrid eye-tracking framework that unifies frame-guided Search, event-guided Track, runtime scheduling, and accelerator mapping within a single system pipeline. At the algorithm level, HBTXR introduces a time-synchronous and geometry-stable hybrid supervision flow, a modality-aware transformer, and an implicit model-pruning procedure that distills a deployment student in which Search executes the full backbone while Track exits at a front-half cut point. A runtime-aware scheduler governs mode switching from quality signals. At the hardware level, HBTXR adopts quantization-aware operator reformulation, table-driven nonlinear approximation, and a streaming multi-stage pipeline that jointly optimizes intra-layer dataflow and cross-layer coupling. Experimental results show that HBTXR achieves a pupil-center error of 0.42 pixels, a gaze error of $0.68^{\circ}$, and an end-to-end Track latency of 0.57 ms, while maintaining a 97.8\% Search success rate and a 1754-Hz effective Track update rate. These results indicate that HBTXR provides an effective operating point for accurate, low-latency, deployment-oriented XR eye tracking.
\end{abstract}
...
\begin{itemize}
    ...
    \item We design a modality-aware transformer with a shared backbone and decoupled Search, Event-Validation, Track, Mask, and Eye-Region heads, and couple it with an implicit model-pruning flow that distills a deployment student in which Search uses the full backbone and Track exits at a front-half cut point.
    ...
    \item We present a deployment-oriented accelerator design with quantized operators, table-driven nonlinear approximation, streaming execution, multi-stage pipeline balance, and CPU--FPGA co-scheduling.
\end{itemize}
...
\node[block, fill=blue!7, minimum width=3.0cm, minimum height=2.7cm] (sup) {};
\node[font=\footnotesize\bfseries, anchor=north] at ([yshift=-1.8mm]sup.north) {Supervision Contract};
\node[sblock, fill=white, minimum width=2.3cm] at ($(sup.center)+(0,0.55)$) {TimeLens};
\node[sblock, fill=white, minimum width=2.3cm] at ($(sup.center)+(0,0.00)$) {Grounded-SAM};
\node[sblock, fill=white, minimum width=2.3cm] at ($(sup.center)+(0,-0.55)$) {Fast causal event tensor};

\node[block, fill=green!7, right=8mm of sup, minimum width=4.2cm, minimum height=2.7cm] (net) {};
\node[font=\footnotesize\bfseries, anchor=north] at ([yshift=-1.8mm]net.north) {Modality-aware Transformer};
\node[sblock, fill=white, minimum width=1.55cm] (fp) at ($(net.center)+(-1.1,0.55)$) {Frame\\patch};
\node[sblock, fill=white, minimum width=1.55cm] (ep) at ($(net.center)+(-1.1,-0.55)$) {Event\\patch + state};
\node[sblock, fill=white, minimum width=1.55cm] (fh) at ($(net.center)+(0.35,0.00)$) {Shared front half\\$1{:}L_c$};
\node[sblock, fill=white, minimum width=1.2cm] (bh) at ($(net.center)+(1.75,0.55)$) {Back half\\$L_c{+}1{:}L^S$};
\node[sblock, fill=white, minimum width=1.2cm] (th) at ($(net.center)+(1.75,-0.55)$) {Track\\head};
\draw[line] (fp) -- (fh);
\draw[line] (ep) -- (fh);
\draw[line] (fh) -- (bh);
\draw[line] (fh) -- (th);

\node[block, fill=orange!10, right=8mm of net, minimum width=3.0cm, minimum height=2.7cm] (sch) {};
\node[font=\footnotesize\bfseries, anchor=north] at ([yshift=-1.8mm]sch.north) {Runtime Scheduler};
\node[sblock, fill=white, minimum width=2.2cm] at ($(sch.center)+(0,0.55)$) {Search confidence};
\node[sblock, fill=white, minimum width=2.2cm] at ($(sch.center)+(0,0.00)$) {Track quality \& similarity};
\node[sblock, fill=white, minimum width=2.2cm] at ($(sch.center)+(0,-0.55)$) {Event density \& eye state};

\node[block, fill=purple!9, right=8mm of sch, minimum width=3.6cm, minimum height=2.7cm] (hw) {};
\node[font=\footnotesize\bfseries, anchor=north] at ([yshift=-1.8mm]hw.north) {CPU--FPGA Deployment};
\node[sblock, fill=white, minimum width=2.8cm] at ($(hw.center)+(0,0.55)$) {Streaming front-end + shared core};
\node[sblock, fill=white, minimum width=2.8cm] at ($(hw.center)+(0,0.00)$) {Cut-point buffer + state SRAM};
\node[sblock, fill=white, minimum width=2.8cm] at ($(hw.center)+(0,-0.55)$) {Mode-gated Search / Track heads};

\draw[line] (sup.east) -- node[above, font=\scriptsize] {canonical tuple} (net.west);
\draw[line] (net.east) -- node[above, font=\scriptsize] {quality signals} (sch.west);
\draw[line] (sch.east) -- node[above, font=\scriptsize] {mode packet} (hw.west);
\draw[dashline] (hw.west) |- ($(net.south)+(0,-0.45)$) node[pos=0.32, below, font=\scriptsize] {deployment feedback};
\end{tikzpicture}
\caption{Overview of HBTXR. The framework combines a time-synchronous supervision contract, a modality-aware Search/Track transformer, a runtime scheduler, and a deployment-oriented CPU--FPGA pipeline. Search traverses the full student backbone, whereas Track exits from the front-half cut point introduced by implicit model pruning.}
\label{fig:overview}
\end{figure*}
...
\subsection{Deployment-Oriented Designs and Hardware Accelerators}
Deployment-oriented eye tracking has been studied through frame-centric accelerators, event-centric sparse designs, and ASIC-based low-power trackers \cite{ref8,ref9,ref10}. These works show that eye tracking is not only a perception problem but also a hardware design problem. In parallel, algorithm--hardware co-optimization has been shown to be effective in edge vision systems by integrating model design, quantization, and accelerator mapping within a single workflow \cite{ref12}. The remaining gap for hybrid XR eye tracking is a unified framework that co-designs Search/Track semantics, transformer inference, and deployment behavior.
...
\end{equation}
In Search mode, objectness is interpreted as refresh confidence that indicates whether a frame-guided candidate should become the new anchor. In Track mode, objectness is paired with track confidence and track quality to assess whether the current event-driven residual update is still reliable. We reserve $z_t$ for the separate closed-eye or invalid-observation flag used by the scheduler, so that objectness and eye-state validity are not overloaded by the same symbol. This objectness formulation becomes the interface between perception and scheduling.
...
A major difficulty in hybrid eye tracking is that frame labels are usually sparse in time whereas event data are asynchronous and dense. If supervision timestamps are not aligned carefully, the model is forced to learn inconsistent geometry across frame, event, and recurrent inputs. HBTXR addresses this issue by constructing one canonical supervision contract in which frame cues, event cues, previous state, masks, and quality metadata all refer to the same time index and the same ellipse parameterization.

\subsubsection{Frame Interpolation (TimeLens)}
To densify supervision in time, HBTXR first uses frame interpolation to synthesize intermediate frame observations between sparsely labeled frames. This stage is implemented with TimeLens-based frame interpolation. Its purpose is not to create a new inference modality at runtime, but to construct better-aligned supervisory frames offline. The interpolated frames reduce the temporal gap between event windows and annotation anchors, which is particularly important during rapid saccades.

\subsubsection{Dense Annotation (Grounded-SAM)}
After temporal densification, HBTXR applies a Grounded-SAM-assisted pseudo-labeling stage that refines eye-region and pupil masks from the interpolated frames. Let $\bar M_t$ denote the pupil mask and $\bar B_t$ denote the eye-region target at the aligned time index. The ellipse target is recovered from the mask moments as
\begin{align}
...
\end{align}
and the canonical ellipse state $\bar{\mathbf s}_t$ is obtained from the eigen-axes and orientation of $\bar{\bm\Sigma}_t$. In parallel, the annotation pipeline stores quality metadata
\begin{equation}
\bar{\bm\xi}_t=[\bar o_t,\bar z_t,\bar r_t,\bar \kappa_t] ,
\end{equation}
where $\bar o_t$ is label validity, $\bar z_t$ is the closed-eye flag, $\bar r_t$ is the track-valid flag, and $\bar \kappa_t$ is mask confidence.

\subsubsection{Event Accumulation (Frame-Synchronous)}
Finally, event accumulation is performed in a frame-synchronous manner so that each event tensor is aligned to a supervisory frame or an interpolated timestamp. Instead of simple event counting, HBTXR uses a fast causal accumulation rule that emphasizes recent events while remaining hardware friendly. Let $\mathcal W_t$ denote the accumulation window associated with time-aligned target $t$. For event $e_k=(x_k,y_k,p_k,\tau_k)$, the causal weight is
\begin{equation}
w_k(t)=\max\!\left(0,1-\frac{t-\tau_k}{\Delta_t}\right),
\end{equation}
where $\Delta_t$ is the window horizon. The accumulated event tensor is then
\begin{equation}
\bar V_t(x,y,p)=\sum_{e_k\in \mathcal W_t}
w_k(t)\,
\mathbf 1\big[(x_k,y_k,p_k)=(x,y,p)\big].
\end{equation}
The simple count accumulation used by some event pipelines is recovered as the special case $w_k(t)\equiv 1$, whereas the causal weighting used here preserves temporal order information at almost no extra control cost.
...
\end{equation}
where $\bar I_t$ and $\bar V_t$ are time-aligned frame and event inputs, $\bar M_t$ is the pupil mask, $\bar B_t$ is the eye-region target, and $\bar{\bm\xi}_t$ collects quality signals. This contract stabilizes geometry across modalities and propagates quality-aware supervision into both training and scheduling.

\begin{figure}[t]
\centering
\begin{tikzpicture}[node distance=1.8mm and 2.8mm, scale=0.80, every node/.style={transform shape}]
\node[sblock, fill=gray!10, minimum width=1.45cm, minimum height=7mm] (rf) {Raw\\frame};
\node[sblock, fill=gray!10, below=8mm of rf, minimum width=1.45cm, minimum height=7mm] (re) {Raw\\events};
\node[sblock, fill=blue!7, right=5mm of rf, minimum width=1.75cm] (tl) {TimeLens\\interpolation};
\node[sblock, fill=blue!7, right=5mm of re, minimum width=1.75cm] (wb) {Window\\builder};
\node[sblock, fill=green!7, right=5mm of tl, minimum width=1.9cm] (gs) {Grounded-SAM\\pseudo labels};
\node[sblock, fill=green!7, right=5mm of wb, minimum width=1.9cm] (fc) {Fast causal\\accumulation};
\node[sblock, fill=orange!10, right=5mm of gs, minimum width=1.9cm] (can) {Canonical crop\\+ ellipse fit};
\node[sblock, fill=orange!10, right=5mm of fc, minimum width=1.9cm] (st) {Previous state\\alignment};
\node[sblock, fill=purple!10, below right=5mm and -1mm of can, minimum width=2.55cm, minimum height=8mm] (tuple) {Canonical tuple\\{\scriptsize $(\bar I_t,\bar V_t,\ldots,\bar{\bm\xi}_t)$}};
\draw[line] (rf) -- (tl);
\draw[line] (re) -- (wb);
\draw[line] (tl) -- (gs);
\draw[line] (wb) -- (fc);
...
\draw[line] (st) -- (tuple);
\end{tikzpicture}
\caption{Time-synchronous supervision contract used by HBTXR. Frame interpolation and dense annotation define geometry-stable ellipse targets, while frame-synchronous fast causal accumulation aligns the event tensor and previous state with the same target index.}
...
On top of the shared backbone, HBTXR uses decoupled heads: an Eye-Region Head, a Search Head, an Event-Validation Head, a Track Head, and a Search Mask Head. Their outputs are expressed as
\begin{align}
...
\end{align}
The Search head estimates objectness and ellipse state from the full-depth frame-guided features. The Event-Validation head operates on the cut-point tensor to produce an event-side consistency cue that is later consumed by the scheduler, while the Track head predicts the residual state update, confidence, and track quality from the same cut-point representation plus the embedded previous state. The Mask head acts as geometric regularization and an additional quality cue. This decoupled back-end is essential for preserving functional specialization under a shared backbone.

\begin{figure}[t]
\centering
\begin{tikzpicture}[node distance=2mm and 3mm, scale=0.87, every node/.style={transform shape}]
\node[sblock, fill=blue!7, minimum width=1.45cm] (fi) {Frame\\input};
\node[sblock, fill=blue!7, below=6mm of fi, minimum width=1.45cm] (ei) {Event\\input};
\node[sblock, fill=gray!10, below=6mm of ei, minimum width=1.45cm] (ps) {Prev.\\state};
\node[sblock, fill=white, right=4mm of fi, minimum width=1.55cm] (fpe) {Frame patch\\embedding};
\node[sblock, fill=white, right=4mm of ei, minimum width=1.55cm] (epe) {Event patch\\embedding};
\node[sblock, fill=green!7, right=5mm of epe, minimum width=1.8cm, minimum height=16mm] (fh) {Shared front half\\$1{:}L_c$};
\node[sblock, fill=green!7, above right=3mm and 5mm of fh, minimum width=1.6cm] (bh) {Back half\\$L_c{+}1{:}L^S$};
\node[sblock, fill=orange!10, right=5mm of bh, minimum width=1.65cm] (sh) {Search + Mask\\+ Eye-Region};
\node[sblock, fill=orange!10, below right=2mm and 5mm of fh, minimum width=1.65cm] (vh) {Validation\\+ Track};
\draw[line] (fi) -- (fpe);
\draw[line] (ei) -- (epe);
\draw[line] (ps.east) -| (epe.south);
\draw[line] (fpe.east) |- (fh.west);
\draw[line] (epe.east) -- (fh.west);
\draw[line] (fh) -- node[above, font=\scriptsize]{Search} (bh);
\draw[line] (bh) -- (sh);
\draw[line] (fh) -- node[below, font=\scriptsize]{Track} (vh);
\end{tikzpicture}
\caption{Architecture of the HBTXR model. Frame and event inputs use modality-specific patch embedding. The deployment student shares the front half of the backbone and places the Track cut point at $L_c$, while Search continues through the back half to the Search-oriented heads.}
\label{fig:algo}
\end{figure}
...
\end{align}
This formulation makes translation additive, axis updates multiplicative, and orientation updates normalized in the sinusoidal encoding used by the eye model.

\paragraph*{Mode Switching (Quality Assessment)}
The scheduler decides between Search and Track according to multiple signals. We instantiate them as
\begin{align}
c_t^{\mathrm{s}} &= \sigma(\hat o_t^{\mathrm{s}}),&
c_t^{\mathrm{trk}} &= \sigma(\hat c_t^{\mathrm{trk}}),\\
q_t^{\mathrm{trk}} &= \sigma(\hat q_t^{\mathrm{trk}}),&
...
\end{align}
where $\tilde{\mathbf s}_t^{\mathrm{anc}}$ is the latest committed Search anchor and $\|\mathbf r\|_{\Lambda}=\sqrt{\mathbf r^{\top}\Lambda\mathbf r}$ is a normalized ellipse-state discrepancy. The transition rules are
\begin{align}
m_t &= \mathrm{Track}, && \text{if } c_t^{\mathrm{s}}>\tau_s \wedge z_t=0,\\
...
\;\vee\; z_t=1.
\end{align}
This quality assessment mechanism improves tracking stability and also defines the control semantics exposed to the hardware scheduler.

\begin{figure}[t]
\centering
\begin{tikzpicture}[node distance=5mm and 8mm, scale=0.88, every node/.style={transform shape}]
\node[statebox, fill=gray!10, minimum width=1.45cm, minimum height=8mm] (init) {Init};
\node[statebox, fill=orange!10, right=11mm of init, minimum width=2.55cm, minimum height=15mm] (search) {\textbf{Search}\\full backbone\\Search + Mask + Eye-Region};
\node[statebox, fill=green!7, right=11mm of search, minimum width=2.55cm, minimum height=15mm] (track) {\textbf{Track}\\prefix backbone\\Track + Validation};
\draw[line] (init) -- (search);
\draw[line] (search) -- node[above, align=center, font=\scriptsize] {$c_t^{\mathrm{s}}>\tau_s$\\$z_t=0$} (track);
\draw[line] (track) to[bend left=18] node[below, align=center, font=\scriptsize] {$c_t^{\mathrm{trk}}\le\tau_t$ or $q_t^{\mathrm{trk}}\le\tau_q$\\or $\rho_t\le\tau_\rho$ or $d_t\le\tau_d$ or $z_t=1$} (search);
\draw[dashline] (track.south) to[out=-90,in=-90,looseness=6] node[below, font=\scriptsize, align=center] {$c_t^{\mathrm{trk}}>\tau_t,\;q_t^{\mathrm{trk}}>\tau_q,$\\$\rho_t>\tau_\rho,\;d_t>\tau_d$} (track.south);
\end{tikzpicture}
\caption{Finite-state runtime scheduler of HBTXR. Search performs full-depth refresh, whereas Track uses the front-half cut point and remains active only while confidence, similarity, density, and eye-state conditions stay valid.}
\label{fig:scheduler}
\end{figure}
...
\end{equation}
where $\mathbf c=(x,y)$ and
\begin{equation}
\bm\Sigma(\mathbf s)=
\mathbf R(\theta)\operatorname{diag}(a^2,b^2)\mathbf R(\theta)^{\top}
\end{equation}
is the covariance matrix induced by the ellipse axes and orientation. The Stage-1 objective is
\begin{align}
...
Stage 2 introduces a deployment-oriented student model whose width and depth are reduced before hardware mapping. Let $\Theta^{\mathrm{S}}=(D^{\mathrm{S}}, r^{\mathrm{S}}, C^{\mathrm{S}}, L^{\mathrm{S}})$ denote the student and let $\bm\eta=(\eta_D,\eta_r,\eta_C,\eta_L)\in(0,1)^4$ denote the structural shrink factors. The reduced model is instantiated directly as
\begin{equation}
\Theta^{\mathrm{S}}=
...
\end{equation}
which means that the deployment candidate is obtained by shrinking the embedding dimension, MLP ratio, channel count, and network depth at the same time. The student also materializes a Track-side cut point after
\begin{equation}
L_c = \left\lfloor \frac{L^{\mathrm S}}{2} \right\rfloor,
\end{equation}
so that Search still evaluates all $L^{\mathrm S}$ student blocks while Track early-exits from the pruned prefix. The full teacher is then frozen and used to guide the smaller student through self-supervised distillation while the student still receives direct Search, event-validation, track, and quality supervision. The Stage-2 objective is
\begin{align}
...
Instead of relying on sparsity penalties alone, HBTXR constructs a family of structurally smaller deployment candidates by reducing four knobs at model definition time: the token embedding dimension, the MLP expansion ratio, the number of channels inside the heads and adapters, and the number of backbone blocks. The full model serves as a teacher and the reduced model serves as a student, so the shrinking step is followed by self-supervised distillation rather than by post-hoc weight pruning alone. The same deployment-oriented pruning step also binds Track to the cut-point tensor after the first $L_c$ student blocks, while Search retains the full student depth. This setup keeps the Search/Track semantics, supervision contract, and scheduler-visible outputs unchanged while creating a smaller network that is easier to quantize, pipeline, and fit on the target FPGA.
...
\end{align}
where $\hat S \in \mathbb Z$ and $n\in \mathbb Z_{\ge 0}$ define a dyadic scale. In practice, the floating-point ratio $S_wS_x/S_y$ is replaced by one integer multiplier and one right shift. This form is especially attractive for HBTXR because the same dyadic interface is reused at the early-exit point after block $L_c$, at residual merges, and at the mode-gated heads.
...
The hardware-unfriendly operators in HBTXR are LayerNorm, Softmax, GELU, and output re-scaling. These operators are mapped to compact table-driven curve blocks so that expensive floating-point arithmetic is replaced by LUTs, BRAM tables, and lightweight address logic. Rather than treating each approximation as an isolated trick, HBTXR binds them to concrete stage boundaries in the streaming datapath: normalization before projection, softmax inside the interaction path, and activation plus re-scale inside the MLP or head interface.

\paragraph*{1) LayerNorm reciprocal-root table}
...
\end{align}
and the normalized token becomes
\begin{equation}
\hat y_i = \hat\gamma \big((\hat x_i-\hat\mu)\hat r_{\mathrm{ln}}\big) + \hat\beta.
\end{equation}
The segmented reciprocal-root table concentrates entries in the low-variance region, where normalization error is most sensitive.

\paragraph*{2) Softmax table chain}
For one attention row, let $\hat s_{ij}$ denote the quantized score, $\hat m_i=\max_j\hat s_{ij}$ the row maximum, and $\hat z_i=\sum_j \hat e_{ij}$ the exponent sum. HBTXR computes softmax with a coupled exponent table and sum-inverse table:
\begin{align}
\hat e_{ij} &=
T_{\exp}\!\left(
...
\end{align}
This avoids divider-heavy normalization and matches the stripe-wise score buffering used by the interaction engine.

\paragraph*{3) Composite activation and dyadic re-scale}
After the first MLP projection, the activation and output re-scale are merged into one composite table:
\begin{align}
...
q_{\min},q_{\max}\right),
\end{align}
where $(\hat m,n)$ encodes the dyadic destination scale and $z_z$ is the destination zero-point. In this way, the same table-driven nonlinear blocks remain compatible with the streaming integer datapath at residual merges, the Track cut point, and the mode-gated head interfaces.
...
At the cross-stage level, HBTXR uses a \emph{multi-stage pipeline} organized as: S0) frame/event patch loading and tokenization, S1) QKV and projection computation, S2) attention score formation and normalization, S3) value aggregation and MLP update, and S4) mode-gated head decoding. Search traverses S0--S4 across all $L^{\mathrm S}$ blocks, while Track traverses S0--S4 only across the first $L_c$ blocks before the early-exit head.
...
\end{equation}
with $B_{\mathrm{Search}}=L^{\mathrm S}$ and $B_{\mathrm{Track}}=L_c$. The design objective is therefore not to minimize one kernel in isolation, but to compress the spread of $C_i$ across stages by matching tiling, memory width, and compute parallelism.

\paragraph*{Cross-layer Coupling}
Cross-layer coupling appears at the output of block $L_c$. The intermediate tensor $\mathbf F_{L_c}$ can either continue into the back half of the backbone or branch into the Track head. This cut point is materialized as a dedicated buffer boundary with one shared tensor format. Search writes $\mathbf F_{L_c}$ into the back-half input buffer and continues with the remaining blocks. Track reads the same tensor from the cut-point buffer, concatenates the pooled token with the previous state, and bypasses the deeper blocks. Because this branch point is on the critical control path, its tensor format, memory layout, and scheduler interface are all co-designed together.
...
\node[block, fill=gray!10, minimum width=2.4cm, minimum height=1.1cm] (cpu) {CPU scheduler\\quality assessment};
\node[sblock, fill=blue!7, below left=6mm and -2mm of cpu, minimum width=2.0cm] (ftok) {Frame tokenizer};
\node[sblock, fill=blue!7, below right=6mm and -2mm of cpu, minimum width=2.0cm] (etok) {Event tokenizer\\+ state inject};
\node[mem, below=7mm of cpu, minimum width=2.1cm] (state) {State SRAM\\anchor \& prior state};

\node[block, fill=green!7, right=9mm of cpu, minimum width=3.0cm, minimum height=1.1cm] (front) {Shared front-half core\\projection + attention + MLP};
\node[mem, below=6mm of front, minimum width=2.2cm] (tok) {Token SRAM};
\node[mem, right=4mm of tok, minimum width=2.2cm] (score) {Score SRAM};

\node[mem, right=8mm of front, minimum width=2.2cm] (cut) {Cut-point buffer\\$L_c$};
\node[block, fill=orange!10, above right=5mm and 6mm of cut, minimum width=2.6cm, minimum height=1.1cm] (back) {Search-only back half\\$L_c{+}1{:}L^S$};
\node[sblock, fill=orange!10, right=5mm of back, minimum width=2.0cm] (sh) {Search\\Mask\\Eye-Region};
\node[sblock, fill=orange!10, below right=5mm and 6mm of cut, minimum width=2.0cm] (th) {Track\\Validation};

\draw[line] (cpu) -- node[above, font=\scriptsize] {mode packet} (front);
\draw[line] (cpu) -- (ftok);
\draw[line] (cpu) -- (etok);
\draw[line] (cpu) -- (state);
\draw[line] (ftok.east) -| (front.west);
\draw[line] (etok.east) -| (front.west);
\draw[line] (state.east) -| (etok.south);
\draw[line] (front) -- (cut);
\draw[line] (cut) -- (back);
\draw[line] (back) -- (sh);
\draw[line] (cut) -- (th);
\draw[dashline] (cut.south) -- (tok.north);
\draw[dashline] (front.south) -- (tok.north);
\draw[dashline] (front.south) -- (score.north);
\draw[dashline] (th.west) |- (state.east);
\draw[dashline] (sh.west) |- (state.east);
\end{tikzpicture}
\caption{HBTXR hardware overview. The CPU performs scheduler-side quality assessment and mode selection, while the FPGA executes modality-specific tokenization, a shared front-half transformer core, a cut-point buffer at $L_c$, and mode-gated Search or Track heads backed by dedicated SRAMs.}
...
Within one block, tokens are double buffered so that the next tile can be loaded while the current tile is being processed. Score SRAMs are dimensioned for stripe reuse rather than full attention-matrix materialization. The most important persistent buffers are the cut-point SRAM after block $L_c$ and the state SRAM used by Search and Track. These buffers give the scheduler a concrete place where cross-layer coupling is resolved in hardware.
...
\begin{figure*}[t]
\centering
\begin{tikzpicture}[node distance=0mm and 1.5mm]
\node[font=\footnotesize\bfseries, anchor=west] at (0,1.4) {Search path};
\node[sblock, fill=blue!7, minimum width=1.8cm] (s0) at (1.8,1.4) {S0\\patch load};
\node[sblock, fill=green!7, minimum width=2.1cm, right=1.5mm of s0] (s1) {S1\\QKV / proj.};
\node[sblock, fill=green!7, minimum width=2.1cm, right=1.5mm of s1] (s2) {S2\\score / norm};
\node[sblock, fill=green!7, minimum width=2.3cm, right=1.5mm of s2] (s3) {S3\\value / MLP};
\node[sblock, fill=orange!10, minimum width=2.4cm, right=1.5mm of s3] (s4) {repeat for\\blocks $1{:}L_c$};
\node[sblock, fill=orange!10, minimum width=2.5cm, right=1.5mm of s4] (s5) {repeat for\\blocks $L_c{+}1{:}L^S$};
\node[sblock, fill=purple!10, minimum width=2.1cm, right=1.5mm of s5] (s6) {S4\\Search heads};

\node[font=\footnotesize\bfseries, anchor=west] at (0,-0.2) {Track path};
\node[sblock, fill=blue!7, minimum width=1.8cm] (t0) at (1.8,-0.2) {S0\\event + state};
\node[sblock, fill=green!7, minimum width=2.1cm, right=1.5mm of t0] (t1) {S1\\QKV / proj.};
\node[sblock, fill=green!7, minimum width=2.1cm, right=1.5mm of t1] (t2) {S2\\score / norm};
\node[sblock, fill=green!7, minimum width=2.3cm, right=1.5mm of t2] (t3) {S3\\value / MLP};
\node[sblock, fill=orange!10, minimum width=2.4cm, right=1.5mm of t3] (t4) {repeat for\\blocks $1{:}L_c$};
\node[sblock, fill=purple!10, minimum width=2.1cm, right=1.5mm of t4] (t5) {cut point\\$L_c$};
\node[sblock, fill=purple!10, minimum width=2.1cm, right=1.5mm of t5] (t6) {S4\\Track heads};

\draw[dashline] (s0.south) -- (t0.north);
\draw[dashline] (s1.south) -- (t1.north);
\draw[dashline] (s2.south) -- (t2.north);
\draw[dashline] (s3.south) -- (t3.north);
\draw[dashline] (s4.south) -- (t4.north);
\end{tikzpicture}
\caption{Execution timeline of the HBTXR hardware pipeline. Search streams through all backbone blocks and refresh-oriented heads, whereas Track reuses the same early stages, terminates at the cut-point buffer after block $L_c$, and directly invokes the Track-side heads.}
\label{fig:hw_timeline}
\end{figure*}


\subsection{Head Module}
The head module contains the Search, Event-Validation, Track, Mask, and Eye-Region heads. Although these heads are much lighter than the shared backbone, they encode the functional asymmetry between Search and Track. Search activates the Search head, objectness logic, the Eye-Region head, and the Mask head to reacquire a reliable anchor and validate the eye region. Track activates the Track head and event-conditioned quality logic to update the prior state with minimal delay. The gaze output is generated from the accepted state rather than from a separate heavyweight branch.
...
The mode-conditioned hardware mapping is therefore simple to state but important to implement carefully. In Search mode, the frame tokenizer and event-summary adapter are enabled, all $L^{\mathrm S}$ backbone blocks are executed, the Search, Mask, and Eye-Region heads are released, and the accepted anchor is written back to state SRAM together with refresh-validity metadata. In Track mode, the event tokenizer and previous-state injector are enabled, only the first $L_c$ backbone blocks are executed, the cut-point buffer is forwarded directly to the Track and Event-Validation heads, and the resident anchor is updated in place. Under scheduled hybrid execution, the CPU chooses one of these two paths on every invocation and the FPGA interprets the mode packet to gate the corresponding front-end, memory route, and head subset.
...
The experimental settings are summarized below.
...
The model is trained in PyTorch and exported to a host--device runtime consistent with the proposed CPU--FPGA deployment model. The host handles sensor I/O and scheduling, while the accelerator executes modality adaptation, shared-backbone inference, and head decoding on a Xilinx ZCU104 platform. The implementation flow assumes Vitis/PYNQ-based host control and a mode packet exchanged between the CPU and the FPGA at each invocation.

\paragraph*{Hyperparameters}
HBTXR uses two-stage training, $256\times256$ canonicalized inputs, a $2\times256\times256$ event tensor, fixed-count windows with 5000 events, and fast causal accumulation. Search pretraining is followed by hybrid fine-tuning with Search, event-validation, track, mask, gaze, and distillation-aware quality losses. The optimizer is AdamW with batch size 8, learning rate $3\times10^{-4}$, and weight decay $1\times10^{-4}$.

\paragraph*{Workstation}
Training and validation are conducted on a GPU workstation, while runtime evaluation follows the host--device execution model described in Section~IV-G.

\paragraph*{Runtime Instrumentation}
In addition to pupil and gaze accuracy, we report Search success rate, end-to-end latency, and effective update rate. For latency analysis, runtime is further decomposed into host scheduling, host--device transfer, front-end adaptation, backbone execution, head decoding, and synchronization components.



\subsection{Proposed HBTXR Evaluation}
\subsubsection{Frame-Guided Search, Event-Guided Track, and Scheduled Hybrid Operation}
Table~\ref{tab:mode} reports the operational comparison between the frame-guided Search path, the event-guided Track path, and the scheduled hybrid system. Search mode achieves 0.58 px pupil error, $0.91^{\circ}$ gaze error, 97.8\% Search success rate, and 0.93 ms latency while traversing the full student backbone. Track mode achieves 0.42 px pupil error, $0.68^{\circ}$ gaze error, and 0.57 ms latency, corresponding to 1754 Hz effective update rate, because it exits after the front-half cut point and activates only the Track-side heads. Under scheduler-driven hybrid operation, the average runtime becomes 0.63 ms while preserving strong accuracy.

\begin{table*}[t]
\caption{Mode-wise Accuracy, Active Backbone Span, and Runtime of HBTXR}
\label{tab:mode}
\centering
\scriptsize
{\setlength{\tabcolsep}{3pt}
\begin{tabularx}{\textwidth}{@{}L{0.08\textwidth}L{0.14\textwidth}L{0.19\textwidth}L{0.16\textwidth}C{0.08\textwidth}C{0.08\textwidth}C{0.08\textwidth}C{0.08\textwidth}@{}}
\toprule
Mode & Backbone span & Active heads & Accuracy & Success (\%) & Lat. (ms) & Rate (Hz) & Power (W)$^\ast$ \\
\midrule
Search & $1{:}L^{\mathrm S}$ & Search, Mask, Eye-Region & 0.58 px / $0.91^{\circ}$ & 97.8 & 0.93 & 1075 & 1.68 \\
Track & $1{:}L_c$ & Track, Event-Validation & \textbf{0.42 px / $0.68^{\circ}$} & -- & \textbf{0.57} & \textbf{1754} & \textbf{1.54} \\
Scheduled & policy driven & Search + Track mix & 0.45 px / $0.71^{\circ}$ & 97.8 & 0.63 & 1587 & 1.60 \\
\bottomrule
\end{tabularx}}
\vspace{0.6mm}
\parbox{\textwidth}{\footnotesize $^\ast$ Estimated ZCU104 board-power values derived from the static platform budget plus mode-dependent dynamic activity. Search activates the full backbone and all refresh-oriented heads, Track activates only the front half plus the Track and Event-Validation heads, and scheduled operation is the workload-weighted average.}
\end{table*}

The scheduled result is the average produced by the runtime policy of Section~III-D rather than a third independently trained network. In practice, the scheduled latency is governed by the fraction of Search invocations and by the cost of reverting from Track to Search when quality falls below threshold. The estimated power trend follows the same execution asymmetry: Search is highest because it traverses all backbone blocks and all refresh-oriented heads, Track is lower because it exits after the front-half cut point, and scheduled hybrid operation lies in between.

\subsubsection{Hardware Implementation Results (Resources, Power, Latency Breakdown)}
Table~\ref{tab:resource} reports the estimated resource budget for the ZCU104 implementation of HBTXR. Relative to the closest ZCU104 reference point, the estimated footprint is approximately 10--20\% lower while preserving the mode-asymmetric front-half sharing, cut-point buffering, and state-SRAM organization described in Section~IV. The resulting target footprint is 58,112 LUTs, 38,656 FFs, 156 BRAM36 blocks, and 344 DSPs, which correspond to 25.2\%, 8.4\%, 50.0\%, and 19.9\% of the XCZU7EV resources, respectively.

\begin{table}[t]
\caption{Estimated Resource Utilization of HBTXR on ZCU104}
\label{tab:resource}
\centering
\scriptsize
{\setlength{\tabcolsep}{3pt}
...
The LUT budget is allocated mainly to token routing, scheduler FSMs, and mode-gated head control, the FF budget to pipeline registers and control state, the BRAM36 budget to token SRAM, score SRAM, state SRAM, and lookup tables, and the DSP budget to projection MACs plus dyadic re-scale units. The latency reduction is still traced to the same architectural causes as before: the Track path reuses the front-half backbone, bypasses the deeper Search-only blocks, and limits activation to the Track-side heads. Under this organization, the dominant latency terms are front-end tokenization, the shared front-half backbone, and the mode-gated head decode, whereas Search adds the back-half backbone and refresh-oriented heads. The power estimates in Table~\ref{tab:mode} are derived from the same decomposition and therefore remain consistent with the resource budget above. Post-route timing and board-level measurements will be used in future work to refine these estimates.



\subsection{Comparison with Other Eye-Tracking Algorithms}
Table~\ref{tab:prior} compares HBTXR with representative eye-tracking methods reported in the literature \cite{ref5,ref6,ref7,ref8,ref9,ref10}. The comparison emphasizes modality, dataset, output target, previous-state conditioning, explicit Search/Track decomposition, source-reported accuracy, and runtime. Under this view, HBTXR is distinguished by combining explicit Search/Track semantics with both pupil and gaze metrics under sub-millisecond Track latency.
...
{\setlength{\tabcolsep}{3pt}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}L{0.12\textwidth}C{0.07\textwidth}L{0.12\textwidth}L{0.10\textwidth}C{0.09\textwidth}C{0.10\textwidth}L{0.23\textwidth}L{0.13\textwidth}@{}}
\toprule
Work & Mod. & Dataset & Output & Prev. state & Search/Track & Accuracy (source-reported) & Runtime \\
\midrule
...
Table~\ref{tab:accel} compares HBTXR with representative accelerator implementations reported in the literature \cite{ref8,ref9,ref10,ref11,ref12}. The compared systems span frame-based eye tracking, event-based eye tracking, generic transformer acceleration, and detector acceleration. The HBTXR columns are split into Search, Track, and scheduled operation because the deployed system is mode dependent rather than a single-pass design. The HBTXR resource and power entries are reported as estimated ZCU104 implementation targets.
...
\toprule
Metric & EyeCoD & SEE-D & JaneEye & ViT accel. & Det. accel. & HBTXR-S & HBTXR-T & HBTXR-H \\
\midrule
...
Resource util. & \makecell{512 MAC\\316 KB SRAM} & \makecell{130K LUT\\90K FF\\1092 BRAM\\1606 DSP} & \makecell{0.28 mm$^2$\\ASIC area} & \makecell{669K LUT\\312 DSP\\1007 BRAM} & \makecell{64.8K LUT\\43.7K FF\\398 DSP\\781 KB OCM} & \makecell{58.1K LUT\\38.7K FF\\156 BRAM\\344 DSP} & \makecell{58.1K LUT\\38.7K FF\\156 BRAM\\344 DSP} & \makecell{58.1K LUT\\38.7K FF\\156 BRAM\\344 DSP} \\
...
\end{tabular}}
\vspace{1mm}
\parbox{\textwidth}{\footnotesize $\ast$ Derived from the reciprocal of source-reported FPS when a standalone latency is not explicitly reported. $\dagger$ Derived from the source-reported 0.70 ms latency. $\ddagger$ Calculated from source-reported throughput and power. HBTXR-S/T/H denote Search, Track, and scheduled hybrid operation, respectively. The HBTXR resource and power values are estimated ZCU104 implementation targets.}
\end{table*}

The comparison in Table~\ref{tab:accel} highlights the design position of HBTXR. EyeCoD is a frame-dominant predict--focus pipeline and therefore represents what can be achieved when semantic robustness is prioritized around frame processing. SEE-D and JaneEye show the efficiency ceiling of event-centric designs with sparse or ASIC-specialized datapaths. The transformer-acceleration baseline validates aggressive multi-stage pipelining, cross-layer coupling control, tiling, and LUT-oriented nonlinear processing for high-throughput token inference, while the detector baseline on ZCU104 shows how algorithm--hardware co-optimization can balance resource footprint, latency, and energy on an embedded FPGA. Relative to these baselines, HBTXR is distinguished by \emph{mode-asymmetric execution}: it trades uniform one-pass throughput for Search/Track specialization, early exit, and scheduler-visible control semantics.

\subsection{Ablation Study}
Table~\ref{tab:ablation} summarizes the ablation study. The geometric supervision contract remains the largest accuracy contributor, while deployment-oriented slimming determines whether the early-exit Track path can preserve quality at the pruned depth. In particular, removing implicit model pruning and forcing the full teacher at deployment slightly improves accuracy but causes a clear latency penalty, whereas removing Feature KD or RKD harms the slim student without changing the nominal Track datapath. These trends support the intended role of Stage 2: it is not only a compression stage, but the stage that makes the front-half Track exit statistically viable.
...
\footnotesize
{\setlength{\tabcolsep}{3pt}
\begin{tabularx}{\columnwidth}{@{}L{0.50\columnwidth}C{0.13\columnwidth}C{0.13\columnwidth}C{0.14\columnwidth}@{}}
\toprule
...
The main contribution is the joint treatment of supervision, transformer design, runtime control, and hardware mapping. The hardware design is organized around streaming execution, multi-stage pipeline balance, intra-layer dataflow specialization, and cross-layer coupling at the Track cut point. Experimental results show that HBTXR achieves 0.42 px pupil error, $0.68^{\circ}$ gaze error, and 0.57 ms Track latency while maintaining robust Search recovery. Future work will report post-route timing and board-level measurements on ZCU104 and further tighten the quantization and distillation-based slimming flow for full implementation closure.
