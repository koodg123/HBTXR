...
\node[sblock, fill=orange!10, below right=2mm and 5mm of fh, minimum width=1.65cm] (vh) {Event-valid.\\+ Track};
\draw[line] (fi) -- (fpe);
\draw[line] (ei) -- (epe);
...
q_{\min},q_{\max}\right),
\end{align}
where $(\hat m,n)$ encodes the dyadic destination scale and $z_z$ is the destination zero-point. In this way, the same table-driven non-linear blocks remain compatible with the streaming integer datapath at residual merges, the Track cut point, and the mode-gated head interfaces.
...
Table~\ref{tab:prior} follows the comparison style commonly used by the project-source eye-tracking papers \cite{ref5,ref6,ref7,ref8,ref9,ref10}. To avoid large blocks of missing entries, the comparison emphasizes modality, dataset, output target, previous-state conditioning, explicit Search/Track decomposition, source-reported accuracy, and runtime rather than forcing every method into one shared parameter/FLOP field. Even under that conservative presentation, HBTXR stands out by combining explicit Search/Track semantics with both pupil and gaze metrics under sub-millisecond Track latency.
...
To match the comparison style of recent FPGA accelerator papers \cite{ref8,ref9,ref10,ref11,ref12}, Table~\ref{tab:accel} is written in a metric-by-system layout. The compared systems span frame-based eye tracking, event-based eye tracking, generic vision transformer acceleration, and detector acceleration. The HBTXR columns are further split into Search, Track, and scheduled operation because the deployed system is mode dependent rather than single pass. For transparency, the HBTXR entries should be read as projected ZCU104 implementation targets rather than post-route measurements.
...
Power cons. & 0.154 W & 3.86 W & 0.038 W & 46.7 W & 1.91 W & 1.68 W & 1.54 W & 1.60 W \\
