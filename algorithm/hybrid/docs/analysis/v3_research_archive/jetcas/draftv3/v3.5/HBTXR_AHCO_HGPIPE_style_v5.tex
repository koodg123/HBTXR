...
\midrule
Primary sensing cue & Canonical frame with event-side summary & Event tensor with previous state token & Dual front-ends feed one shared token-processing substrate. \\
...
Active heads & Search, Mask, Eye-region, Gaze & Track, Event, Gaze & Head gating avoids Search-only computation during Track. \\
State handling & Commits a refreshed anchor and validity metadata & Reuses and updates the resident state SRAM & Mid-backbone buffers and state SRAM must survive across invocations. \\
\bottomrule
\end{tabularx}
\end{table*}

Table~\ref{tab:modechar} explains why the deployed accelerator is organized around mode asymmetry rather than around a single static graph. The first two rows establish that the two modes consume different sensing cues and different backbone depths. The next two rows show that they also activate different heads and therefore expose different control and memory behavior. The final row is equally important: Search creates a new anchor, whereas Track modifies the current anchor already stored on chip. This is why HBTXR requires explicit state SRAM, mode-aware scheduling, and an early-exit interface between the front and back halves of the backbone.
...
\begin{table}[t]
\caption{Approximation Policy for the Non-linear and Re-scaling Operators in HBTXR}
\label{tab:approx}
\centering
\footnotesize
\begin{tabularx}{\columnwidth}{@{}L{0.23\columnwidth}L{0.25\columnwidth}Y@{}}
\toprule
Operator & Approximation block & Role in the datapath \\
\midrule
LayerNorm denominator & Segmented reciprocal-root table & Generates the normalization factor from the quantized variance accumulator without floating-point division or square root. \\
Attention exponent & Max-referenced exponent table & Maps clipped score differences to exponent codes after max subtraction. \\
Attention sum inverse & Sum-inverse table & Converts the accumulated exponent sum to a normalization factor. \\
GELU + output re-scale & Composite activation table & Collapses activation shaping and output re-scaling into one table-driven stage. \\
Residual / head re-quantization & Dyadic re-scale path & Aligns intermediate tensors to the next stage using integer multiply-shift and saturation. \\
\bottomrule
\end{tabularx}
\end{table}

Table~\ref{tab:approx} is not only a summary of hardware-friendly operators; each row corresponds to a specific datapath transformation.
...
\centering
\small
\begin{tabularx}{\textwidth}{@{}L{0.10\textwidth}L{0.18\textwidth}L{0.15\textwidth}L{0.19\textwidth}L{0.16\textwidth}Y@{}}
\toprule
Mode & Active front-end & Backbone span & Memory behavior & Active heads & Scheduling implication \\
\midrule
Search & Frame tokenizer + event summary adapter & Full $L$ blocks & Writes a refreshed anchor to state SRAM and updates validity flags & Search, Mask, Eye-region, Gaze & Triggered at initialization or whenever quality falls below threshold \\
Track & Event tokenizer + previous-state injector & First $L/2$ blocks only & Reads and updates state SRAM; reuses the cut-point buffer & Track, Event, Gaze & Dominant steady-state mode for low-latency updates \\
Scheduled hybrid & CPU-selected path with mode packet & Mode dependent & Commit-on-success state update policy & Mode-gated activation & Balances global robustness and local update rate \\
\bottomrule
\end{tabularx}
\end{table*}
...
HBTXR uses two-stage training, $256\times256$ canonicalized inputs, a $2\times256\times256$ event tensor, fixed-count windows with 5000 events, and fast causal accumulation. Search pretraining is followed by hybrid fine-tuning with Search, Event, Track, Mask, gaze, and auxiliary supervision terms. The optimizer is AdamW with batch size 8, learning rate $3\times10^{-4}$, and weight decay $1\times10^{-4}$.
...
The dataset split, software/hardware environment, hyper-parameters, workstation setup, and runtime instrumentation are described directly in Section~V-A.
\end{tabularx}
\end{table}
...
\toprule
Mode & Accuracy & Success (\%) & Lat. (ms) & Rate (Hz) & Power (W) \\
\midrule
Search & 0.58 px / $0.91^{\circ}$ & 97.8 & 0.93 & 1075 & \TBD \\
Track & \textbf{0.42 px / $0.68^{\circ}$} & -- & \textbf{0.57} & \textbf{1754} & \TBD \\
Scheduled & 0.45 px / $0.71^{\circ}$ & 97.8 & 0.63 & 1587 & \TBD \\
\bottomrule
\end{tabularx}
\end{table}

The scheduled result is the average produced by the runtime policy of Section~III-D rather than a third independently trained network. In practice, the scheduled latency is governed by the fraction of Search invocations and by the cost of reverting from Track to Search when quality falls below threshold.

\subsubsection{Hardware Implementation Results (Resources, Power, Latency Breakdown)}
The current source draft verifies mode latency but does not yet include a finalized post-route implementation report. Instead of omitting the hardware-reporting slot, Table~\ref{tab:resource} explicitly records the ZCU104 target platform and leaves the final LUT/FF/BRAM/DSP and board-power entries to be filled from the implementation report. This makes the manuscript structurally complete without fabricating unsupported measurements.

\begin{table}[t]
\caption{ZCU104 Resource-Reporting Table for the HBTXR Prototype}
\label{tab:resource}
\centering
\scriptsize
\begin{tabularx}{\columnwidth}{@{}L{0.24\columnwidth}C{0.24\columnwidth}Y@{}}
\toprule
Resource / item & ZCU104 capacity & HBTXR report status \\
\midrule
FPGA device & XCZU7EV & Deployment target confirmed \\
LUT & 230,400 & \TBD\ (post-route utilization) \\
FF & 460,800 & \TBD\ (post-route utilization) \\
BRAM & 312 & \TBD\ (post-route utilization) \\
DSP & 1,728 & \TBD\ (post-route utilization) \\
Clock & Fixed PL clocks available & 200 MHz target implementation \\
Board power & -- & \TBD\ (board measurement pending) \\
Mode split & Shared front half + early-exit Track path & Verified by the latency measurements in Table~\ref{tab:mode} \\
\bottomrule
\end{tabularx}
\end{table}

The most important verified hardware fact at this stage is therefore architectural rather than purely numerical: the measured latency reduction comes from the shared front-half backbone, the early-exit Track path, and the mode-gated heads. Once the final implementation report is available, the explicit reporting slots in Table~\ref{tab:resource} can be populated without changing the surrounding narrative.
...
\centering
\scriptsize
\begin{tabularx}{\textwidth}{@{}L{0.12\textwidth}C{0.07\textwidth}L{0.12\textwidth}L{0.10\textwidth}L{0.23\textwidth}C{0.09\textwidth}C{0.11\textwidth}L{0.12\textwidth}@{}}
\toprule
...
Resource util. & \makecell{512 MAC\\316 KB SRAM} & \makecell{130K LUT\\90K FF\\1092 BRAM\\1606 DSP} & \makecell{0.28 mm$^2$\\ASIC area} & \makecell{669K LUT\\312 DSP\\1006.5 BRAM} & \makecell{64.8K LUT\\43.7K FF\\398 DSP\\781 KB OCM} & \TBD & \TBD & \TBD \\
...
Power cons. & 0.154 W & 3.86 W & 0.0377 W & 46.7 W & 1.91 W & \TBD & \TBD & \TBD \\
Throughput & 240 FPS & 1428 FPS$^\dagger$ & 2000 FPS & 7118 FPS & 79.8 FPS & 1075 Hz & 1754 Hz & 1587 Hz \\
Power eff. & -- & 2.29 mJ/inf. & 567 GOP/s/W & 381.0 GOP/s/W & 80.5 GOPS/W & -- & -- & -- \\
Frame eff. & 1555 FPS/W$^\ddagger$ & 370 FPS/W$^\ddagger$ & 53.0K FPS/W$^\ddagger$ & 152 FPS/W$^\ddagger$ & 41.9 FPS/W & \TBD & \TBD & \TBD \\
\bottomrule
\end{tabular}}
\vspace{1mm}
...
The comparison in Table~\ref{tab:accel} clarifies the design position of HBTXR without requiring an extra ``Positioning w.r.t. HBTXR'' column. EyeCoD is a frame-dominant predict--focus pipeline and therefore represents what can be achieved when semantic robustness is prioritized around frame processing. SEE-D and JaneEye show the efficiency ceiling of event-centric designs with sparse or ASIC-specialized datapaths. HG-PIPE~\cite{ref11} is a transformer-acceleration baseline that validates aggressive pipeline overlap, tiling, and LUT-oriented non-linear processing for high-throughput ViT inference. AHCO-YOLO~\cite{ref12} demonstrates how a ZCU104-targeted algorithm--hardware co-optimization flow can yield balanced FPS/W and GOPS/W on a detector workload. Relative to these points, HBTXR is distinguished by \emph{mode-asymmetric execution}: it trades uniform one-pass throughput for Search/Track specialization, early exit, and scheduler-visible control semantics.
