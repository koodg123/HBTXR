...
아래는 `Introduction`부터 `Conclusion`까지 이어지는 연속 `LaTeX` 본문입니다.  
표 수치는 이전과 동일하게 **draft placeholder** 기준이며, 제출 전 실측값으로 교체해야 합니다.
...
다음으로 가장 필요한 작업은 두 가지입니다.

1. 이 본문에 맞춰 `\bibitem` 또는 `.bib` 초안을 정리하기  
2. 실제 실험 로그에 맞춰 표 수치와 본문 퍼센트를 최종 교체하기