...
- Added comprehensive project synthesis at `docs/resources/current_project_synthesis_2026_06_27.md`.
- Added dated handover at `docs/HANDOVER_2026_06_27.md`.
- Added root `HANDOVER.md` pointer for fast continuation.
- Updated `docs/Master-Plan.md`, `docs/Sub-Plan.md`, `docs/Spec.md`, `docs/Validation.md`, `docs/track/PROGRESS.md`, `docs/track/CONVERSATION.md`, and `docs/track/log.md` with the current Stage1 authority, S1-OC status, and continuation runbook.
- Confirmed that S1-OC is implemented and smoke-validated, but no real `*s1oc*` run/log artifacts exist yet.
...
- Added `scripts/external/eval_eyelorin_refinement.py` for EyeLoRiN-style M2F median refinement evaluation with time-gap gating, before/after center/P10/P5 metrics, and jitter proxy metrics.
...
- Added `scripts/external/run_xr03_ellipsestate_adamw_leader_probe.sh` for AdamW-leader warm-start ellipse-state axis/angle loss probing.
- Added `scripts/external/compare_xr01_adamw_bracket.py` and made the XR-03 runner init checkpoint overrideable for XR-01-promoted leaders.
...
- Extended `scripts/external/compare_xr01_adamw_bracket.py` to scan the next fixed255k LR `6e-6` and `1e-5` branch.
- Started XR-01 fixed255k LR bracket in tmux: `6e-6` on GPU0 and `1e-5` on GPU1.
- Added `scripts/external/summarize_eval_failure_buckets.py` and generated `runs/diagnostics/xr04_failure_buckets_adamw255k_leader_test_20260616.json`; low-similarity rows and specific `session_201` buckets are now identified as the dominant leader failure modes.
...
- Added `configs/external/mode1_stage2_raw_event_count_lr5e-6_weakdistill_trackonly_trackstateaux_finetune_fullwidth.yaml` and `scripts/external/run_xr06_weakdistill_trackstateaux_probe.sh` as a validated weak-distill fallback.
- Added `scripts/external/run_xr07_trackstateaux_centerhinge_probe.sh` and launched XR-07A/B tiny center-hinge recovery from XR-05B on GPU1/GPU0.
...
- Added `scripts/external/check_xr02_dense_trajectory_availability.py` and generated `runs/diagnostics/xr02_dense_trajectory_availability_20260616_084506.json`.
...
- Added `--limit` to `scripts/external/infer_hbtxr.py` for bounded confidence/quality diagnostic smoke runs.
- Next priority moves to a no-train `track_pred` confidence/quality diagnostic before any new training branch.
- Added `scripts/external/build_confidence_probe_manifest.py` and built `data/_internal/manifests/manifest1/confidence_probe_low0p1_high0p6_128/test_manifest.jsonl` with 128 low-sim and 128 high-sim test rows.
...
- Launched XR-15A support-adaptive center lane on GPU0 and P10 lane on GPU1. Both lanes passed startup validation. Added `scripts/external/eval_xr15_p5_checkpoint.sh` for post-train best-P5 evaluation.
...
- Added and launched XR-15D track-adapter/coordinate-head probe on GPU0/GPU1. Runner `scripts/external/run_xr15d_trackadapters_probe.sh` passed syntax/contract/model-build checks; both lanes entered epoch `1/16`.
...
- Closed XR-31 no-train interpolation between XR-29 LR `1.75e-4` and XR-30 LR `1.875e-4`. Alpha `0.25` promoted P10 to `32.5795` with center `17.0466`, but P5 `11.5038` narrowly missed the strict XR-29 P5 gate. Added `scripts/external/run_xr31_xr29_xr30_heatmap_interp_eval.sh` and `docs/resources/xr31_xr29_xr30_interpolation_results_2026_06_16.md`; next P0 is trained LR `1.8125e-4`.
...
- Added and launched XR-34 heatmap loss-ratio refinement. XR-34A uses XR-29 center/P5 anchor with heatmap/offset/center `0.004/0.0015/0.0015` on GPU0; XR-34B uses XR-33 P10 anchor with `0.006/0.001/0.001` on GPU1. Added `scripts/external/run_xr34_heatmap_lossratio_refinement.sh` and `docs/resources/xr34_heatmap_lossratio_plan_2026_06_16.md`.
...
- Closed XR-59 with no promotion. Added `scripts/external/eval_xr59_completed_checkpoints.sh` for post-train checkpoint eval; best XR-59 P10 was `34.43409944261823`, below XR-58A and XR-39.
...
- Added `docs/track/EXTERNAL_HYBRID_PACKAGE_PAST_EXPERIMENT_RESULTS.md`, consolidating legacy `external_hybrid_package/docs` experiment results and mapping them to current raw event-count training decisions.
...
- Added `scripts/external/run_xr60_p10_candidate_head.sh`.
...
- Added `scripts/external/write_xr_eye_tracking_detailed_analysis.py`.
...
- Added `scripts/external/run_xr62_facet_geometry_aux_refresh.sh`.
...
- Updated `scripts/external/write_xr_eye_tracking_detailed_analysis.py` to generate codebase reports closer to the requested FlexLLM-style reference, including repository metadata, paper linkage, core source responsibility, HGTXR contract matrices, quality findings, and reproduction runbooks.
...
- Added `scripts/external/analyze_p10_teacher_target_oracle.py`.
...
- Added target override builder: `scripts/external/build_xr64_teacher_target_overrides.py`.
- Added XR-64 runner: `scripts/external/run_xr64_teacher_target_construction.sh`.
