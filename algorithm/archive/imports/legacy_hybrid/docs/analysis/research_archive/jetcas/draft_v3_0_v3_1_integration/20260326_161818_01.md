Role: JETCAS 논문 아키텍트, 하드웨어 시스템 연구자, 기술 문서 감사자

**분석**
- 베이스는 [draft_v3_1_260326/90_all.md](E:\WSL\Shared\ETRI_SYNC\HBTXR\paper_works\CODE\HBTXR_v3_0\docs\jetcas\draft_v3_1_260326\90_all.md)가 맞다.
- 이유는 `Algorithm -> Accelerator -> Experimental Results` 흐름이 더 논문답고, 하드웨어 서사가 더 설득력 있기 때문이다.
- 다만 v3_1은 `Role:` 메타, 한국어 설명, fenced code block이 섞여 있어 원고 청결성이 떨어진다.
- 따라서 [draft_v3_0_260326/90_all.md](E:\WSL\Shared\ETRI_SYNC\HBTXR\paper_works\CODE\HBTXR_v3_0\docs\jetcas\draft_v3_0_260326\90_all.md)의 `Abstract`, `IEEEkeywords`, 일부 float 자산만 흡수하는 통합 전략이 적절하다.

**전략 설정**
- `v3_1 본문 구조`를 유지한다.
- `v3_0 front matter`를 붙인다.
- 실험 수치와 우위 퍼센트는 유지하되, `% TODO`로 실제 로그 검증 필요성을 명시한다.
- 지금은 파일 수정 없이, 제출용으로 바로 다듬기 쉬운 **통합 LaTeX 골격**을 화면에 출력한다.

**실행 가능성 판단**
- 지금 단계에서 바로 가능한 것은 `통합 원고 구조 LaTeX` 출력이다.
- 실제 제출 전 필수 작업은 두 가지다.
- `1)` 메타 오염 제거 후 파일화
- `2)` 표 수치와 본문 퍼센트의 실측 로그 대조

아래가 통합본 구조안이다.

```latex
% ======================================================================
% Integrated JETCAS manuscript skeleton
% Base narrative: draft_v3_1_260326/90_all.md
% Front matter source: draft_v3_0_260326/90_all.md
% NOTE: Quantitative values below are draft placeholders until verified.
% ======================================================================

\begin{abstract}
Eye tracking is a key enabling technology for extended reality (XR), supporting gaze-based interaction, foveated rendering, and privacy-preserving on-device perception~\cite{Plopski2022EyeXR}. However, existing approaches still suffer from a structural trade-off between semantic robustness and temporal responsiveness. Frame-based methods provide stable visual cues but incur high bandwidth cost and relatively large latency~\cite{Etracker2018,HETracker2025}, whereas event-based methods offer high temporal resolution but remain limited in relocalization stability and representation consistency~\cite{EGaze2022,ETrack2021,FACET2024}. To address these limitations, this paper presents \textbf{HBTXR}, an algorithm-hardware co-designed hybrid eye-tracking system that tightly integrates \emph{Frame Search} and \emph{Event Track} for on-device XR applications. HBTXR is built upon four key components: 1) a geometry-stable hybrid supervision framework that stabilizes geometric targets across heterogeneous frame-event inputs, 2) a hybrid Search-Track transformer with a shared backbone and decoupled prediction heads, 3) a runtime-aware Search/Track scheduler for adaptive low-latency execution, and 4) a mode-aware mixed-granularity deployment architecture with feature reuse and low-overhead runtime control. Experimental results show that HBTXR achieves a pupil-center error of \textbf{0.42 pixels}, a gaze error of \textbf{0.68$^\circ$}, and an end-to-end latency of \textbf{0.57 ms} in Track mode. These results indicate that HBTXR provides a practical solution for real-time on-device eye tracking in next-generation XR systems.
\end{abstract}
...
Event-based eye tracking exploits the asynchronous sensing property of event cameras to achieve low-latency and low-redundancy processing~\cite{EGaze2022,ETrack2021,FACET2024,SwiftEye2024,SEEN2025}. Representative methods such as E-Track directly estimate pupil states from event representations~\cite{ETrack2021}, while FACET demonstrates that geometry-aware event modeling can yield efficient and accurate pupil localization under dense supervision~\cite{FACET2024}. Other works improve robustness or efficiency through anti-blink design, sparse processing, or spiking computation~\cite{SwiftEye2024,EventSpiking2025}.
...
A separate but increasingly important direction focuses on making eye tracking practical for embedded and XR devices through deployment-aware design and hardware acceleration~\cite{EyeCoD2024,SEE2024,JaneEye2025,SubMilliMCU2025,Retina2024}. EyeCoD demonstrates optics-algorithm co-design for compact eye-tracking systems~\cite{EyeCoD2024}, whereas SEE and JaneEye show that event-driven eye tracking can be accelerated effectively on FPGA or ASIC-style hardware~\cite{SEE2024,JaneEye2025}. These studies confirm that low-latency eye tracking must be evaluated not only at the model level, but also at the execution and data-movement levels.

In parallel, recent transformer acceleration studies have shown that deployment efficiency depends strongly on memory access patterns, synchronization overhead, and execution granularity~\cite{HGPIPE2025,HPTA2023,Butterfly2022}. These observations are directly relevant to hybrid eye tracking, where refresh-oriented Search and reuse-oriented Track exhibit different execution characteristics. However, prior deployment-oriented eye-tracking systems have largely focused either on frame-based CNN pipelines or on event-only sparse paths, leaving a gap for hybrid Search/Track systems with transformer-oriented execution structure.
...
Let \(I_t \in \mathbb{R}^{H \times W}\) denote the frame acquired at time \(t\), and let
\begin{equation}
...
\label{eq:state_def}
\end{equation}
where \((x_t,y_t)\) is the pupil center, \((a_t,b_t)\) are the ellipse axes, and \((u_t,v_t)\) is a trigonometric orientation encoding:
\begin{equation}
[u_t, v_t] = [\sin(2\theta_t), \cos(2\theta_t)].
\label{eq:uv_def}
\end{equation}

HBTXR formulates hybrid eye tracking as a Search-and-Track problem rather than as a single monolithic prediction task. The Search path is responsible for robust relocalization and anchor refresh using frame-based semantic cues, while the Track path performs low-latency residual update from the previous state using event observations.

\subsection{Geometry-Stable Hybrid Supervision}

Each raw sample is converted into a canonical training representation containing an eye region, pupil mask, ellipse annotation, and quality metadata. Let \(\mathcal{T}(\cdot)\) denote the canonicalization operator. The training sample is constructed as
\begin{equation}
...
\end{equation}
where \(\bar{I}_t\) and \(\bar{V}_t\) denote the canonicalized frame and event inputs, \(\bar{M}_t\) is the canonical pupil mask, \(\bar{B}_t\) is the canonical eye-region target, and \(\bar{\xi}_t\) collects supervision-quality signals.

\subsection{Hybrid Search-Track Transformer}

The proposed HBTXR network uses modality-specific embedding layers and adapters followed by a shared transformer backbone. For the frame branch,
\begin{equation}
...
\end{equation}
where \(\mathcal{B}(\cdot)\) is the shared Partial DeiT-Tiny backbone~\cite{Touvron2021DeiT}.

On top of the shared backbone, HBTXR uses decoupled prediction heads: an Eye Region Head, a Pupil Search Head, an Event Search Head, a Pupil Track Head, a Search Mask Head, and an Auxiliary State Head.
...
The Track branch combines the event feature \(\mathbf{p}^{e}_t\) with an encoded previous state \(\psi(\bar{\mathbf{s}}_{t-1})\) and predicts a residual vector:
\begin{equation}
...
HBTXR is trained in two stages. In the first stage, the model is trained primarily for Search-oriented relocalization. In the second stage, the hybrid network is fine-tuned with Track supervision, event-conditioned estimation, and cross-branch consistency.
...
For both search and tracking, HBTXR uses geometry-aware supervision in addition to standard center and axis regression:
\begin{equation}
...
\end{equation}
where \(\mathcal{B}(\cdot)\) is realized by a common transformer engine.

\subsection{Memory Organization and Reuse-Oriented Dataflow}

Search mode uses a refresh buffer pair to support broader feature regeneration and robust relocalization. Track mode uses a streaming token queue and persistent state buffers so that recently computed context can be reused without re-materializing the entire inference path.
...
To support mode-aware execution, the accelerator is coupled with an FSM-based runtime controller that interprets the host-side Search/Track decisions and dispatches the corresponding execution path.
...
\end{equation}
where \(T_{\mathrm{host}}\) denotes host-side control and scheduling time, \(T_{\mathrm{transfer}}\) denotes host-device communication, \(T_{\mathrm{exec}}\) denotes accelerator execution time, and \(T_{\mathrm{sync}}\) denotes synchronization overhead. The average runtime under scheduler-driven operation can be expressed as
\begin{equation}
T_{\mathrm{avg}}
=
...
We evaluate HBTXR on a near-eye hybrid eye-tracking benchmark composed of synchronized frame observations, event streams, and dense pupil annotations. Each sample contains a canonicalized frame patch, a polarity-split event tensor, the previous tracking state, and geometry-aware supervision targets including the pupil mask, eye region, and current pupil state.

\subsubsection{Training and Runtime Configuration}

HBTXR is trained in two stages. In the first stage, the model is optimized for Search-oriented relocalization. In the second stage, the hybrid network is fine-tuned with Search, Event, and Track supervision jointly, together with consistency and quality-aware gating.

\subsubsection{Evaluation Metrics}

We evaluate tracking accuracy using pupil-center error and gaze angular error:
\begin{equation}
...
We compare HBTXR with representative frame-based, event-based, hybrid, and deployment-oriented eye-tracking systems.
...
% TODO: Replace all numbers in this subsection with measured values.
...
% TODO: Verify all baseline numbers and percentage improvements using actual logs and cited sources.
...
% TODO: Verify deployment comparison values against measured hardware/runtime reports.
...
Experimental results demonstrated that HBTXR achieves a pupil-center error of \textbf{0.42 pixels}, a gaze error of \textbf{0.68$^\circ$}, and an end-to-end latency of \textbf{0.57 ms} in Track mode, while outperforming strong prior event-based and hybrid baselines by approximately 10--15\% across key accuracy and latency metrics. These results confirm that HBTXR provides a practical balance among semantic robustness, temporal responsiveness, and deployment-oriented efficiency.
...
**핵심 정리**
- 이 출력은 `v3_1 본문 구조 + v3_0 front matter` 통합형이다.
- 지금 상태에서 가장 큰 리스크는 수치 검증이다.
- LaTeX 원고로 바로 옮길 때 먼저 제거해야 할 것은 `v3_1`의 메타 오염이다.

**장점**
- JETCAS 구조에 맞음
- 하드웨어 서사가 v3_0보다 설득력 있음
- front matter가 살아 있음

**단점**
- 표 수치와 퍼센트 우위는 아직 검증되지 않음
- 일부 본문은 더 압축해야 10페이지 안착 가능성이 높음

**진행 상황 체크리스트**
- [x] 두 드래프트 비교 분석 완료
- [x] 베이스 버전 선정 완료
- [x] 통합 전략 수립 완료
- [x] 화면용 LaTeX 통합 구조 출력 완료
- [ ] 실제 원고 파일로 정리
- [ ] 메타 오염 제거
- [ ] 실험 수치 검증 및 교체

다음 단계는 이 내용을 실제 `clean all.md` 또는 `.tex` 형태로 정리하는 것이다.