...
from swift_hbtxr.dataset import SwiftHBTXRDataset
from swift_hbtxr.event_repr import build_event_frame
...
def test_dataset_supports_fixed_count_and_time_bin(synthetic_workspace):
    ...
    fixed_dataset = SwiftHBTXRDataset(
        str(manifest_path),
    ...
    time_dataset = SwiftHBTXRDataset(
        str(manifest_path),
        canonical_root=str(synthetic_workspace["canonical_root"]),
    ...
    assert fixed_sample["ellipse_xywht"].shape[-1] == 5
    assert fixed_sample["state6"].shape[-1] == 6
    assert fixed_sample["meta"]["source_frame_path"].endswith(".png")
    assert fixed_sample["meta"]["interpolated_frame_path"].endswith(".png")
    assert fixed_sample["meta"]["antiblink_source"] == "gsa_reviewed"
