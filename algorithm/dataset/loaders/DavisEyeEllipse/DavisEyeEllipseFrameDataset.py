...
from dataset.loaders.DavisEyeEllipse.DavisEyeEllipseDataset import (
    draw_umich_gaussian,
    gaussian_radius,
)
from dataset.loaders.DavisEyeEllipse.utils import cal_ellipse_area, convert_to_ellipse
from utils.cache.MemmapCacheStructedEvents import load_memmap
from utils.tonic.functional.ToFrameStack import to_frame_stack_numpy
...
class DavisEyeEllipseFrameDataset(Dataset):
    def __init__(
        ...
        self.use_hybrid = use_hybrid                              # APS(1ch) + event(2ch backward) -> 3ch
        ...
        if not (self.use_cached_events or self.use_hybrid) and self.channels not in (1, 3):
        ...
        self.aps_paths = self._load_aps_paths() if (self.use_cached_aps or self.use_hybrid) else None
        self.ev_mm, self.ev_idx = self._load_events() if (self.use_cached_events or self.use_hybrid) else (None, None)
        self.win_start, self.frame_ts = (
            self._load_event_meta() if ((self.use_cached_events or self.use_hybrid) and self.event_accum != "frame") else (None, None)
        )
        self.num_samples = len(self.ellipses)
        if self.use_cached_aps or self.use_hybrid:
    ...
    def _load_ellipses(self):
        if self.use_cached_aps or self.use_cached_events or self.use_hybrid:   # crop dataset: plain .npy [t,x,y,a,b,ang] (crop coords)
    ...
    def _ensure_memmaps(self):
        ...
        if (self.use_cached_events or self.use_hybrid) and self.ev_mm is None:
    ...
    def __getitem__(self, index):
        ...
        hybrid_ev = None
        if self.use_hybrid:                                        # APS(1ch) drives transform; event(2ch) concat later
            frame = self._load_frame(self.aps_paths[index])
            if self.event_accum == "frame":
                s, e = self.ev_idx[index]; ev = self.ev_mm[int(s):int(e)]
            else:
                ev = self._ev_backward(index)                      # causal backward count/time within window
            hybrid_ev = self._render_event(ev)                     # (160, 240, 2)
            image_shape = frame.shape
        elif self.use_cached_aps:
        ...
        if self.use_hybrid:                                        # same geometric transform on the event frame (alignment)
            hybrid_ev = A.ReplayCompose.replay(replay, image=hybrid_ev)["image"]
        ...
        if self.use_hybrid:
            aps = np.expand_dims(frame.astype(np.float32) / 255.0, 0)         # (1, H, W) APS
            evc = np.moveaxis(hybrid_ev.astype(np.float32) / 255.0, -1, 0)    # (2, H, W) event
            frame = np.concatenate([aps, evc], axis=0)                         # (3, H, W)
        elif self.use_cached_events:
