...
from models.postprocess.ellipse_predict import topk
...
def sqrt_newton_schulz_autograd(A, numIters, dtype=None):
    batchSize = A.data.shape[0]
    dim = A.data.shape[1]
    normA = A.mul(A).sum(dim=1).sum(dim=1).sqrt()
    Y = A.div(normA.view(batchSize, 1, 1).expand_as(A))
    I = Variable(
        torch.eye(dim, dim, device=A.device, dtype=A.dtype)
        .view(1, dim, dim)
...
def gwds_loss(pred, target, weight, eps=1e-6):
    ...
    wasserstein_distance = torch.max(
        ...
        Variable(
            wasserstein_distance_item1.new_zeros(wasserstein_distance_item1.shape[0]),
            requires_grad=False,
        ),
    )
    wasserstein_distance = torch.max(
...
class GWDLoss(nn.Module):
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        K = 100
        _, inds, _, y, x = topk(pred_tensor["hm"])
        ...
        _, _, _, y, x = topk(target_tensor["hm"])
...
class WeightLoss(nn.Module):
    ...
    def forward(self, pred):
        target = pred.new_tensor(np.array([0.4, 0.3, 0.2, 0.1] * 2, dtype=np.float32))
...
class CtdetLoss(torch.nn.Module):
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        # Get the mask for the close tensor
        valid_mask = target_tensor["close"] == 0

        if valid_mask.any():  # Compute the loss only when the mask contains True values
...
# HBTXR historically named this loss HBTXRCtdetLoss; keep the alias.
HBTXRCtdetLoss = CtdetLoss
