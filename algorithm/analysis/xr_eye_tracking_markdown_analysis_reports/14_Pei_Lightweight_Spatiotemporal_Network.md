# Paper Summary: A Lightweight Spatiotemporal Network for Online Eye Tracking with Event Camera

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `Pei_A_Lightweight_Spatiotemporal_Network_for_Online_Eye_Tracking_with_Event_CVPRW_2024_paper.pdf` |
...
| **Venue** | CVPRW |
| **Year** | 2024 |
| **Research Field** | Causal spatiotemporal event eye tracking |
...
- **Target Problem:** event-based online eye tracking에서 temporal information을 활용하면서도 edge hardware에 적합한 causal/low-latency network가 필요한 문제.
- **Research Objective:** streaming inference 가능한 causal spatiotemporal convolutional network 제안.
...
- **Core Idea:** causal event volume binning과 (1+2)D temporal+spatial convolution blocks, CenterNet-like head를 사용해 online pupil center prediction을 수행한다.
- **Proposed Approach:** event streams를 10ms causal event frames로 binning한다. temporal blocks는 FIFO buffer로 streaming 처리할 수 있으며, spatial blocks가 공간 특징을 추출한다. final grid head는 pupil presence focal loss와 SmoothL1 offset regression을 사용한다.
...
1. causal spatiotemporal CNN backbone 제안
2. FIFO buffer 기반 streaming inference 구조 설명
3. CenterNet-like detection head와 event affine augmentation 적용
4. AIS 2024 challenge high p10 performance 달성

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| AIS 2024 challenge | Pupil tracking | p10 public/private/validation/overall | 0.988 / 0.992 / 0.963 / 0.9898 | challenge submission | Table 1 |
| Validation ablation | Model architecture | p10 / dist / params / MACs | 0.963 / 2.79 / 809K / 55.2M | benchmark causal event volume | Table 4 |
| Validation ablation | Conv3D variant | p10 / dist / params / MACs | 0.969 / 2.50 / 1.21M / 267M | higher accuracy, much higher compute | Table 4 |

## 1.5 Brief Overview

> 이 논문은 Causal spatiotemporal event eye tracking 분야에서 event-based online eye tracking에서 temporal information을 활용하면서도 edge hardware에 적합한 causal/low-latency network가 필요한 문제.를 해결하기 위해 causal event volume binning과 (1+2)D temporal+spatial convolution blocks, CenterNet-like head를 사용해 online pupil center prediction을 수행한다.라는 설계 방향을 제안한다. 핵심 방법은 event streams를 10ms causal event frames로 binning한다. temporal blocks는 FIFO buffer로 streaming 처리할 수 있으며, spatial blocks가 공간 특징을 추출한다. final grid head는 pupil presence focal loss와 SmoothL1 offset regression을 사용한다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 streaming inference 가능한 causal spatiotemporal convolutional network 제안.에 초점을 맞춘다.
...
- **Primary axis:** Causal spatiotemporal event eye tracking
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `causal event volume binning과 (1+2)D temporal+spatial convolution blocks, CenterNet-like head를 사용해 online pupil center prediction을 수행한다.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| Causal event volume binning | future event를 사용하지 않는 event representation | online inference causality | Fig. 3.C |
| (1+2)D spatiotemporal block | temporal conv와 spatial conv를 factorized 구성 | 효율적 temporal/spatial feature extraction | Fig. 3.A |
| CenterNet-like head | grid cell별 pupil probability와 offset 회귀 | coordinate localization | Method/Loss |
...
event streams를 10ms causal event frames로 binning한다. temporal blocks는 FIFO buffer로 streaming 처리할 수 있으며, spatial blocks가 공간 특징을 추출한다. final grid head는 pupil presence focal loss와 SmoothL1 offset regression을 사용한다.
...
- spatiotemporal block sequence
- CenterNet-like detection head
- focal + SmoothL1 loss
- streaming inference with temporal FIFO
...
이 논문은 event-based online eye tracking에서 temporal information을 활용하면서도 edge hardware에 적합한 causal/low-latency network가 필요한 문제.를 대상으로 causal event volume binning과 (1+2)D temporal+spatial convolution blocks, CenterNet-like head를 사용해 online pupil center prediction을 수행한다.를 제안한다.
...
- causal online inference를 명시적으로 고려
- accuracy/compute ablation이 비교적 명확
- L1 activation regularization으로 sparsity 유도 가능성을 논의

## 7.5 Weak or Unclear Parts

- PyTorch inference가 unoptimized라고 명시되어 실제 edge latency는 별도 구현 필요
- Conv3D는 정확도는 좋지만 compute가 크게 증가
- challenge dataset 외 일반화는 추가 검증 필요
...
> A Lightweight Spatiotemporal Network for Online Eye Tracking with Event Camera는 Causal spatiotemporal event eye tracking 관점에서 causal event volume binning과 (1+2)D temporal+spatial convolution blocks, CenterNet-like head를 사용해 online pupil center prediction을 수행한다.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** causal event volume binning과 (1+2)D temporal+spatial convolution blocks, CenterNet-like head를 사용해 online pupil center prediction을 수행한다.
...
- **Evidence:** Cons list: PyTorch inference가 unoptimized라고 명시되어 실제 edge latency는 별도 구현 필요, Conv3D는 정확도는 좋지만 compute가 크게 증가, challenge dataset 외 일반화는 추가 검증 필요
