# Paper Summary: RITnet: Real-time Semantic Segmentation of the Eye for Gaze Tracking

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `RITnet Real-time Semantic Segmentation of the Eye for Gaze Tracking.pdf` |
...
| **Venue** | IEEE ICCV Workshop / eye segmentation paper |
| **Year** | 2019 |
| **Research Field** | Frame-based eye semantic segmentation |
...
- **Target Problem:** gaze tracking을 위한 eye segmentation은 person-dependent accuracy, annotation scarcity, real-time constraint 문제가 있음.
- **Research Objective:** compact real-time semantic segmentation network로 eye components를 segment하여 gaze tracking downstream을 지원.
...
- **Core Idea:** U-Net/DenseNet-inspired compact architecture와 pupil/iris/eye semantic labels를 사용해 OpenEDS에서 높은 segmentation accuracy를 달성한다.
- **Proposed Approach:** encoder-decoder segmentation network로 eye image를 semantic mask로 변환한다. postprocessing/ellipse fitting과 결합하면 pupil center/gaze estimation에 사용할 수 있다.
...
1. real-time eye semantic segmentation network RITnet 제안
2. OpenEDS dataset에서 compact model로 high accuracy 달성
3. gaze tracking을 위한 pupil/iris/eyelid segmentation baseline 제공
4. 후속 EyeCoD/segmentation-based tracker의 baseline으로 활용 가능

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| OpenEDS | Eye semantic segmentation | Accuracy | 95.3% | paper-reported | Abstract/Experiments |
| GTX 1080Ti | Inference speed | FPS | >300Hz | paper-reported real-time speed | Abstract/Experiments |
| Model size | Deployment | Model size | <1 MB | compact segmentation network | Abstract |

## 1.5 Brief Overview

> 이 논문은 Frame-based eye semantic segmentation 분야에서 gaze tracking을 위한 eye segmentation은 person-dependent accuracy, annotation scarcity, real-time constraint 문제가 있음.를 해결하기 위해 U-Net/DenseNet-inspired compact architecture와 pupil/iris/eye semantic labels를 사용해 OpenEDS에서 높은 segmentation accuracy를 달성한다.라는 설계 방향을 제안한다. 핵심 방법은 encoder-decoder segmentation network로 eye image를 semantic mask로 변환한다. postprocessing/ellipse fitting과 결합하면 pupil center/gaze estimation에 사용할 수 있다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 compact real-time semantic segmentation network로 eye components를 segment하여 gaze tracking downstream을 지원.에 초점을 맞춘다.
...
- **Primary axis:** Frame-based eye semantic segmentation
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `U-Net/DenseNet-inspired compact architecture와 pupil/iris/eye semantic labels를 사용해 OpenEDS에서 높은 segmentation accuracy를 달성한다.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| Semantic eye segmentation | pupil/iris/sclera/background 등 eye parts를 pixel-level 분류 | downstream pupil/gaze estimation | Method |
| Compact encoder-decoder | real-time segmentation을 위한 경량 architecture | low latency inference | Method |
| OpenEDS | near-eye semantic segmentation benchmark | evaluation dataset | Experiments |
...
encoder-decoder segmentation network로 eye image를 semantic mask로 변환한다. postprocessing/ellipse fitting과 결합하면 pupil center/gaze estimation에 사용할 수 있다.
...
- near-eye frame input
- RITnet semantic segmentation
- pupil/iris region extraction
- optional ellipse fitting / gaze tracking downstream
...
이 논문은 gaze tracking을 위한 eye segmentation은 person-dependent accuracy, annotation scarcity, real-time constraint 문제가 있음.를 대상으로 U-Net/DenseNet-inspired compact architecture와 pupil/iris/eye semantic labels를 사용해 OpenEDS에서 높은 segmentation accuracy를 달성한다.를 제안한다.
...
- frame-based eye segmentation baseline으로 중요
- model size와 speed가 우수해 embedded preprocessing 후보
- pupil/iris semantic information이 풍부해 event-frame hybrid 초기화에 유용

## 7.5 Weak or Unclear Parts

- event-based high-frequency tracking 자체는 다루지 않음
- segmentation metric이 tracking/gaze metric과 직접 동일하지 않음
- frame sensor/frame-rate/power 제약은 그대로 남음
...
> RITnet: Real-time Semantic Segmentation of the Eye for Gaze Tracking는 Frame-based eye semantic segmentation 관점에서 U-Net/DenseNet-inspired compact architecture와 pupil/iris/eye semantic labels를 사용해 OpenEDS에서 높은 segmentation accuracy를 달성한다.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** U-Net/DenseNet-inspired compact architecture와 pupil/iris/eye semantic labels를 사용해 OpenEDS에서 높은 segmentation accuracy를 달성한다.
...
- **Evidence:** Cons list: event-based high-frequency tracking 자체는 다루지 않음, segmentation metric이 tracking/gaze metric과 직접 동일하지 않음, frame sensor/frame-rate/power 제약은 그대로 남음
