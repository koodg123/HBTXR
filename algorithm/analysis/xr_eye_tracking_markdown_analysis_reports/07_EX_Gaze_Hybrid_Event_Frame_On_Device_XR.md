# Paper Summary: EX-Gaze: High-frequency and Low-latency Gaze Tracking with Hybrid Event-frame Cameras for On-Device Extended Reality

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `EX-Gaze_High-Frequency_and_Low-Latency_Gaze_Tracking_with_Hybrid_Event-Frame_Cameras_for_On-Device_Extended_Reality.pdf` |
...
| **Authors** | Ning Chen, Yiran Shen, Tongyu Zhang, Yanni Yang, Hongkai Wen |
| **Venue** | IEEE TVCG |
| **Year** | 2025 |
| **Research Field** | On-device XR gaze tracking with hybrid event-frame cameras |
| **Paper Link / DOI** | 논문 PDF 기준 분석; DOI는 파일별로 상이하거나 논문에 명시되지 않음 |
| **Code / Project Link** | https://github.com/Ningreka/EX-Gaze |
...
- **Target Problem:** XR용 on-device gaze tracking은 kHz-level frequency와 low latency를 요구하지만 frame-only system은 slow/expensive이고 event-only system은 semantic robustness가 부족한 문제.
- **Research Objective:** Jetson Orin Nano에서 2KHz real-time tracking을 달성하는 event-frame hybrid system 제안.
...
- **Core Idea:** frame-based initialization/relocalization, Sparse Event-Patch Transformer (SEPT), gradient-based approach-adaptation, polynomial gaze regression을 조합한다.
- **Proposed Approach:** frame-based MobileNetV3-small detector가 eye/pupil region을 초기화하고, event-based SEPT가 0.5ms interval에서 pupil ellipse offset을 갱신한다. similarity threshold가 낮으면 frame-based relocalization을 수행하며, 최종 pupil center는 polynomial regression으로 PoG에 매핑된다.
...
1. hybrid event-frame camera 기반 on-device EX-Gaze system 제안
2. sparse event-patch representation과 SEPT로 event tracking 가속
3. gradient similarity 기반 adaptation으로 frame relocalization을 선택적으로 수행
4. Jetson Orin Nano에서 2KHz tracking과 평균 0.407ms 수준 runtime 제시

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| EV-Eye | Event-based tracking threshold | IoU / F1 / Pixel Error | 0.880 / 0.933 / 1.183 at THec=50 | threshold ablation | Table 1 |
| EV-Eye | Approach adaptation | Pixel Error / relocalization probability | 1.412 / 8.1% at THs=0.80 | accuracy-efficiency compromise | Table 2 |
| Jetson Orin Nano | Runtime | frame relocalization / event tracking / adaptation | 1.17ms / 0.32ms / 0.02ms | module-wise measurement | Evaluation runtime table |
| Jetson Orin Nano | Tracking frequency | Frequency | 2KHz | 0.5ms update interval | Abstract/Section 3 |

## 1.5 Brief Overview

> 이 논문은 On-device XR gaze tracking with hybrid event-frame cameras 분야에서 XR용 on-device gaze tracking은 kHz-level frequency와 low latency를 요구하지만 frame-only system은 slow/expensive이고 event-only system은 semantic robustness가 부족한 문제.를 해결하기 위해 frame-based initialization/relocalization, Sparse Event-Patch Transformer (SEPT), gradient-based approach-adaptation, polynomial gaze regression을 조합한다.라는 설계 방향을 제안한다. 핵심 방법은 frame-based MobileNetV3-small detector가 eye/pupil region을 초기화하고, event-based SEPT가 0.5ms interval에서 pupil ellipse offset을 갱신한다. similarity threshold가 낮으면 frame-based relocalization을 수행하며, 최종 pupil center는 polynomial regression으로 PoG에 매핑된다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 Jetson Orin Nano에서 2KHz real-time tracking을 달성하는 event-frame hybrid system 제안.에 초점을 맞춘다.
...
- **Primary axis:** On-device XR gaze tracking with hybrid event-frame cameras
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `frame-based initialization/relocalization, Sparse Event-Patch Transformer (SEPT), gradient-based approach-adaptation, polynomial gaze regression을 조합한다.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| Sparse Event-Patch Representation | pupil ellipse 주변 8개 patch만 event image로 추출 | input size 감소 | Section 3.4.1, Fig. 6 |
| SEPT | patch-shared CNN + transformer encoder로 pupil offset 회귀 | event-based tracking core | Section 3.4.2, Fig. 5 |
| Approach-adaptation | ellipse normal과 frame gradient similarity로 relocalization trigger | accuracy-efficiency trade-off | Section 3.5, Fig. 7 |
...
frame-based MobileNetV3-small detector가 eye/pupil region을 초기화하고, event-based SEPT가 0.5ms interval에서 pupil ellipse offset을 갱신한다. similarity threshold가 낮으면 frame-based relocalization을 수행하며, 최종 pupil center는 polynomial regression으로 PoG에 매핑된다.
...
- frame-based initialization: eye/pupil region detection
- event patch accumulation: THec=50, THtc=5ms, 0.5ms check
- SEPT offset estimation
- gradient similarity S로 relocalization 판단
- polynomial gaze regression
...
이 논문은 XR용 on-device gaze tracking은 kHz-level frequency와 low latency를 요구하지만 frame-only system은 slow/expensive이고 event-only system은 semantic robustness가 부족한 문제.를 대상으로 frame-based initialization/relocalization, Sparse Event-Patch Transformer (SEPT), gradient-based approach-adaptation, polynomial gaze regression을 조합한다.를 제안한다.
...
- on-device implementation 관점이 강하고 module runtime을 분해함
- event sparsity를 unstructured sparse CNN이 아니라 dense patch representation으로 변환해 CUDA 친화적으로 처리
- frame relocalization 빈도를 낮추는 adaptation 설계가 실용적

## 7.5 Weak or Unclear Parts

- DAVIS346 같은 hybrid event-frame camera에 의존
- frame-based module과 calibration이 필요하여 완전 event-only가 아님
- Jetson Orin Nano는 XR headset 내 실제 thermal/power budget과 차이가 있을 수 있음
...
> EX-Gaze: High-frequency and Low-latency Gaze Tracking with Hybrid Event-frame Cameras for On-Device Extended Reality는 On-device XR gaze tracking with hybrid event-frame cameras 관점에서 frame-based initialization/relocalization, Sparse Event-Patch Transformer (SEPT), gradient-based approach-adaptation, polynomial gaze regression을 조합한다.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** frame-based initialization/relocalization, Sparse Event-Patch Transformer (SEPT), gradient-based approach-adaptation, polynomial gaze regression을 조합한다.
...
- **Evidence:** Cons list: DAVIS346 같은 hybrid event-frame camera에 의존, frame-based module과 calibration이 필요하여 완전 event-only가 아님, Jetson Orin Nano는 XR headset 내 실제 thermal/power budget과 차이가 있을 수 있음
