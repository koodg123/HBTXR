# Paper Summary: Overcoming Data Scarcity for Event-Based Pupil Tracking with Synthetic and Unlabeled Data

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `Overcoming Data Scarcity for Event-Based Pupil Tracking with Synthetic and Unlabeled Data.pdf` |
...
| Item | Description |
|---|---|
| **Title** | Overcoming Data Scarcity for Event-Based Pupil Tracking with Synthetic and Unlabeled Data |
| **Authors** | Aaron Tognoli, Andrea Simpsi, Andrea Aspesi, Marco Cannici, Luca Merigo, Matteo Matteucci, Simone Mentasti |
| **Venue** | PACM HCI |
| **Year** | 2026 |
| **Research Field** | Synthetic/unlabeled data for event-based pupil tracking |
...
- **Target Problem:** wearable event-based pupil tracking은 labeled real event recordings가 부족하고, hardware/device 변화에 따라 annotation cost가 큰 문제.
- **Research Objective:** synthetic events와 unlabeled real events, limited labeled real data를 결합한 training framework 제안.
...
- **Core Idea:** U2Eyes synthetic eye rendering + v2e event simulation으로 synthetic supervision을 만들고, unsupervised adaptation/pretraining과 limited real fine-tuning을 결합한다.
- **Proposed Approach:** synthetic event streams를 생성해 supervised learning을 수행하고, unlabeled real recordings로 representation을 adaptation/pretraining한다. 이후 제한된 real labels로 fine-tune하여 subject/device gap을 줄인다.
...
1. event-based pupil tracking data scarcity 해결을 위한 synthetic+unlabeled training framework
2. synthetic events가 synthetic images보다 작은 sim-to-real gap을 보임
3. INI-30/3ET+에서 limited label setting 성능 개선
4. label-efficient deployment 관점 제시

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| INI-30 | Pupil tracking | MPD | 2.48 (Ours) / 1.63 (Ours*) | Retina 3.24 | Table 3 |
| 3ET+ | Pupil tracking | MPD | 3.24 (Ours) / 1.36 (Ours*) | temporal challenge methods separately compared | Table 3 |
| EV-Eye sim-to-real | Event vs image simulation | MAE | event sim-only 6.55 vs real-only 3.10; image sim-only 55.06 vs real-only 1.79 | Table 1 sim-to-real gap | Table 1 |

## 1.5 Brief Overview

> 이 논문은 Synthetic/unlabeled data for event-based pupil tracking 분야에서 wearable event-based pupil tracking은 labeled real event recordings가 부족하고, hardware/device 변화에 따라 annotation cost가 큰 문제.를 해결하기 위해 U2Eyes synthetic eye rendering + v2e event simulation으로 synthetic supervision을 만들고, unsupervised adaptation/pretraining과 limited real fine-tuning을 결합한다.라는 설계 방향을 제안한다. 핵심 방법은 synthetic event streams를 생성해 supervised learning을 수행하고, unlabeled real recordings로 representation을 adaptation/pretraining한다. 이후 제한된 real labels로 fine-tune하여 subject/device gap을 줄인다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 synthetic events와 unlabeled real events, limited labeled real data를 결합한 training framework 제안.에 초점을 맞춘다.
...
- **Primary axis:** Synthetic/unlabeled data for event-based pupil tracking
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `U2Eyes synthetic eye rendering + v2e event simulation으로 synthetic supervision을 만들고, unsupervised adaptation/pretraining과 limited real fine-tuning을 결합한다.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| U2Eyes + v2e | synthetic eye image와 event camera simulator를 결합 | synthetic event generation | Method |
| Unlabeled real adaptation | real event distribution을 supervision 없이 반영 | domain gap reduction | Method |
| Limited real fine-tuning | 소량 real label로 model을 최종 보정 | deployment practicality | Method |
...
synthetic event streams를 생성해 supervised learning을 수행하고, unlabeled real recordings로 representation을 adaptation/pretraining한다. 이후 제한된 real labels로 fine-tune하여 subject/device gap을 줄인다.
...
- U2Eyes synthetic eye generation
- v2e로 synthetic events 생성
- unlabeled real event adaptation/pretraining
- synthetic supervised training
- limited labeled real fine-tuning
- INI-30/3ET+ evaluation
...
이 논문은 wearable event-based pupil tracking은 labeled real event recordings가 부족하고, hardware/device 변화에 따라 annotation cost가 큰 문제.를 대상으로 U2Eyes synthetic eye rendering + v2e event simulation으로 synthetic supervision을 만들고, unsupervised adaptation/pretraining과 limited real fine-tuning을 결합한다.를 제안한다.
...
- data scarcity라는 실제 deployment bottleneck을 정면으로 다룸
- synthetic event가 image synthetic보다 domain gap이 작을 수 있음을 실험으로 제시
- device bring-up/새 하드웨어 세대에 유용한 training recipe

## 7.5 Weak or Unclear Parts

- synthetic generator/v2e fidelity에 결과가 크게 의존
- real-world long-term/lighting/user diversity에서 추가 검증 필요
- fully supervised upper bound와의 gap 및 temporal online metric은 논문 조건별로 주의 필요
...
> Overcoming Data Scarcity for Event-Based Pupil Tracking with Synthetic and Unlabeled Data는 Synthetic/unlabeled data for event-based pupil tracking 관점에서 U2Eyes synthetic eye rendering + v2e event simulation으로 synthetic supervision을 만들고, unsupervised adaptation/pretraining과 limited real fine-tuning을 결합한다.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** U2Eyes synthetic eye rendering + v2e event simulation으로 synthetic supervision을 만들고, unsupervised adaptation/pretraining과 limited real fine-tuning을 결합한다.
...
- **Evidence:** Cons list: synthetic generator/v2e fidelity에 결과가 크게 의존, real-world long-term/lighting/user diversity에서 추가 검증 필요, fully supervised upper bound와의 gap 및 temporal online metric은 논문 조건별로 주의 필요
