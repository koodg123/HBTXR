# Paper Summary: Swift-Eye: Towards Anti-blink Pupil Tracking for Precise and Robust High-Frequency Near-Eye Movement Analysis with Event Cameras

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `Swift-Eye_Towards_Anti-blink_Pupil_Tracking_for_Precise_and_Robust_High-Frequency_Near-Eye_Movement_Analysis_with_Event_Cameras.pdf` |
...
| **Authors** | Tongyu Zhang, Yiran Shen, Guangrong Zhao, Lin Wang, Xiaoming Chen, Lu Bai, Yuanfeng Zhou |
| **Venue** | IEEE TVCG |
| **Year** | 2024 |
| **Research Field** | Offline high-frequency anti-blink pupil tracking |
| **Paper Link / DOI** | 논문 PDF 기준 분석; DOI는 파일별로 상이하거나 논문에 명시되지 않음 |
| **Code / Project Link** | https://github.com/ztysdu/Swift-Eye |
...
- **Target Problem:** high-frequency pupil tracking에서 involuntary blink/partial occlusion이 pupil region estimation을 크게 저하시켜 accurate trace analysis가 어려운 문제.
- **Research Objective:** event-based frame interpolation과 anti-blink pupil estimation을 결합해 robust high-frequency near-eye movement analysis를 지원.
...
- **Core Idea:** low-rate frames + event streams를 Timelens 기반 5,000fps high-speed video로 변환하고, temporal feature fusion + adaptive strategy + synthetic occlusion training으로 blink robustness를 높인다.
- **Proposed Approach:** Timelens를 near-eye data에 fine-tune하여 high-speed video를 생성한다. Swin/FPN 기반 multi-scale spatial feature extractor와 temporal feature fusion이 pupil ellipse를 예측하고, occlusion ratio에 따라 strategy를 바꾼다. LAMA로 synthetic eyelid occlusion images를 생성해 학습한다.
...
1. event-based frame interpolation으로 high-speed near-eye video 생성
2. temporal feature fusion 기반 anti-blink pupil estimation 제안
3. LAMA 기반 synthetic eyelid occlusion augmentation으로 training data 확장
4. EV-Eye에서 occlusion ratio >80%일 때 SOTA 대비 IoU/F1 개선

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| EV-Eye extra occlusion-labeled subset | Pupil estimation under >80% occlusion | IoU / F1-score improvement | 20% / 12.5% improvement | over second-best EllSeg; IoU 0.58->0.702, F1 0.726->0.817 | Abstract/Section 4.4 |
| EV-Eye | High-frequency trace | Video frequency | 5,000fps | 25fps original frame interpolated with events | Section 4.5/Fig. 15 |

## 1.5 Brief Overview

> 이 논문은 Offline high-frequency anti-blink pupil tracking 분야에서 high-frequency pupil tracking에서 involuntary blink/partial occlusion이 pupil region estimation을 크게 저하시켜 accurate trace analysis가 어려운 문제.를 해결하기 위해 low-rate frames + event streams를 Timelens 기반 5,000fps high-speed video로 변환하고, temporal feature fusion + adaptive strategy + synthetic occlusion training으로 blink robustness를 높인다.라는 설계 방향을 제안한다. 핵심 방법은 Timelens를 near-eye data에 fine-tune하여 high-speed video를 생성한다. Swin/FPN 기반 multi-scale spatial feature extractor와 temporal feature fusion이 pupil ellipse를 예측하고, occlusion ratio에 따라 strategy를 바꾼다. LAMA로 synthetic eyelid occlusion images를 생성해 학습한다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 event-based frame interpolation과 anti-blink pupil estimation을 결합해 robust high-frequency near-eye movement analysis를 지원.에 초점을 맞춘다.
...
- **Primary axis:** Offline high-frequency anti-blink pupil tracking
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `low-rate frames + event streams를 Timelens 기반 5,000fps high-speed video로 변환하고, temporal feature fusion + adaptive strategy + synthetic occlusion training으로 blink robustness를 높인다.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| Event-based frame interpolation | 25fps frame + events -> 5,000fps high-speed video | offline high-frequency trace 생성 | Section 3/Fig. 2 |
| Temporal feature fusion | 이전 frame template feature를 현재 pupil estimation에 결합 | partial occlusion robustness | Section 3.3/Fig. 4 |
| Occlusion-ratio adaptation | occlusion 정도에 따라 detection strategy 전환 | anti-blink handling | Section 3.4 |
...
Timelens를 near-eye data에 fine-tune하여 high-speed video를 생성한다. Swin/FPN 기반 multi-scale spatial feature extractor와 temporal feature fusion이 pupil ellipse를 예측하고, occlusion ratio에 따라 strategy를 바꾼다. LAMA로 synthetic eyelid occlusion images를 생성해 학습한다.
...
- low-rate DAVIS frames + event stream 입력
- event-based interpolation to 5,000fps
- multi-scale spatial feature extraction
- adaptive template temporal fusion
- occlusion-ratio based approach adaptation
- pupil ellipse trace output
...
이 논문은 high-frequency pupil tracking에서 involuntary blink/partial occlusion이 pupil region estimation을 크게 저하시켜 accurate trace analysis가 어려운 문제.를 대상으로 low-rate frames + event streams를 Timelens 기반 5,000fps high-speed video로 변환하고, temporal feature fusion + adaptive strategy + synthetic occlusion training으로 blink robustness를 높인다.를 제안한다.
...
- blink/occlusion이라는 실제 XR failure mode를 정면으로 해결
- synthetic occlusion data generation과 temporal fusion이 실험적으로 뒷받침됨
- high-frequency trace analysis에 적합한 고품질 visualization 제공

## 7.5 Weak or Unclear Parts

- 저자도 latency를 고려하지 않았다고 명시; foveated rendering 같은 low-latency online application에는 바로 적용 어려움
- Timelens/interpolation pipeline이 compute-heavy일 수 있음
- offline analysis용 성격이 강해 on-device deployment 검증 필요
...
> Swift-Eye: Towards Anti-blink Pupil Tracking for Precise and Robust High-Frequency Near-Eye Movement Analysis with Event Cameras는 Offline high-frequency anti-blink pupil tracking 관점에서 low-rate frames + event streams를 Timelens 기반 5,000fps high-speed video로 변환하고, temporal feature fusion + adaptive strategy + synthetic occlusion training으로 blink robustness를 높인다.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** low-rate frames + event streams를 Timelens 기반 5,000fps high-speed video로 변환하고, temporal feature fusion + adaptive strategy + synthetic occlusion training으로 blink robustness를 높인다.
...
- **Evidence:** Cons list: 저자도 latency를 고려하지 않았다고 명시; foveated rendering 같은 low-latency online application에는 바로 적용 어려움, Timelens/interpolation pipeline이 compute-heavy일 수 있음, offline analysis용 성격이 강해 on-device deployment 검증 필요
