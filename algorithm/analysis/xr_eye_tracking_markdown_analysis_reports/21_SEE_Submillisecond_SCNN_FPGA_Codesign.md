# Paper Summary: Co-designing a Sub-millisecond Latency Event-based Eye Tracking System with Submanifold Sparse CNN

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `Zhang_Co-designing_a_Sub-millisecond_Latency_Event-based_Eye_Tracking_System_with_Submanifold_CVPRW_2024_paper.pdf` |
...
| **Venue** | CVPRW |
| **Year** | 2024 |
| **Research Field** | Software-hardware co-design / FPGA event eye tracking |
| **Paper Link / DOI** | 논문 PDF 기준 분석; DOI는 파일별로 상이하거나 논문에 명시되지 않음 |
| **Code / Project Link** | https://github.com/CASR-HKU/ESDA/tree/eye_tracking |
...
- **Target Problem:** event camera의 sparsity를 DNN과 embedded hardware에서 충분히 활용하지 못해 low-latency, low-power, high-precision을 동시에 만족하기 어려운 문제.
- **Research Objective:** submanifold sparse CNN backbone과 FPGA sparse dataflow accelerator를 co-design하여 sub-millisecond event-based eye tracking 구현.
...
- **Core Idea:** SCNN backbone은 FPGA programmable logic에서 int8 sparse dataflow로 실행하고, GRU+FC는 Arm Cortex-A53 NEON에서 float로 실행한다. hardware simulator AGNA 기반 model search로 accuracy-latency Pareto를 찾는다.
- **Proposed Approach:** event clips를 sparse voxel representation으로 변환하고 SCNN backbone이 feature embedding을 추출한다. GRU는 temporal fusion을 수행하고 FC가 normalized eye center를 회귀한다. FPGA accelerator는 conv/pooling layers를 spatially mapped sparse dataflow로 pipeline 처리하며, weights는 BRAM에 statically 저장한다.
...
1. SEE event-based eye tracking system 제안
2. submanifold sparse convolution으로 activation dilation 방지 및 sparsity 보존
3. FPGA sparse dataflow accelerator와 Arm CPU SIMD를 결합한 heterogeneous implementation
4. 0.6–0.94ms latency와 ~99% p10 accuracy 달성

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| Event-based Eye-Tracking AIS2024 / 3ET+ | SEE-D pupil tracking | p5 / p10 / Dist / Params / Latency | 81.37 / 99.53 / 3.71 / 178K / 0.70ms | MobileNetV2 87.36 / 99.53 / 3.15 / 797K / 1.45ms | Table 5 / Fig. 5 |
| AIS2024 | SEE-C speed point | Latency / p10 | 0.60ms / 98.39% | ~2.5× speedup over MobileNetV2 with ~1% p10 drop | Section 4.3 |
| Hardware implementation | Embedded GPU comparison | Speedup | 11.2× to 72.6× | Jetson standard/submanifold implementations | Fig. 6 / Conclusion |
| Hardware platform | Implementation | Tool/Board | Vitis HLS 2020.2, Vivado 2020.2, ZCU102 MPSoC | FPGA SCNN + CPU GRU/FC | Section 4.1 |

## 1.5 Brief Overview

> 이 논문은 Software-hardware co-design / FPGA event eye tracking 분야에서 event camera의 sparsity를 DNN과 embedded hardware에서 충분히 활용하지 못해 low-latency, low-power, high-precision을 동시에 만족하기 어려운 문제.를 해결하기 위해 SCNN backbone은 FPGA programmable logic에서 int8 sparse dataflow로 실행하고, GRU+FC는 Arm Cortex-A53 NEON에서 float로 실행한다. hardware simulator AGNA 기반 model search로 accuracy-latency Pareto를 찾는다.라는 설계 방향을 제안한다. 핵심 방법은 event clips를 sparse voxel representation으로 변환하고 SCNN backbone이 feature embedding을 추출한다. GRU는 temporal fusion을 수행하고 FC가 normalized eye center를 회귀한다. FPGA accelerator는 conv/pooling layers를 spatially mapped sparse dataflow로 pipeline 처리하며, weights는 BRAM에 statically 저장한다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 submanifold sparse CNN backbone과 FPGA sparse dataflow accelerator를 co-design하여 sub-millisecond event-based eye tracking 구현.에 초점을 맞춘다.
...
- **Primary axis:** Software-hardware co-design / FPGA event eye tracking
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `SCNN backbone은 FPGA programmable logic에서 int8 sparse dataflow로 실행하고, GRU+FC는 Arm Cortex-A53 NEON에서 float로 실행한다. hardware simulator AGNA 기반 model search로 accuracy-latency Pareto를 찾는다.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| Submanifold Sparse Convolution | input non-zero 위치에서만 output non-zero를 생성해 sparsity 보존 | compute reduction | Section 3.2.2/Fig. 2 |
| Token-feature stream | token `[.x,.y,.end]`와 feature stream으로 sparse activation을 pipeline 전송 | FPGA sparse dataflow interface | Section 3.3.2/Fig. 3 |
| HAWQv3 int8 quantization | SCNN backbone input/weight int8 quantization | FPGA integer arithmetic | Section 3.2.3/Eq. 1 |
...
event clips를 sparse voxel representation으로 변환하고 SCNN backbone이 feature embedding을 추출한다. GRU는 temporal fusion을 수행하고 FC가 normalized eye center를 회귀한다. FPGA accelerator는 conv/pooling layers를 spatially mapped sparse dataflow로 pipeline 처리하며, weights는 BRAM에 statically 저장한다.
...
- event stream clip partition
- sparse voxel representation
- SCNN backbone on FPGA int8 accelerator
- GRU + FC on Arm Cortex-A53 NEON
- normalized coordinate -> pixel coordinate
- AGNA simulator 기반 network search
...
이 논문은 event camera의 sparsity를 DNN과 embedded hardware에서 충분히 활용하지 못해 low-latency, low-power, high-precision을 동시에 만족하기 어려운 문제.를 대상으로 SCNN backbone은 FPGA programmable logic에서 int8 sparse dataflow로 실행하고, GRU+FC는 Arm Cortex-A53 NEON에서 float로 실행한다. hardware simulator AGNA 기반 model search로 accuracy-latency Pareto를 찾는다.를 제안한다.
...
- algorithm/hardware co-design이 명확하고 실제 FPGA SoC 구현 결과가 있음
- submanifold sparse convolution과 sparse dataflow가 event sparsity에 잘 맞음
- latency/accuracy Pareto를 제시해 design space exploration에 유용

## 7.5 Weak or Unclear Parts

- GRU+FC가 CPU에서 float로 실행되어 완전 FPGA end-to-end는 아님
- weights on-chip BRAM 저장은 model size scalability 제약
- GRU/attention 같은 recurrent nonlinear module의 FPGA 통합은 future work로 남음
...
> Co-designing a Sub-millisecond Latency Event-based Eye Tracking System with Submanifold Sparse CNN는 Software-hardware co-design / FPGA event eye tracking 관점에서 SCNN backbone은 FPGA programmable logic에서 int8 sparse dataflow로 실행하고, GRU+FC는 Arm Cortex-A53 NEON에서 float로 실행한다. hardware simulator AGNA 기반 model search로 accuracy-latency Pareto를 찾는다.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** SCNN backbone은 FPGA programmable logic에서 int8 sparse dataflow로 실행하고, GRU+FC는 Arm Cortex-A53 NEON에서 float로 실행한다. hardware simulator AGNA 기반 model search로 accuracy-latency Pareto를 찾는다.
...
- **Evidence:** Cons list: GRU+FC가 CPU에서 float로 실행되어 완전 FPGA end-to-end는 아님, weights on-chip BRAM 저장은 model size scalability 제약, GRU/attention 같은 recurrent nonlinear module의 FPGA 통합은 future work로 남음
