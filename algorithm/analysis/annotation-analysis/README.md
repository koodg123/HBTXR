...
> **저장 위치 안내**: 원래 요청하신 출력 경로는 `E:\DATASET\codes\annotation-analysis` 였으나, 현재 세션의 폴더 마운트 서브시스템이 기존 WSL UNC 마운트 충돌로 새 드라이브(E:\DATASET\codes)를 마운트하지 못합니다. 그래서 **연결되어 있는 프로젝트 폴더**(`.../HBTXR/annotation-analysis/`)에 저장했습니다. 이 폴더 전체를 `E:\DATASET\codes\annotation-analysis`로 복사하시면 됩니다.
