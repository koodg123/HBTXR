...
class MacsEstimationHook:
    def __init__(self, num_conv_layers):
        # Initialize the hook with the number of convolution layers
        self.num_conv_layers = num_conv_layers
        self.layer_id = 0
        self._params_per_layer = torch.zeros(num_conv_layers)  # Number of parameters per layer
    ...
    def __call__(self, module, input, output):
        ...
        self._macs_per_layer_with_sparsity[self.layer_id] = (
            macs * self.nonzeros[self.layer_id] / self.totals[self.layer_id]
        )
...
def regression_loss(pred, center, openness):
    ...
    center_loss = F.smooth_l1_loss(pred, center, beta=0.11, reduction="none").sum(
        1
    )  # (batch, frames)
...
def tracking_loss(pred, center, openness, gamma=2):
    ...
    x, y = center.moveaxis(1, 0)
    x_ind = (x * width).long().clamp(0, width - 1)
    y_ind = (y * height).long().clamp(0, height - 1)
    x_mod = (x * width) % 1
    y_mod = (y * height) % 1

    center_mod = torch.stack([x_mod, y_mod], dim=1)
    ...
    pupil_mask = torch.zeros_like(pred_pupil).bool()
    ...
    center_loss = F.smooth_l1_loss(
        pred_center_mod, center_mod[..., None, None], beta=0.11, reduction="none"
    ).sum(1)
...
class Losses:
    def __init__(self, detector_head, reg_factor, model):
        if detector_head:
            ...
            self.prediction_loss = regression_loss
...
def process_detector_prediction(pred):
    ...
    if len(pred.shape) == 3:
        ...
        pred_x_mod = pred_x_mod[
            batch_range, frames_range, pupil_ind_y.flatten(), pupil_ind_x.flatten()
        ]
        pred_y_mod = pred_y_mod[
            batch_range, frames_range, pupil_ind_y.flatten(), pupil_ind_x.flatten()
        ]
...
def p_acc(pred, center, openness, height, width, tolerance=10, detector_head=True):

    pred = pred.detach().clone()
    center = center.detach().clone()

    if detector_head:
        # Post-process predictions from the detector head
    ...
    overall_accuracy = (distances < tolerance).sum() / distances.numel()

    noblink_accuracy = (
        distances_noblinks < tolerance
    ).sum() / distances_noblinks.numel()

    mean_distance = distances.mean()

    return overall_accuracy, noblink_accuracy, mean_distance
