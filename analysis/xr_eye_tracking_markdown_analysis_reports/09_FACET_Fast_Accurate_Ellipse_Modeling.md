# Paper Summary: FACET: Fast and Accurate Event-Based Eye Tracking Using Ellipse Modeling for Extended Reality

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `FACET_Fast_and_Accurate_Event-Based_Eye_Tracking_Using_Ellipse_Modeling_for_Extended_Reality.pdf` |
...
| **Venue** | arXiv / IEEE-style preprint |
| **Year** | 2024 |
| **Research Field** | Event-based ellipse detector for XR eye tracking |
| **Paper Link / DOI** | 논문 PDF 기준 분석; DOI는 파일별로 상이하거나 논문에 명시되지 않음 |
| **Code / Project Link** | https://github.com/DeanJY/FACET |
...
- **Target Problem:** segmentation 후 ellipse fitting을 수행하는 기존 event-frame tracker는 비용이 크고, event camera가 pupil boundary를 잘 강조한다는 특성을 충분히 활용하지 못하는 문제.
- **Research Objective:** event data에서 pupil ellipse parameters를 end-to-end로 직접 예측하는 lightweight detector 제안.
...
- **Core Idea:** fixed-count event binning + fast causal event volume + MobileNetV3/FPN-DSC + CenterNet-style heads + trigonometric loss를 결합해 ellipse `(x,y,a,b,θ)`를 직접 출력한다.
- **Proposed Approach:** 5000 events fixed-count bin을 fast causal event volume으로 2-channel representation으로 만든다. MobileNetV3 backbone과 FPN with depthwise separable convolution으로 features를 만들고, four heads가 center heatmap, offset, axis size, rotation vector를 출력한다. Loss는 heatmap/offset/size/Gaussian IoU/trigonometric loss의 weighted sum이다.
...
1. event-based end-to-end ellipse pupil detector FACET 제안
2. EV-Eye label을 semi-supervised로 1.5M samples까지 확장하고 mask를 ellipse label로 변환
3. angle discontinuity를 해결하는 trigonometric loss 제안
4. fast causal event volume으로 event accumulation time 감소

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| Enhanced EV-Eye | Pupil ellipse/center detection | P10 / P5 / P1 / PE | 100 / 99.98 / 99.59 / 0.2030 px | best among listed methods | Table II |
| Enhanced EV-Eye | Efficiency | Params / GFLOPs / inference time | 3.92M / 3.44 / 0.5302ms | EV-Eye: 17.27M / 40.11 / 0.9438ms | Table II |
| Ablation | Event representation | PE / EPT | fast causal event volume PE 0.2030, EPT 1.6493ms | causal/event volume worse in PE/EPT | Table III |

## 1.5 Brief Overview

> 이 논문은 Event-based ellipse detector for XR eye tracking 분야에서 segmentation 후 ellipse fitting을 수행하는 기존 event-frame tracker는 비용이 크고, event camera가 pupil boundary를 잘 강조한다는 특성을 충분히 활용하지 못하는 문제.를 해결하기 위해 fixed-count event binning + fast causal event volume + MobileNetV3/FPN-DSC + CenterNet-style heads + trigonometric loss를 결합해 ellipse `(x,y,a,b,θ)`를 직접 출력한다.라는 설계 방향을 제안한다. 핵심 방법은 5000 events fixed-count bin을 fast causal event volume으로 2-channel representation으로 만든다. MobileNetV3 backbone과 FPN with depthwise separable convolution으로 features를 만들고, four heads가 center heatmap, offset, axis size, rotation vector를 출력한다. Loss는 heatmap/offset/size/Gaussian IoU/trigonometric loss의 weighted sum이다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 event data에서 pupil ellipse parameters를 end-to-end로 직접 예측하는 lightweight detector 제안.에 초점을 맞춘다.
...
- **Primary axis:** Event-based ellipse detector for XR eye tracking
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `fixed-count event binning + fast causal event volume + MobileNetV3/FPN-DSC + CenterNet-style heads + trigonometric loss를 결합해 ellipse `(x,y,a,b,θ)`를 직접 출력한다.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| Fast Causal Event Volume | causal event volume에 saturation limit를 두어 accumulation을 빠르게 함 | real-time event representation | Section IV-A, Algorithm 1/Fig. 3 |
| Trigonometric Loss | θ를 `(sin(2θ), cos(2θ))`로 표현해 0/180도 discontinuity 해결 | angle regression 안정화 | Section IV-C, Fig. 4/Table I |
| Ellipse Heads | heatmap/offset/size/rotation heads로 ellipse parameters 예측 | end-to-end ellipse detection | Section IV-B, Fig. 2 |
...
- fixed-count event binning: 5000 events
- fast causal event volume 생성
- MobileNetV3 + FPN with DSConv
- heatmap/offset/size/rotation heads
- ellipse output `(x,y,a,b,θ)`
- 후속 high-frequency tracker의 detector로 사용 가능
...
이 논문은 segmentation 후 ellipse fitting을 수행하는 기존 event-frame tracker는 비용이 크고, event camera가 pupil boundary를 잘 강조한다는 특성을 충분히 활용하지 못하는 문제.를 대상으로 fixed-count event binning + fast causal event volume + MobileNetV3/FPN-DSC + CenterNet-style heads + trigonometric loss를 결합해 ellipse `(x,y,a,b,θ)`를 직접 출력한다.를 제안한다.
...
- ellipse를 직접 출력하여 segmentation+fitting보다 pipeline이 단순하고 빠름
- angle discontinuity 문제를 수학적으로 정확히 처리
- accuracy와 model efficiency 모두 강한 결과

## 7.5 Weak or Unclear Parts

- enhanced label이 U-Net pseudo-mask/ellipse fitting에 의존하므로 label noise 영향 가능
- 64×64 resolution metric은 다른 해상도/데이터셋과 직접 비교 주의
- end-to-end detection은 high-frequency continuous tracking 전체 latency와는 별도
...
> FACET: Fast and Accurate Event-Based Eye Tracking Using Ellipse Modeling for Extended Reality는 Event-based ellipse detector for XR eye tracking 관점에서 fixed-count event binning + fast causal event volume + MobileNetV3/FPN-DSC + CenterNet-style heads + trigonometric loss를 결합해 ellipse `(x,y,a,b,θ)`를 직접 출력한다.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** fixed-count event binning + fast causal event volume + MobileNetV3/FPN-DSC + CenterNet-style heads + trigonometric loss를 결합해 ellipse `(x,y,a,b,θ)`를 직접 출력한다.
...
- **Evidence:** Cons list: enhanced label이 U-Net pseudo-mask/ellipse fitting에 의존하므로 label noise 영향 가능, 64×64 resolution metric은 다른 해상도/데이터셋과 직접 비교 주의, end-to-end detection은 high-frequency continuous tracking 전체 latency와는 별도
