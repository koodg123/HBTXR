# Paper Summary: Retina: Low-Power Eye Tracking with Event Camera and Spiking Hardware

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `retina.pdf` |
...
| **Venue** | event-based / neuromorphic systems paper |
| **Year** | 2024 |
| **Research Field** | SNN neuromorphic eye tracking |
...
- **Target Problem:** ANN 기반 event eye tracking은 edge wearable power budget에 부담이고, event camera의 asynchronous sparse nature와 neuromorphic hardware를 충분히 결합하지 못하는 문제.
- **Research Objective:** SNN regression model Retina와 Ini-30 dataset, Speck neuromorphic processor 기반 low-power eye tracking을 제안.
...
- **Core Idea:** Integrate-and-Fire neurons 기반 directly trained SNN을 event input에 적용하고, Speck chip에서 mW-level power로 pupil regression을 수행한다.
- **Proposed Approach:** DVS event를 SNN input으로 구성하고 Integrate-and-Fire neuron 기반 network를 학습한다. 모델은 pupil coordinate regression을 수행하며 Speck processor에서 latency/power/energy를 측정한다.
...
1. Ini-30 event-based eye-tracking dataset 제안
2. 64K parameter Retina SNN model 제안
3. Speck neuromorphic processor에서 low-power inference 측정
4. 3ET 대비 parameter/MAC reduction과 낮은 power 제시

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| Ini-30 | Pupil tracking | Pixel error | 3.24 px | 3ET 4.48 px | Table 10 / Abstract |
| Model complexity | Efficiency | Params / MACs | 64K params / 3.03M MACs | 3ET 418K params / 107M MACs | Tables/Abstract |
| Speck processor | Hardware efficiency | Power / latency / energy | 2.89–4.8 mW / 5.57–8.01ms / 16.10–38.40mJ | device measurements | Hardware evaluation |

## 1.5 Brief Overview

> 이 논문은 SNN neuromorphic eye tracking 분야에서 ANN 기반 event eye tracking은 edge wearable power budget에 부담이고, event camera의 asynchronous sparse nature와 neuromorphic hardware를 충분히 결합하지 못하는 문제.를 해결하기 위해 Integrate-and-Fire neurons 기반 directly trained SNN을 event input에 적용하고, Speck chip에서 mW-level power로 pupil regression을 수행한다.라는 설계 방향을 제안한다. 핵심 방법은 DVS event를 SNN input으로 구성하고 Integrate-and-Fire neuron 기반 network를 학습한다. 모델은 pupil coordinate regression을 수행하며 Speck processor에서 latency/power/energy를 측정한다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 SNN regression model Retina와 Ini-30 dataset, Speck neuromorphic processor 기반 low-power eye tracking을 제안.에 초점을 맞춘다.
...
- **Primary axis:** SNN neuromorphic eye tracking
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `Integrate-and-Fire neurons 기반 directly trained SNN을 event input에 적용하고, Speck chip에서 mW-level power로 pupil regression을 수행한다.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| SNN regression | spike-based network로 pupil center를 직접 회귀 | low-power event processing | Method |
| Ini-30 dataset | 30 volunteers, glass-mounted DVS cameras | neuromorphic benchmark | Dataset |
| Speck processor | low-power neuromorphic edge processor | SNN deployment | Hardware |
...
DVS event를 SNN input으로 구성하고 Integrate-and-Fire neuron 기반 network를 학습한다. 모델은 pupil coordinate regression을 수행하며 Speck processor에서 latency/power/energy를 측정한다.
...
- Ini-30 dataset event stream 수집
- event preprocessing
- Retina SNN training
- Speck deployment
- pixel error / power / latency / energy 평가
...
이 논문은 ANN 기반 event eye tracking은 edge wearable power budget에 부담이고, event camera의 asynchronous sparse nature와 neuromorphic hardware를 충분히 결합하지 못하는 문제.를 대상으로 Integrate-and-Fire neurons 기반 directly trained SNN을 event input에 적용하고, Speck chip에서 mW-level power로 pupil regression을 수행한다.를 제안한다.
...
- mW-level neuromorphic deployment이라는 차별성이 강함
- event camera와 SNN/hardware modality가 잘 정합
- model complexity가 작고 edge power 측면에서 강점

## 7.5 Weak or Unclear Parts

- SNN은 Speck 같은 specialized processor 의존성이 큼
- accuracy는 FACET/BRAT/EV-Eye 계열 최신 방법 대비 낮음
- continuous neuron reset/online state 관리가 실제 tracking에서 한계로 언급됨
...
> Retina: Low-Power Eye Tracking with Event Camera and Spiking Hardware는 SNN neuromorphic eye tracking 관점에서 Integrate-and-Fire neurons 기반 directly trained SNN을 event input에 적용하고, Speck chip에서 mW-level power로 pupil regression을 수행한다.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** Integrate-and-Fire neurons 기반 directly trained SNN을 event input에 적용하고, Speck chip에서 mW-level power로 pupil regression을 수행한다.
...
- **Evidence:** Cons list: SNN은 Speck 같은 specialized processor 의존성이 큼, accuracy는 FACET/BRAT/EV-Eye 계열 최신 방법 대비 낮음, continuous neuron reset/online state 관리가 실제 tracking에서 한계로 언급됨
