# Paper Summary: MambaPupil: Bidirectional Selective Recurrent Model for Event-based Eye Tracking

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `MambaPupil Bidirectional Selective Recurrent model for Event-based Eye tracking.pdf` |
...
| **Authors** | USTCEventGroup / Zheng-jun Zha, Wei Zhai, Han Han, Bohao Liao, Yuliang Wu |
| **Venue** | AIS 2024 Challenge / workshop report |
| **Year** | 2024 |
| **Research Field** | Recurrent state-space event eye tracking |
...
- **Target Problem:** event stream의 static information loss와 irregular temporal pattern 때문에 단순 CNN/GRU가 long-term eye movement context를 충분히 모델링하지 못하는 문제.
- **Research Objective:** Bi-GRU와 LTV-SSM/Mamba block을 결합해 short/long temporal dependency를 함께 모델링.
...
- **Core Idea:** Bina-rep event representation + CNN spatial extractor + bidirectional GRU + Linear Time-Varying State-Space Model (LTV-SSM)으로 pupil center를 예측한다.
- **Proposed Approach:** event stream을 binary map representation으로 구성하고 CNN blocks가 spatial features를 추출한다. adaptive global pooling 및 dropout 후 Bi-GRU와 LTV-SSM이 temporal context를 모델링하고 FC/regressor가 pupil center를 출력한다.
...
1. MambaPupil dual recurrent module 제안
2. Bina-rep event representation으로 compact temporal encoding
3. Bi-GRU와 LTV-SSM 결합으로 temporal modeling 강화
4. AIS 2024 challenge에서 상위 성능 달성

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| 3ET+ validation | Pupil center tracking | p5 / p10 / p15 / pixel error | 0.937 / 0.984 / 0.990 / 2.03 | train stride 5 setting | Table 1 |
| Model complexity | Efficiency | Params / FLOPs | 8.59M / 2.61T | CNN-GRU 37.23M / 1.9T; CB-ConvLSTM 417K / 1.09T | Table 2 |
| AIS 2024 leaderboard | Challenge ranking | p10 private | 99.58% | rank 1 | AIS 2024 survey Table 1 |

## 1.5 Brief Overview

> 이 논문은 Recurrent state-space event eye tracking 분야에서 event stream의 static information loss와 irregular temporal pattern 때문에 단순 CNN/GRU가 long-term eye movement context를 충분히 모델링하지 못하는 문제.를 해결하기 위해 Bina-rep event representation + CNN spatial extractor + bidirectional GRU + Linear Time-Varying State-Space Model (LTV-SSM)으로 pupil center를 예측한다.라는 설계 방향을 제안한다. 핵심 방법은 event stream을 binary map representation으로 구성하고 CNN blocks가 spatial features를 추출한다. adaptive global pooling 및 dropout 후 Bi-GRU와 LTV-SSM이 temporal context를 모델링하고 FC/regressor가 pupil center를 출력한다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 Bi-GRU와 LTV-SSM/Mamba block을 결합해 short/long temporal dependency를 함께 모델링.에 초점을 맞춘다.
...
- **Primary axis:** Recurrent state-space event eye tracking
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `Bina-rep event representation + CNN spatial extractor + bidirectional GRU + Linear Time-Varying State-Space Model (LTV-SSM)으로 pupil center를 예측한다.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| Bina-rep | sequential event frames를 bit-stack/binary map으로 압축 | input representation | Method |
| Bi-GRU | 양방향 recurrent context aggregation | short-term temporal patterns | Method |
| LTV-SSM / Mamba | selective state-space temporal modeling | long-term dependency | Method/Eq. 2-6 |
...
event stream을 binary map representation으로 구성하고 CNN blocks가 spatial features를 추출한다. adaptive global pooling 및 dropout 후 Bi-GRU와 LTV-SSM이 temporal context를 모델링하고 FC/regressor가 pupil center를 출력한다.
...
- event stream -> Bina-rep
- CNN spatial feature extractor
- adaptive global pooling + spatial dropout
- Bi-GRU
- LTV-SSM/Mamba block
- RMSE loss로 coordinate regression
...
이 논문은 event stream의 static information loss와 irregular temporal pattern 때문에 단순 CNN/GRU가 long-term eye movement context를 충분히 모델링하지 못하는 문제.를 대상으로 Bina-rep event representation + CNN spatial extractor + bidirectional GRU + Linear Time-Varying State-Space Model (LTV-SSM)으로 pupil center를 예측한다.를 제안한다.
...
- temporal modeling 관점이 강하고 challenge 성능이 우수
- Bina-rep이 event data를 compact하게 표현
- Bi-GRU + SSM 조합이 event sparsity에 따른 state 유지 문제에 적합

## 7.5 Weak or Unclear Parts

- FLOPs가 매우 커 hardware/edge deployment에는 부담
- bidirectional structure는 online causality 측면에서 주의 필요
- model size/compute 대비 efficiency trade-off가 SEE/FACET/Retina 대비 약함
...
> MambaPupil: Bidirectional Selective Recurrent Model for Event-based Eye Tracking는 Recurrent state-space event eye tracking 관점에서 Bina-rep event representation + CNN spatial extractor + bidirectional GRU + Linear Time-Varying State-Space Model (LTV-SSM)으로 pupil center를 예측한다.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** Bina-rep event representation + CNN spatial extractor + bidirectional GRU + Linear Time-Varying State-Space Model (LTV-SSM)으로 pupil center를 예측한다.
...
- **Evidence:** Cons list: FLOPs가 매우 커 hardware/edge deployment에는 부담, bidirectional structure는 online causality 측면에서 주의 필요, model size/compute 대비 efficiency trade-off가 SEE/FACET/Retina 대비 약함
