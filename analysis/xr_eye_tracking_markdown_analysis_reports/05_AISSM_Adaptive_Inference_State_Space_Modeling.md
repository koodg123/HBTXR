# Paper Summary: Enhancing Eye Feature Estimation from Event Data Streams through Adaptive Inference State Space Modeling

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `ENHANCING EYE FEATURE ESTIMATION FROM EVENT DATA STREAMS THROUGH ADAPTIVE INFERENCE STATE SPACE MODELING.pdf` |
...
| **Authors** | Viet Dung Nguyen, Mobina Ghorbaninejad, Chengyi Ma, Reynold Bailey, Gabriel J. Diaz |
| **Venue** | arXiv preprint |
| **Year** | 2026 |
| **Research Field** | Adaptive state-space modeling for event-based eye feature estimation |
...
- **Target Problem:** event density와 SNR이 eye behavior에 따라 급격히 변할 때, CNN/RNN 기반 feature extractor가 current observation과 history를 고정적으로 결합해 성능이 불안정해지는 문제.
- **Research Objective:** Kalman filter에서 영감을 받은 Adaptive Inference State Space Model (AISSM)로 현재 event 정보와 과거 state 정보를 동적으로 가중.
...
- **Core Idea:** dynamic confidence network가 event density/label confidence를 반영해 posterior/current observation과 prior/history를 adaptive fusion한다.
- **Proposed Approach:** event representation을 CNN encoder로 처리해 posterior를 만들고, recurrent transition model이 prior를 생성한다. dynamic confidence network가 event density와 ROI confidence를 이용해 두 정보를 가중 결합한 뒤 task head가 pupil feature를 예측한다.
...
1. AISSM: event density 변화에 대응하는 adaptive inference state-space model 제안
2. dynamic confidence network로 current vs historical information 가중
3. long-horizon training으로 training efficiency와 stability 개선
4. ≈500K parameter constraint에서 baseline 대비 P5/P10/P15 개선

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| 3ET+ at 320×320 | Eye feature / pupil center estimation | P5 / P10 / P15 | 46.85±21.49 / 72.38±8.13 / 85.41±0.79 | ≈500K parameter constraint; best among listed baselines | Table 1 |
| 3ET+ | Normalized predicted error | Distance | 0.01±0.01 | CNN/CNN-GRU/CB-ConvLSTM/MambaPupil baselines worse or higher variance | Table 1 |
| 3ET+ | Stability | Std. deviation | Lower than baselines | authors attribute to adaptive weighting across event density changes | Section Experimental Results |

## 1.5 Brief Overview

> 이 논문은 Adaptive state-space modeling for event-based eye feature estimation 분야에서 event density와 SNR이 eye behavior에 따라 급격히 변할 때, CNN/RNN 기반 feature extractor가 current observation과 history를 고정적으로 결합해 성능이 불안정해지는 문제.를 해결하기 위해 dynamic confidence network가 event density/label confidence를 반영해 posterior/current observation과 prior/history를 adaptive fusion한다.라는 설계 방향을 제안한다. 핵심 방법은 event representation을 CNN encoder로 처리해 posterior를 만들고, recurrent transition model이 prior를 생성한다. dynamic confidence network가 event density와 ROI confidence를 이용해 두 정보를 가중 결합한 뒤 task head가 pupil feature를 예측한다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 Kalman filter에서 영감을 받은 Adaptive Inference State Space Model (AISSM)로 현재 event 정보와 과거 state 정보를 동적으로 가중.에 초점을 맞춘다.
...
- **Primary axis:** Adaptive state-space modeling for event-based eye feature estimation
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `dynamic confidence network가 event density/label confidence를 반영해 posterior/current observation과 prior/history를 adaptive fusion한다.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| AISSM | posterior와 prior를 adaptive하게 결합하는 state-space model | event density/SNR 변화 대응 | Section 3 |
| Dynamic Confidence Network | current observation 신뢰도를 추정하는 module | gating/weighting signal 생성 | Section 3.3 |
| Long-horizon Training | 긴 sequence에서 state reuse 안정화를 위한 학습 기법 | training stability | Section 3/Experiments |
...
event representation을 CNN encoder로 처리해 posterior를 만들고, recurrent transition model이 prior를 생성한다. dynamic confidence network가 event density와 ROI confidence를 이용해 두 정보를 가중 결합한 뒤 task head가 pupil feature를 예측한다.
...
- 3ET+ event stream preprocessing 및 binarep representation 구성
- encoder로 current observation posterior 추정
- transition model로 history 기반 prior 추정
- dynamic confidence network로 fusion weight 추정
- task head로 eye feature/pupil center 예측
...
이 논문은 event density와 SNR이 eye behavior에 따라 급격히 변할 때, CNN/RNN 기반 feature extractor가 current observation과 history를 고정적으로 결합해 성능이 불안정해지는 문제.를 대상으로 dynamic confidence network가 event density/label confidence를 반영해 posterior/current observation과 prior/history를 adaptive fusion한다.를 제안한다.
...
- event density/SNR 변화라는 실제 event tracking 문제를 명확히 겨냥
- current observation과 history를 adaptive하게 결합하는 설계 논리가 강함
- 동일 parameter budget에서 baseline들을 재구현하여 비교하려는 시도가 있음

## 7.5 Weak or Unclear Parts

- training/inference speed는 고려하지 않았다고 저자가 명시
- metric scale이 기존 3ET+/challenge paper와 직접 비교하기 어려움
- hardware deployment/latency/energy 분석은 제공되지 않음
...
> Enhancing Eye Feature Estimation from Event Data Streams through Adaptive Inference State Space Modeling는 Adaptive state-space modeling for event-based eye feature estimation 관점에서 dynamic confidence network가 event density/label confidence를 반영해 posterior/current observation과 prior/history를 adaptive fusion한다.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** dynamic confidence network가 event density/label confidence를 반영해 posterior/current observation과 prior/history를 adaptive fusion한다.
...
- **Evidence:** Cons list: training/inference speed는 고려하지 않았다고 저자가 명시, metric scale이 기존 3ET+/challenge paper와 직접 비교하기 어려움, hardware deployment/latency/energy 분석은 제공되지 않음
