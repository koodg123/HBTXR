# Paper Summary: EV-Eye: Rethinking High-frequency Eye Tracking through the Lenses of Event Cameras

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `196_EV_Eye_Rethinking_High_fre.pdf` |
...
| **Year** | 2023 |
| **Research Field** | Event-based eye/gaze tracking dataset and benchmark |
| **Paper Link / DOI** | 논문 PDF 기준 분석; DOI는 파일별로 상이하거나 논문에 명시되지 않음 |
| **Code / Project Link** | https://github.com/Ningreka/EV-Eye |
...
- **Target Problem:** kHz 이상 high-frequency eye/gaze tracking을 위해 기존 frame-based camera의 bandwidth/frame-rate 한계를 넘는 대규모 multimodal dataset과 benchmark가 부족한 문제.
- **Research Objective:** DAVIS346 event camera와 Tobii gaze reference를 결합한 EV-Eye dataset을 구축하고, frame-event hybrid pupil/gaze tracking benchmark를 제시.
...
- **Core Idea:** 1.5M near-eye grayscale images와 2.7B events, 2.7M gaze references를 포함하는 large-scale dataset을 제공하고, U-Net segmentation + event-based template matching + polynomial gaze regression pipeline을 benchmark로 제안.
- **Proposed Approach:** Frame-based U-Net segmentation으로 pupil mask와 boundary template를 얻고, candidate event subset을 pupil boundary 주변 annulus에서 선택한다. 이후 nearest-neighbor 기반 points-to-edge matching으로 translation을 반복 추정하여 pupil center를 갱신한다. pupil center는 n-order polynomial regression으로 Point of Gaze로 변환된다.
...
1. 48명 대상의 대규모 multimodal frame-event high-frequency eye/gaze tracking dataset 구축
2. fixation/saccade/smooth pursuit를 포함한 dense gaze reference 제공
3. DL-based pupil segmentation과 event-based points-to-edge matching을 결합한 hybrid benchmark 제안
4. pupil tracking뿐 아니라 gaze estimation evaluation까지 수행

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| EV-Eye | Frame-based pupil segmentation | IoU / F1 | 0.9187 / 0.9560 | DL-based vs model-based 0.8360 / 0.9075 | Section 5.2, Fig. 5 |
| EV-Eye | Frame-based pupil center | Pixel Error | 0.64 px | model-based 1.3 px | Section 5.2, Fig. 6(a) |
| EV-Eye | Event-based pupil tracking | Pixel Error | 1.2 px | EVBEYE model-based 7.7 px | Section 5.3, Fig. 6(b) |
| EV-Eye | Tracking frequency | Peak frequency | 38.4 KHz | 20 events per update | Section 5.4, Fig. 7 |

## 1.5 Brief Overview

> 이 논문은 Event-based eye/gaze tracking dataset and benchmark 분야에서 kHz 이상 high-frequency eye/gaze tracking을 위해 기존 frame-based camera의 bandwidth/frame-rate 한계를 넘는 대규모 multimodal dataset과 benchmark가 부족한 문제.를 해결하기 위해 1.5M near-eye grayscale images와 2.7B events, 2.7M gaze references를 포함하는 large-scale dataset을 제공하고, U-Net segmentation + event-based template matching + polynomial gaze regression pipeline을 benchmark로 제안.라는 설계 방향을 제안한다. 핵심 방법은 Frame-based U-Net segmentation으로 pupil mask와 boundary template를 얻고, candidate event subset을 pupil boundary 주변 annulus에서 선택한다. 이후 nearest-neighbor 기반 points-to-edge matching으로 translation을 반복 추정하여 pupil center를 갱신한다. pupil center는 n-order polynomial regression으로 Point of Gaze로 변환된다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 DAVIS346 event camera와 Tobii gaze reference를 결합한 EV-Eye dataset을 구축하고, frame-event hybrid pupil/gaze tracking benchmark를 제시.에 초점을 맞춘다.
...
- **Primary axis:** Event-based eye/gaze tracking dataset and benchmark
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `1.5M near-eye grayscale images와 2.7B events, 2.7M gaze references를 포함하는 large-scale dataset을 제공하고, U-Net segmentation + event-based template matching + polynomial gaze regression pipeline을 benchmark로 제안.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| DAVIS346 event stream | pixel-level intensity change를 asynchronous event `(x,y,t,p)`로 기록 | high-frequency pupil tracking의 입력 | Section 3.1-3.2 |
| Template-based pupil tracking | frame에서 얻은 pupil boundary를 event point subset으로 갱신 | event update마다 pupil center를 빠르게 갱신 | Section 4.2, Fig. 3-4 |
| Polynomial gaze regression | pupil center를 screen PoG로 subject-dependent mapping | gaze estimation으로 확장 | Section 4.3 |
...
Frame-based U-Net segmentation으로 pupil mask와 boundary template를 얻고, candidate event subset을 pupil boundary 주변 annulus에서 선택한다. 이후 nearest-neighbor 기반 points-to-edge matching으로 translation을 반복 추정하여 pupil center를 갱신한다. pupil center는 n-order polynomial regression으로 Point of Gaze로 변환된다.
...
- DAVIS346 grayscale frame + event stream 수집
- U-Net으로 pupil segmentation 및 morphological closing
- pupil boundary Q와 center c 추출
- event candidate subset P 선택: λ1γ < ||(x,y)-c|| < λ2γ
- points-to-edge matching으로 translation T 추정
- pupil center 갱신 후 polynomial regression으로 PoG 추정
...
이 논문은 kHz 이상 high-frequency eye/gaze tracking을 위해 기존 frame-based camera의 bandwidth/frame-rate 한계를 넘는 대규모 multimodal dataset과 benchmark가 부족한 문제.를 대상으로 1.5M near-eye grayscale images와 2.7B events, 2.7M gaze references를 포함하는 large-scale dataset을 제공하고, U-Net segmentation + event-based template matching + polynomial gaze regression pipeline을 benchmark로 제안.를 제안한다.
...
- Dataset 규모와 modality가 강함: grayscale, events, gaze references를 함께 제공
- saccade/fixation/smooth pursuit를 포함하여 기존 EBVEYE보다 gaze distribution이 조밀함
- template matching benchmark가 단순하고 해석 가능하며 ultra-high-frequency update 가능

## 7.5 Weak or Unclear Parts

- gaze reference는 Tobii 기반 reference이며 true ground truth가 아님
- headrest 기반 수집으로 실제 HMD 착용 중 head motion/lighting 변화 일반화는 추가 검증 필요
- U-Net segmentation과 polynomial calibration이 포함되어 완전한 event-only pipeline은 아님
...
> EV-Eye: Rethinking High-frequency Eye Tracking through the Lenses of Event Cameras는 Event-based eye/gaze tracking dataset and benchmark 관점에서 1.5M near-eye grayscale images와 2.7B events, 2.7M gaze references를 포함하는 large-scale dataset을 제공하고, U-Net segmentation + event-based template matching + polynomial gaze regression pipeline을 benchmark로 제안.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** 1.5M near-eye grayscale images와 2.7B events, 2.7M gaze references를 포함하는 large-scale dataset을 제공하고, U-Net segmentation + event-based template matching + polynomial gaze regression pipeline을 benchmark로 제안.
...
- **Evidence:** Cons list: gaze reference는 Tobii 기반 reference이며 true ground truth가 아님, headrest 기반 수집으로 실제 HMD 착용 중 head motion/lighting 변화 일반화는 추가 검증 필요, U-Net segmentation과 polynomial calibration이 포함되어 완전한 event-only pipeline은 아님
