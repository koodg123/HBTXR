# Paper Summary: EyeGraph: Modularity-aware Spatio Temporal Graph Clustering for Continuous Event-based Eye Tracking

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `EyeGraph Modularity-aware Spatio Temporal Graph Clustering for Continuous Event-based Eye Tracking.pdf` |
...
| **Year** | 2024 |
| **Research Field** | Unsupervised graph-based event eye tracking and dataset |
...
- **Target Problem:** 기존 event-based eye tracking은 RGB frame 의존, sparse labels, controlled lab dataset 의존성이 크고, unsupervised continuous tracking benchmark가 부족한 문제.
- **Research Objective:** wearable event camera dataset EyeGraph와 modularity-aware spatio-temporal graph clustering pipeline 제안.
...
- **Core Idea:** event를 graph node로 표현하고, dynamic graph construction + topology-guided clustering + RANSAC tube model로 pupil coordinates를 추정한다.
- **Proposed Approach:** event stream을 spatio-temporal graph로 변환하고 dynamic edge construction을 수행한다. 이후 topology-aware clustering으로 event groups를 만들고, RANSAC 기반 geometric fitting으로 pupil center를 산출한다.
...
1. EyeGraph dataset: 40 participants, mobility/lighting variation을 포함한 near-eye event dataset
2. dynamic graph construction으로 event relation을 adaptive하게 모델링
3. modularity-aware unsupervised clustering으로 pupil-related event cluster 추정
4. supervised method 대비 labeling 없이 competitive performance 제시

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| 3ET+ | Unsupervised pupil coordinate estimation | p10 / p5 / p1 / l2 / l1 | 91.45 / 89.22 / 28.34 / 3.88 / 4.24 | best supervised MambaPupil p10 99.42; unsupervised baselines lower | Table 4 |
| EyeGraph | Robustness under lighting/mobility | p10 / p5 / p1 / l2 / l1 | 93.67 / 91.78 / 31.90 / 3.54 / 3.98 | MambaPupil p10 95.88 but supervised | Table 4 |
| EyeGraph/EBV-Eye | Graph construction/clustering | SC/DB/Mo/Cd | superior or comparable to baselines | Radius/kNN/FPS/Gabriel/DBSCAN etc. | Table 2-3 |

## 1.5 Brief Overview

> 이 논문은 Unsupervised graph-based event eye tracking and dataset 분야에서 기존 event-based eye tracking은 RGB frame 의존, sparse labels, controlled lab dataset 의존성이 크고, unsupervised continuous tracking benchmark가 부족한 문제.를 해결하기 위해 event를 graph node로 표현하고, dynamic graph construction + topology-guided clustering + RANSAC tube model로 pupil coordinates를 추정한다.라는 설계 방향을 제안한다. 핵심 방법은 event stream을 spatio-temporal graph로 변환하고 dynamic edge construction을 수행한다. 이후 topology-aware clustering으로 event groups를 만들고, RANSAC 기반 geometric fitting으로 pupil center를 산출한다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 wearable event camera dataset EyeGraph와 modularity-aware spatio-temporal graph clustering pipeline 제안.에 초점을 맞춘다.
...
- **Primary axis:** Unsupervised graph-based event eye tracking and dataset
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `event를 graph node로 표현하고, dynamic graph construction + topology-guided clustering + RANSAC tube model로 pupil coordinates를 추정한다.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| Dynamic graph construction | event를 node로 두고 spatio-temporal relation edge를 adaptive 생성 | event topology 표현 | Section Dynamic Graph Construction |
| Modularity-aware clustering | cluster modularity를 높이도록 topology-guided clustering | pupil-related cluster 분리 | Section Graph Clustering |
| Tube-model RANSAC | cluster에서 pupil coordinate를 robust하게 추정 | coordinate estimation | Fig. 4 / Method |
...
event stream을 spatio-temporal graph로 변환하고 dynamic edge construction을 수행한다. 이후 topology-aware clustering으로 event groups를 만들고, RANSAC 기반 geometric fitting으로 pupil center를 산출한다.
...
- event stream graph node 생성
- Hawkes/dynamic edge feature attribution
- modularity-aware graph clustering
- cluster에서 pupil candidate extraction
- RANSAC/tube model로 coordinate estimation
...
이 논문은 기존 event-based eye tracking은 RGB frame 의존, sparse labels, controlled lab dataset 의존성이 크고, unsupervised continuous tracking benchmark가 부족한 문제.를 대상으로 event를 graph node로 표현하고, dynamic graph construction + topology-guided clustering + RANSAC tube model로 pupil coordinates를 추정한다.를 제안한다.
...
- label-free unsupervised pipeline으로 annotation burden을 줄임
- dataset이 controlled lab을 넘어 mobility/lighting variation을 포함
- graph topology 관점이 event stream의 sparse/asynchronous property와 잘 맞음

## 7.5 Weak or Unclear Parts

- supervised SOTA 대비 p10 gap 존재
- graph construction/clustering은 real-time latency와 hardware cost가 불명확
- dataset과 method가 함께 제안되어 외부 dataset 일반화 검증이 더 필요
...
> EyeGraph: Modularity-aware Spatio Temporal Graph Clustering for Continuous Event-based Eye Tracking는 Unsupervised graph-based event eye tracking and dataset 관점에서 event를 graph node로 표현하고, dynamic graph construction + topology-guided clustering + RANSAC tube model로 pupil coordinates를 추정한다.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** event를 graph node로 표현하고, dynamic graph construction + topology-guided clustering + RANSAC tube model로 pupil coordinates를 추정한다.
...
- **Evidence:** Cons list: supervised SOTA 대비 p10 gap 존재, graph construction/clustering은 real-time latency와 hardware cost가 불명확, dataset과 method가 함께 제안되어 외부 dataset 일반화 검증이 더 필요
