# Paper Summary: 3ET: Efficient Event-based Eye Tracking using a Change-Based ConvLSTM Network

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `3ET Efficient Event-based Eye Tracking using a Change-Based ConvLSTM Network.pdf` |
...
| **Venue** | IEEE BioCAS |
| **Year** | 2023 |
| **Research Field** | Efficient event-based neural eye tracking |
...
- **Target Problem:** event-based eye tracking에서 temporal memory가 필요하지만 ConvLSTM은 연산량이 크고, 단순 CNN은 sparse event의 temporal state를 충분히 보존하지 못하는 문제.
- **Research Objective:** event frame 변화가 있는 부분만 hidden-state update하는 sparse Change-Based ConvLSTM으로 정확도와 연산 효율을 동시에 확보.
...
- **Core Idea:** CB-ConvLSTM은 입력 변화가 있는 위치 중심으로 recurrent state를 갱신하여 표준 ConvLSTM 대비 FLOPs를 줄인다.
- **Proposed Approach:** event frames를 입력으로 받아 ConvLSTM memory를 유지하되, 변화가 작은 위치는 update를 skip한다. threshold θ를 조절하여 network sparsity와 accuracy를 trade-off한다.
...
1. event-based eye tracking용 sparse CB-ConvLSTM 제안
2. activation 변화 기반 sparse recurrent update로 연산량 감소
3. CNN baseline보다 높은 정확도, 표준 ConvLSTM과 유사한 정확도를 낮은 FLOPs로 달성

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| 3ET / event-based eye tracking benchmark | Pupil center tracking | p10 | 99.20% at θ=0.5 | vanilla ConvLSTM 99.40%, CNN 91.40% | Table I/II |
| 3ET | Efficiency | FLOPs | 9.00M at θ=0.5 | vanilla ConvLSTM 42.61M | Table I/II |
| 3ET | Sparsity | Network sparsity | 0.853 at θ=0.5 | threshold-controlled sparse update | Table I |

## 1.5 Brief Overview

> 이 논문은 Efficient event-based neural eye tracking 분야에서 event-based eye tracking에서 temporal memory가 필요하지만 ConvLSTM은 연산량이 크고, 단순 CNN은 sparse event의 temporal state를 충분히 보존하지 못하는 문제.를 해결하기 위해 CB-ConvLSTM은 입력 변화가 있는 위치 중심으로 recurrent state를 갱신하여 표준 ConvLSTM 대비 FLOPs를 줄인다.라는 설계 방향을 제안한다. 핵심 방법은 event frames를 입력으로 받아 ConvLSTM memory를 유지하되, 변화가 작은 위치는 update를 skip한다. threshold θ를 조절하여 network sparsity와 accuracy를 trade-off한다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 event frame 변화가 있는 부분만 hidden-state update하는 sparse Change-Based ConvLSTM으로 정확도와 연산 효율을 동시에 확보.에 초점을 맞춘다.
...
- **Primary axis:** Efficient event-based neural eye tracking
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `CB-ConvLSTM은 입력 변화가 있는 위치 중심으로 recurrent state를 갱신하여 표준 ConvLSTM 대비 FLOPs를 줄인다.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| CB-ConvLSTM | 변화 기반 sparse recurrent convolution | event 변화 위치 중심으로 hidden state 갱신 | Method Section |
| Threshold θ | sparse update 여부를 결정하는 threshold | sparsity-accuracy trade-off 조절 | Table I |
| p-accuracy | Euclidean pixel error가 p 이하인 sample 비율 | p3/p5/p10로 accuracy 평가 | Experiments |
...
event frames를 입력으로 받아 ConvLSTM memory를 유지하되, 변화가 작은 위치는 update를 skip한다. threshold θ를 조절하여 network sparsity와 accuracy를 trade-off한다.
...
- event stream을 frame representation으로 변환
- CB-ConvLSTM recurrent feature update
- prediction head로 pupil center 회귀
- p3/p5/p10 및 FLOPs로 평가
...
이 논문은 event-based eye tracking에서 temporal memory가 필요하지만 ConvLSTM은 연산량이 크고, 단순 CNN은 sparse event의 temporal state를 충분히 보존하지 못하는 문제.를 대상으로 CB-ConvLSTM은 입력 변화가 있는 위치 중심으로 recurrent state를 갱신하여 표준 ConvLSTM 대비 FLOPs를 줄인다.를 제안한다.
...
- recurrent temporal modeling을 유지하면서 FLOPs를 크게 줄임
- threshold로 sparsity/accuracy를 조절할 수 있어 hardware-friendly
- event data의 sparse temporal property를 직접 활용

## 7.5 Weak or Unclear Parts

- 정확한 hardware latency/power는 별도 accelerator 없이 FLOPs 중심으로 제시됨
- threshold 선택이 dataset/motion regime에 민감할 수 있음
- static/no-event 구간에서 state drift나 stale state 처리 검증이 추가로 필요
...
> 3ET: Efficient Event-based Eye Tracking using a Change-Based ConvLSTM Network는 Efficient event-based neural eye tracking 관점에서 CB-ConvLSTM은 입력 변화가 있는 위치 중심으로 recurrent state를 갱신하여 표준 ConvLSTM 대비 FLOPs를 줄인다.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** CB-ConvLSTM은 입력 변화가 있는 위치 중심으로 recurrent state를 갱신하여 표준 ConvLSTM 대비 FLOPs를 줄인다.
...
- **Evidence:** Cons list: 정확한 hardware latency/power는 별도 accelerator 없이 FLOPs 중심으로 제시됨, threshold 선택이 dataset/motion regime에 민감할 수 있음, static/no-event 구간에서 state drift나 stale state 처리 검증이 추가로 필요
