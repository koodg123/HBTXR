# Paper Summary: BRAT: Bidirectional Relative Positional Attention Transformer for Event-based Eye Tracking

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `Wu_BRAT_Bidirectional_Relative_Positional_Attention_Transformer_for_Event-based_Eye_tracking_CVPRW_2025_paper.pdf` |
...
| **Authors** | Yuliang Wu, Han Han, Jinze Chen, Wei Zhai, Yang Cao, Zheng-jun Zha |
| **Venue** | CVPRW |
| **Year** | 2025 |
| **Research Field** | Transformer/recurrent event eye tracking |
| **Paper Link / DOI** | 논문 PDF 기준 분석; DOI는 파일별로 상이하거나 논문에 명시되지 않음 |
| **Code / Project Link** | https://github.com/hh-xiaohu/Event-based-Eye-Tracking-Challenge-Solution |
...
- **Target Problem:** event camera는 high temporal resolution을 제공하지만 static information loss, dynamic signal complexity, irregular spatio-temporal distribution 문제가 있어 long-range temporal modeling이 어렵다.
- **Research Objective:** Bi-GRU와 bidirectional relative positional attention transformer로 event eye tracking의 short/long temporal dependency를 모두 모델링.
...
- **Core Idea:** CNN spatial encoder + Bi-GRU + BRAT block을 사용하고, attention score에 forward/backward relative temporal bias를 추가한다.
- **Proposed Approach:** event binary representation을 CNN spatial encoder에 통과시킨 후, sequence features를 Bi-GRU로 처리한다. 이후 BRAT transformer가 relative temporal distance bias를 반영한 bidirectional attention으로 long-range dependencies를 추출하고, regression head가 pupil center를 출력한다.
...
1. Bidirectional Relative Positional Attention (BRAT) block 제안
2. CNN spatial encoder와 Bi-GRU temporal decoder 결합
3. multi-time-step sampling strategy로 long temporal span 학습
4. 2025 Event-Based Eye Tracking Challenge 1위 성능 달성

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| 3ET+ 2025 validation | Pupil center tracking | p5 / p10 / p15 / pixel error | 0.978 / 0.995 / 1.000 / 1.14 | MambaPupil 0.963 / 0.990 / 0.997 / 1.34 | Table 1 / Table 3 |
| Model complexity | Efficiency | Params / MACs | 7.1M / 2.9G | challenge factsheet | Table 2 |
| 3ET+ 2025 challenge | Leaderboard | Pixel error | 1.14 | rank 1 | Challenge survey Table 1 |

## 1.5 Brief Overview

> 이 논문은 Transformer/recurrent event eye tracking 분야에서 event camera는 high temporal resolution을 제공하지만 static information loss, dynamic signal complexity, irregular spatio-temporal distribution 문제가 있어 long-range temporal modeling이 어렵다.를 해결하기 위해 CNN spatial encoder + Bi-GRU + BRAT block을 사용하고, attention score에 forward/backward relative temporal bias를 추가한다.라는 설계 방향을 제안한다. 핵심 방법은 event binary representation을 CNN spatial encoder에 통과시킨 후, sequence features를 Bi-GRU로 처리한다. 이후 BRAT transformer가 relative temporal distance bias를 반영한 bidirectional attention으로 long-range dependencies를 추출하고, regression head가 pupil center를 출력한다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 Bi-GRU와 bidirectional relative positional attention transformer로 event eye tracking의 short/long temporal dependency를 모두 모델링.에 초점을 맞춘다.
...
- **Primary axis:** Transformer/recurrent event eye tracking
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `CNN spatial encoder + Bi-GRU + BRAT block을 사용하고, attention score에 forward/backward relative temporal bias를 추가한다.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| BRAT attention | QK^T/sqrt(d)에 forward/backward relative position bias B를 더함 | temporal distance-aware attention | Equation 1-3, Fig. 3 |
| Bi-GRU | short-term temporal motion pattern 처리 | pre-attention temporal encoding | Fig. 2 |
| Multi-time-step sampling | sliding window와 step size로 다양한 temporal span sampling | generalization | Method |
...
event binary representation을 CNN spatial encoder에 통과시킨 후, sequence features를 Bi-GRU로 처리한다. 이후 BRAT transformer가 relative temporal distance bias를 반영한 bidirectional attention으로 long-range dependencies를 추출하고, regression head가 pupil center를 출력한다.
...
- CNN spatial encoder with large kernels
- Bi-GRU short-term temporal block
- BRAT relative positional attention
- coordinate regression with sequence RMSE loss
...
이 논문은 event camera는 high temporal resolution을 제공하지만 static information loss, dynamic signal complexity, irregular spatio-temporal distribution 문제가 있어 long-range temporal modeling이 어렵다.를 대상으로 CNN spatial encoder + Bi-GRU + BRAT block을 사용하고, attention score에 forward/backward relative temporal bias를 추가한다.를 제안한다.
...
- relative temporal bias를 통해 event sequence temporal distance를 명시적으로 모델링
- 2025 challenge 최고 성능으로 accuracy 강함
- CNN/Bi-GRU/Transformer 역할 분리가 명확

## 7.5 Weak or Unclear Parts

- 7.1M params와 2.9G MACs는 edge/hardware deployment에 부담
- bidirectional attention은 causal online inference에 제약 가능
- 고성능이 challenge split/data setting에 특화되었는지 추가 검증 필요
...
> BRAT: Bidirectional Relative Positional Attention Transformer for Event-based Eye Tracking는 Transformer/recurrent event eye tracking 관점에서 CNN spatial encoder + Bi-GRU + BRAT block을 사용하고, attention score에 forward/backward relative temporal bias를 추가한다.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** CNN spatial encoder + Bi-GRU + BRAT block을 사용하고, attention score에 forward/backward relative temporal bias를 추가한다.
...
- **Evidence:** Cons list: 7.1M params와 2.9G MACs는 edge/hardware deployment에 부담, bidirectional attention은 causal online inference에 제약 가능, 고성능이 challenge split/data setting에 특화되었는지 추가 검증 필요
