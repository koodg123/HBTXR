# Paper Summary: Exploring Temporal Dynamics in Event-based Eye Tracker / TDTracker

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `TDTracker.pdf` |
...
| Item | Description |
|---|---|
| **Title** | Exploring Temporal Dynamics in Event-based Eye Tracker / TDTracker |
| **Authors** | Hongwei Ren, Xiaopeng Lin, Hongxiang Huang, Yue Zhou, Bojun Cheng |
| **Venue** | CVPRW 2025 challenge-related paper |
| **Year** | 2025 |
| **Research Field** | Temporal dynamics modeling for event-based eye tracking |
| **Paper Link / DOI** | 논문 PDF 기준 분석; DOI는 파일별로 상이하거나 논문에 명시되지 않음 |
| **Code / Project Link** | https://github.com/rhwxmx/TDTracker |
...
- **Target Problem:** event-based eye tracking에서 short-term implicit temporal dynamics와 long-term explicit temporal dynamics를 동시에 모델링해야 하는 문제.
- **Research Objective:** 3D CNN과 Frequency-Aware Module, GRU, Mamba를 cascading하여 temporal dynamics를 포괄적으로 추출.
...
- **Core Idea:** Implicit Temporal Dynamic (ITD) blocks가 local short-term dynamics를 추출하고, Explicit Temporal Dynamic (ETD)가 frequency-aware/GRU/Mamba로 long-term patterns를 모델링한다.
- **Proposed Approach:** event representation을 3D CNN에 입력해 short-term temporal structure를 추출하고, Frequency-Aware Module/GRU/Mamba cascade가 long-term temporal dependencies를 처리한다. 최종적으로 heatmap으로 pupil coordinate를 회귀한다.
...
1. TDTracker architecture 제안: 3D CNN + cascaded temporal models
2. implicit and explicit temporal dynamics를 분리하여 모델링
3. heatmap regression으로 precise coordinate estimation
4. 2025 challenge 상위권 결과 달성

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| 3ET+ 2025 challenge | Pupil center tracking | Private / Public pixel error | 1.50861 / 1.46623 | rank 3 in challenge survey | Table 6 / challenge survey |
| 3ET+ 2025 | Efficiency | Params / FLOPs / latency | 3.04M / 265M / 1.79ms | paper-reported | Table 6 |

## 1.5 Brief Overview

> 이 논문은 Temporal dynamics modeling for event-based eye tracking 분야에서 event-based eye tracking에서 short-term implicit temporal dynamics와 long-term explicit temporal dynamics를 동시에 모델링해야 하는 문제.를 해결하기 위해 Implicit Temporal Dynamic (ITD) blocks가 local short-term dynamics를 추출하고, Explicit Temporal Dynamic (ETD)가 frequency-aware/GRU/Mamba로 long-term patterns를 모델링한다.라는 설계 방향을 제안한다. 핵심 방법은 event representation을 3D CNN에 입력해 short-term temporal structure를 추출하고, Frequency-Aware Module/GRU/Mamba cascade가 long-term temporal dependencies를 처리한다. 최종적으로 heatmap으로 pupil coordinate를 회귀한다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 3D CNN과 Frequency-Aware Module, GRU, Mamba를 cascading하여 temporal dynamics를 포괄적으로 추출.에 초점을 맞춘다.
...
- **Primary axis:** Temporal dynamics modeling for event-based eye tracking
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `Implicit Temporal Dynamic (ITD) blocks가 local short-term dynamics를 추출하고, Explicit Temporal Dynamic (ETD)가 frequency-aware/GRU/Mamba로 long-term patterns를 모델링한다.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| ITD | 3D CNN 기반 short-term temporal feature extraction | implicit local dynamics | Fig. 4 |
| ETD | Frequency-Aware Module + GRU + Mamba cascade | explicit long-term dynamics | Fig. 4 |
| Heatmap regression | coordinate를 heatmap 형태로 예측 | fine localization | Method/Fig. 5 |
...
event representation을 3D CNN에 입력해 short-term temporal structure를 추출하고, Frequency-Aware Module/GRU/Mamba cascade가 long-term temporal dependencies를 처리한다. 최종적으로 heatmap으로 pupil coordinate를 회귀한다.
...
- GRU
- Mamba
- heatmap generation and coordinate regression
...
이 논문은 event-based eye tracking에서 short-term implicit temporal dynamics와 long-term explicit temporal dynamics를 동시에 모델링해야 하는 문제.를 대상으로 Implicit Temporal Dynamic (ITD) blocks가 local short-term dynamics를 추출하고, Explicit Temporal Dynamic (ETD)가 frequency-aware/GRU/Mamba로 long-term patterns를 모델링한다.를 제안한다.
...
- temporal dynamics를 short/long-term으로 분해한 설계가 명확
- challenge 상위권 accuracy와 moderate model size를 달성
- frequency-aware processing은 eye movement signal 특성과 잘 맞음

## 7.5 Weak or Unclear Parts

- 3D CNN+GRU+Mamba cascade는 hardware mapping이 복잡할 수 있음
- latency 1.79ms는 2KHz 요구에는 부족할 수 있음
- ablation/energy/hardware deployment 분석은 제한적
...
> Exploring Temporal Dynamics in Event-based Eye Tracker / TDTracker는 Temporal dynamics modeling for event-based eye tracking 관점에서 Implicit Temporal Dynamic (ITD) blocks가 local short-term dynamics를 추출하고, Explicit Temporal Dynamic (ETD)가 frequency-aware/GRU/Mamba로 long-term patterns를 모델링한다.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** Implicit Temporal Dynamic (ITD) blocks가 local short-term dynamics를 추출하고, Explicit Temporal Dynamic (ETD)가 frequency-aware/GRU/Mamba로 long-term patterns를 모델링한다.
...
- **Evidence:** Cons list: 3D CNN+GRU+Mamba cascade는 hardware mapping이 복잡할 수 있음, latency 1.79ms는 2KHz 요구에는 부족할 수 있음, ablation/energy/hardware deployment 분석은 제한적
