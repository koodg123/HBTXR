# Paper Summary: Inference-Time Gaze Refinement for Micro-Expression Recognition: Enhancing Event-Based Eye Tracking with Motion-Aware Post-Processing

## Analysis Coverage

| Item | Description |
|---|---|
| Source PDF | `Inference-Time Gaze Refinement for Micro-Expression Recognition Enhancing Event-Based Eye Tracking with Motion-Aware Post-Processing.pdf` |
...
| **Year** | 2025 |
| **Research Field** | Model-agnostic event eye tracking post-processing |
| **Paper Link / DOI** | 논문 PDF 기준 분석; DOI는 파일별로 상이하거나 논문에 명시되지 않음 |
| **Code / Project Link** | github/EyeLoRiN (as stated in survey) |
...
- **Target Problem:** blink artifact, temporal inconsistency, local event distribution misalignment 때문에 model output gaze/pupil trajectory가 불안정해지는 문제.
- **Research Objective:** retraining 없이 기존 event eye tracking model에 붙일 수 있는 inference-time post-processing 제안.
...
- **Core Idea:** Motion-Aware Median Filtering (M2F)로 temporal smoothness를 보장하고, Optical Flow-based Refinement (OFE)로 local event flow에 맞춰 prediction을 보정한다.
- **Proposed Approach:** base model의 prediction sequence를 받아 local motion variance를 계산하고 adaptive median filtering을 적용한다. 이후 prediction 주변 ROI에서 event flow direction을 추정하여 local misalignment를 보정한다.
...
1. model-agnostic M2F/OFE post-processing 제안
2. blink artifact와 abrupt gaze shift 완화
3. CG, BigBrains base model에서 pixel error 개선
4. learnable parameter 없이 수백 FLOPs 수준 overhead

## 1.4 Representative Results

| Dataset | Task | Metric | Result | Comparison Condition | Source |
| --- | --- | --- | --- | --- | --- |
| 3ET+ 2025 challenge | Post-processing on CG | Private pixel error | 7.504 | CG baseline 7.922 | Table 4 |
| 3ET+ 2025 challenge | Post-processing on BigBrains | Private pixel error | 1.423 | BB baseline 1.500; M2F only 1.466 | Table 4 |
| Complexity | Overhead | FLOPs per prediction | M2F≈172, OFE≈340 | no learnable params | Results/Table 5 |

## 1.5 Brief Overview

> 이 논문은 Model-agnostic event eye tracking post-processing 분야에서 blink artifact, temporal inconsistency, local event distribution misalignment 때문에 model output gaze/pupil trajectory가 불안정해지는 문제.를 해결하기 위해 Motion-Aware Median Filtering (M2F)로 temporal smoothness를 보장하고, Optical Flow-based Refinement (OFE)로 local event flow에 맞춰 prediction을 보정한다.라는 설계 방향을 제안한다. 핵심 방법은 base model의 prediction sequence를 받아 local motion variance를 계산하고 adaptive median filtering을 적용한다. 이후 prediction 주변 ROI에서 event flow direction을 추정하여 local misalignment를 보정한다.이다. 실험적으로는 위 표의 대표 수치처럼 accuracy, latency, model complexity, robustness 중 하나 이상의 축에서 기존 방법 대비 개선을 보고한다. 다만 dataset, resolution, online/offline setting, hardware platform 차이가 있어 다른 논문과의 직접 비교는 주의가 필요하다.
...
저자들은 event camera의 high temporal resolution과 sparse output을 활용하면 XR/AR/VR wearable에서 high-frequency eye tracking을 더 효율적으로 구현할 수 있다고 본다. 본 논문은 그중에서도 retraining 없이 기존 event eye tracking model에 붙일 수 있는 inference-time post-processing 제안.에 초점을 맞춘다.
...
- **Primary axis:** Model-agnostic event eye tracking post-processing
- **Input modality:** event-only 또는 event-frame hybrid 여부는 논문별로 다르며, 본 논문은 `Motion-Aware Median Filtering (M2F)로 temporal smoothness를 보장하고, Optical Flow-based Refinement (OFE)로 local event flow에 맞춰 prediction을 보정한다.`에 해당하는 접근을 취한다.
...
| --- | --- | --- | --- |
| M2F | local motion variance에 따라 adaptive median window를 정하는 filtering | temporal smoothness | Algorithm 1 |
| OFE | ROI 내 event flow vector로 prediction을 부드럽게 shift | spatial refinement | Algorithm 2 |
| Jitter Metric | temporal instability를 측정하기 위한 metric | micro-expression/gaze trace quality | Method |
...
base model의 prediction sequence를 받아 local motion variance를 계산하고 adaptive median filtering을 적용한다. 이후 prediction 주변 ROI에서 event flow direction을 추정하여 local misalignment를 보정한다.
...
- base model prediction sequence 획득
- motion variance estimation
- adaptive median filtering
- ROI event extraction
- rule-based local optical flow estimation
- prediction shift/refinement
...
이 논문은 blink artifact, temporal inconsistency, local event distribution misalignment 때문에 model output gaze/pupil trajectory가 불안정해지는 문제.를 대상으로 Motion-Aware Median Filtering (M2F)로 temporal smoothness를 보장하고, Optical Flow-based Refinement (OFE)로 local event flow에 맞춰 prediction을 보정한다.를 제안한다.
...
- architecture 변경 없이 여러 model에 적용 가능
- overhead가 매우 작아 edge deployment에 유리
- blink/temporal smoothness라는 실제 tracking failure mode를 직접 겨냥

## 7.5 Weak or Unclear Parts

- rule-based threshold/ROI 설정이 dataset별 tuning을 요구할 수 있음
- 근본적인 model representation 개선은 아니므로 큰 systematic bias는 해결 제한
- ground truth temporal smoothness와 over-smoothing trade-off 검증 필요
...
> Inference-Time Gaze Refinement for Micro-Expression Recognition: Enhancing Event-Based Eye Tracking with Motion-Aware Post-Processing는 Model-agnostic event eye tracking post-processing 관점에서 Motion-Aware Median Filtering (M2F)로 temporal smoothness를 보장하고, Optical Flow-based Refinement (OFE)로 local event flow에 맞춰 prediction을 보정한다.를 통해 event-based eye tracking의 accuracy-efficiency trade-off를 개선하려는 연구다.
...
- **Evidence:** Motion-Aware Median Filtering (M2F)로 temporal smoothness를 보장하고, Optical Flow-based Refinement (OFE)로 local event flow에 맞춰 prediction을 보정한다.
...
- **Evidence:** Cons list: rule-based threshold/ROI 설정이 dataset별 tuning을 요구할 수 있음, 근본적인 model representation 개선은 아니므로 큰 systematic bias는 해결 제한, ground truth temporal smoothness와 over-smoothing trade-off 검증 필요
