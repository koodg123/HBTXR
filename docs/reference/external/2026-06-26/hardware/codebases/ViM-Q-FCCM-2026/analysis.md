...
source_name: ViM-Q-FCCM-2026
analysis_scope: static
integrated_package: analysis/integrated-2026-06-26
source_group: references-hardware-codebase
---

# ViM-Q-FCCM-2026 Analysis

- corpus: `References/Hardware`
- local_path: `/home/kjm26/project/PRJXR/References/Hardware/ViM-Q-FCCM-2026`
- category: `Quantized ViM hardware`
- HGTXR relevance: `Quantized vision sequence accelerator`
- recommended_priority: `P1`
- evidence_type: static local inventory and integrated reference analysis

## 1. 분석 범위와 판정

이 문서는 `/home/kjm26/project/PRJXR/References/Hardware` 하위의 `ViM-Q-FCCM-2026` 항목을 HGTXR 하드웨어 설계 관점에서 분리 저장한 개별 분석 문서다. 현재 단계에서는 정적 파일 인벤토리와 상위 통합 분석을 근거로 하며, 학습/합성/보드 실행 결과를 새로 생성하지 않았다.
...
|---|---:|
| 로컬 파일 수 추정 | `199` |
| 주요 확장자 | `.py:56, [no_ext]:23, .h:17, .scala:16, .dat:14, .sample:14, .sh:10, .cu:9` |
| README 감지 | `README.md` |

## 3. 핵심 파일 / 구성요소

- `HW/SPINAL/src/main/verilog/*/all.v`
...
| 관점 | 판단 |
|---|---|
| 알고리즘 역할 | `Quantized ViM hardware` |
| HGTXR 적용성 | `Quantized vision sequence accelerator` |
| 우선순위 | `P1` |
| 직접 HW 구현성 | 높음 |
...
| 항목 | 내용 |
|---|---|
| 예상 효과 | `Quantized vision sequence accelerator` 기반의 실험 후보 확보 |
...
`ViM-Q-FCCM-2026`는 `P1` 우선순위의 참고 항목이다. 현재 HGTXR 목표인 DSP 활용률 확대, LUT 압박 완화, URAM/LUTRAM 역할 분리와 충돌하지 않는 범위에서만 실험 후보로 승격한다.
