...
source_name: acap-gemm-sa
analysis_scope: static
integrated_package: analysis/integrated-2026-06-26
source_group: references-hardware-codebase
---

# acap-gemm-sa Analysis

- corpus: `References/Hardware`
- local_path: `/home/kjm26/project/PRJXR/References/Hardware/acap-gemm-sa`
- category: `ACAP GEMM systolic array`
- HGTXR relevance: `Systolic MAC reference`
- recommended_priority: `P2`
- evidence_type: static local inventory and integrated reference analysis

## 1. 분석 범위와 판정

이 문서는 `/home/kjm26/project/PRJXR/References/Hardware` 하위의 `acap-gemm-sa` 항목을 HGTXR 하드웨어 설계 관점에서 분리 저장한 개별 분석 문서다. 현재 단계에서는 정적 파일 인벤토리와 상위 통합 분석을 근거로 하며, 학습/합성/보드 실행 결과를 새로 생성하지 않았다.
...
|---|---:|
| 로컬 파일 수 추정 | `102` |
| 주요 확장자 | `.cc:20, .hh:17, .py:15, .sample:14, [no_ext]:13, .txt:6, .cfg:4, .tcl:4` |
| README 감지 | `README.md` |
...
> ## Build
> Make any design parameter changes in `$SRC_DIR`:
...
| 관점 | 판단 |
|---|---|
| 알고리즘 역할 | `ACAP GEMM systolic array` |
| HGTXR 적용성 | `Systolic MAC reference` |
| 우선순위 | `P2` |
| 직접 HW 구현성 | 중간/간접 |
...
| 항목 | 내용 |
|---|---|
| 예상 효과 | `Systolic MAC reference` 기반의 실험 후보 확보 |
...
`acap-gemm-sa`는 `P2` 우선순위의 참고 항목이다. 현재 HGTXR 목표인 DSP 활용률 확대, LUT 압박 완화, URAM/LUTRAM 역할 분리와 충돌하지 않는 범위에서만 실험 후보로 승격한다.
