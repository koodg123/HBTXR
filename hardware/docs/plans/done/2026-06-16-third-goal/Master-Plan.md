...
- Latest no-board P0 evidence: separate Search/Track mode-profile tops pass HLS CSim/csynth/IP package and ZCU104 Vivado route against the active latency targets. Search reports `772,268` cycles / `3.861 ms` against `4 ms` with routed WNS `2.638 ns`; Track reports `99,449` cycles / `0.497 ms` against `1 ms` with routed WNS `1.481 ns`. Search still consumes `96/96` URAM, so resource margin remains a P0 risk even though route passed.
...
5. Reduce Search mode URAM pressure or explicitly keep it as zero-margin routed no-board evidence.
6. Promote only bounded successor variants after C3b smoke is captured and the C3b protection checklist passes.
7. Keep P1/P2 attention/MoE/model-structure ideas as software-first ablations until paper-scope review approves them.
...
- Search/Track mode-latency Vivado-level claims may cite the routed no-board mode-profile evidence, but deployed board-runtime claims still require runtime/board instrumentation.
