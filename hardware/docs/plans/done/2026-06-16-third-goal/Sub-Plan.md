...
Latest no-board mode-profile work used GPT-5.3-Codex-Spark read-only sidecars for mode-top structure, script/Tcl integration audits, and documentation-update planning. Main agent implemented the Search/Track HLS profile tops, wrappers, report collector integration, CSim/csynth/package runs, ZCU104 Vivado route runs, and documentation updates.
...
- `MODE-TOP-AUDIT`: GPT-5.3-Codex-Spark read-only audit for Search/Track HGTXR top structure; found that current E2E AXIS top runs Search and Track sequentially and recommended separate mode-profile tops.
- `MODE-SCRIPT-AUDIT`: GPT-5.3-Codex-Spark read-only audit for no-board Vitis/Vivado script extension points; found that script-only mode suffixes were insufficient because the current E2E top always executes the full path.
- `NB-P0-VIVADO-DOC-PREP`: GPT-5.3-Codex-Spark read-only documentation audit for mode-profile route evidence sections and metrics to capture.

## Task Cards

```yaml
task_card:
  task_id: NB-P0-MODE-PROFILE
  sub_agent: "gpt5.3-codex-spark + codex-native"
  role: "analyst + implementer + evaluator"
  objective: "Add no-board Search/Track mode-profile HLS tops and prove Search <= 4 ms and Track <= 1 ms at CSim/csynth/IP-package level."
  file_ownership:
    - "hls/src/hgtxr_mode_profile_top.cpp"
    - "hls/tb/tb_hgtxr_mode_profile_top.cpp"
    - "vivado/scripts/run_mode_profile_q4w8a_csim.tcl"
    - "vivado/scripts/run_mode_profile_q4w8a_csynth.tcl"
    - "vivado/scripts/package_mode_profile_ip.tcl"
    - "scripts/run/run_mode_profile_no_board.sh"
    - "scripts/report/collect_no_board_reports.py"
    - "analysis/no-board-results/MODE_PROFILE_RESULTS_2026_06_27.md"
    - "analysis/no-board-results/P0_PROGRESS_2026_06_26.md"
    - "analysis/NO_BOARD_PERFORMANCE_EXPERIMENT_PLAN_2026_06_26.md"
    - "docs/Master-Plan.md"
    - "docs/Sub-Plan.md"
    - "docs/Spec.md"
  ...
  inputs:
    ...
    - "existing no-board HLS/Vivado run scripts"
  outputs:
    - "Search profile csynth: 772268 cycles, 3.861 ms, target met"
    - "Track profile csynth: 99449 cycles, 0.497 ms, target met"
    - "Search and Track HLS IP component.xml/export.zip"
    - "Search profile Vivado route: WNS 2.638 ns, route errors 0, bitgen pass"
    - "Track profile Vivado route: WNS 1.481 ns, route errors 0, bitgen pass"
    - "collector output analysis/no-board-results/p0_mode_profile_2026_06_27/"
  validation:
    ...
    - "sh scripts/run/run_mode_profile_vivado_no_board.sh search_par32"
    - "sh scripts/run/run_mode_profile_vivado_no_board.sh track_par32"
    - "python3 scripts/report/collect_no_board_reports.py --run-id p0_mode_profile_2026_06_27 --experiment-id NB-P0-MODE-PROFILE --output-dir analysis/no-board-results/p0_mode_profile_2026_06_27"
  dependencies:
    - "Search URAM margin is zero at 96/96 and should be reduced before relying on resource headroom."
    - "Board-runtime and search/track invocation distribution remain out of scope for this no-board lane."
