...
struct HbtxrCfgBase {                       // config/design/hbtxr_config.hpp
  ...
  static constexpr int REQ_M_BITS = 18, REQ_N_MAX = 31;   // requant 승수·시프트 폭
};
...
#### `M` 은 18비트입니다 — 승수 폭을 명시해서 (S1 발견 · 2026-08-03 반영)

`dyadic_params` 의 탐색은 **단조**입니다 — shift 가 커지면 오차가 항상 줄어서 **탐색이 언제나
`shift_max` 에 착지**합니다. 그러면 `M = round(scale·2³¹)` 이라 비율이 1을 넘는 엣지에서
`M` 이 33비트가 되고, `ap_int<20>` 누산기와 곱하면 **53비트 곱**입니다. DSP48E2(`27×18`)
한 개로 안 들어갑니다.

**33은 결정된 적이 없습니다** — `shift_max` 를 안 정한 부작용입니다.

**조이는 지점은 shift 가 아니라 `M` 의 폭입니다.** shift 를 자르는 것이 직관적이고 틀립니다:
`max(1, round(scale·2ⁿ))` 이 1에서 바닥을 치므로 **비율이 작은 엣지는 큰 shift 를 진짜로
필요로 합니다.** `shift_max=17` 은 최대 상대오차 1.58e-02 를 냅니다 (`M ≤ 18b` 는 3.81e-06).

| | `M` 최대 | `acc·M` | `n` 범위 |
|---|---:|---:|---|
| 이전 (제한 없음) | 4,461,753,799 (**33b**) | **53b** | 2 – 31 |
| **현재 `M ≤ 18b`** | 262,111 (**18b**) | **38b** | 2 – 31 |

**shift 범위가 그대로 남는 것**이 요점입니다 — 잘리는 것은 승수뿐입니다.
`algorithm` 의 `i_ops.DYADIC_MULT_BITS = 18` 과 `REQ_M_BITS` 가 같은 계약의 양 끝이고,
tb 로더가 안 맞는 승수를 **적재 시점에 거부**합니다.

##### 무손실 범위 — 정확히 말하면

| 설정 | 골든 |
|---|---|
| 4비트 블록 (`-a4` 전부) · **실제 모델**(`w4s4h8`, 8비트 헤드 포함) | **비트 동일** |
| 순수 8비트 블록 프리셋 (`-a8`) | **바뀜** — `block_y` 포함 |

4비트 출력 격자가 `3.8e-06` 의 비율 오차를 흡수하고 8비트는 못 합니다. **8비트에서 비트 동일을
쫓는 것은 의미가 없습니다** — 폭 스윕이 단조가 아닙니다 (24는 동일, **26은 아님**). 반올림이
어디 떨어지느냐의 우연이지 폭이 늘면 좋아지는 성질이 아닙니다. `-a8` 블록 프리셋은
혼합정밀 우회용이지 배포 설정이 아닙니다.

> 근거·측정: `algorithm/docs/reports/2026-07-31-requant-multiplier-width.md`
...
> **S4 의 범위**: 위 버퍼 표는 **목표 스케줄**입니다. S4 는 산술 체인과 그 골든을 세웠고,
> 스테이지 사이를 FIFO 가 아니라 **완전 버퍼**로 연결했습니다 (`qkv_buf` 가 head 별 reorder
> 버퍼 대신 Q·K·V 전체를 듭니다). **dataflow 영역과 FIFO 깊이는 S8·S9** 입니다 —
> 아직 비트 정확하지 않은 체인의 스케줄을 맞추는 것은 순서가 거꾸로입니다.
