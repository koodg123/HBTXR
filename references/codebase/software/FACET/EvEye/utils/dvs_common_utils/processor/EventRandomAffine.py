...
class EventRandomAffine:
    ...
    Returns:
        events (torch.Tensor): transformed events
    """
    ...
    def __call__(self, events, labels):
        ...
        Args:
            events (torch.Tensor): events with shape (4, N)
            labels (torch.Tensor): labels with shape (N, 3)
        Returns:
            events (torch.Tensor): transformed events
            centers (torch.Tensor): transformed centers x,y
            closes (torch.Tensor): transformed closes
        """
        if self.augment_flag:
            ...
            R = torch.tensor(
                [[cos, -sin, 0], [sin, cos, 0], [0, 0, 1]], dtype=torch.float
            ).type_as(events)
            S = torch.tensor(
                [[scale[0], 0, 0], [0, scale[1], 0], [0, 0, 1]], dtype=torch.float
            ).type_as(events)
            T = torch.tensor(
        ...
        if labels is None:
            ...
            labels = labels.T
            centers = F.pad(labels[0:2], (0, 0, 0, 1), value=1)
