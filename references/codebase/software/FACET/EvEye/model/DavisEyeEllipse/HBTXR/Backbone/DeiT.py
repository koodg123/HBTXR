...
class DeiT(nn.Module):
    ...
    def forward_patch_map(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if tokens != grid_size * grid_size:
            raise ValueError(
                f"expected {grid_size * grid_size} patch tokens, got {tokens}"
            )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.forward_features(x)
        x = self.head(self.norm(x)[:, 0])
        return x


def build_deit(config: DeiTConfig, *, init_seed: int = 0, pretrained = False) -> DeiT:
