...
class KLDiscretLoss(nn.Module):
    ...
    def forward(self, output_x, output_y, x_p, y_p):
        ...
        for idx in range(num_joints):
            ...
            coord_x_gt = x_p[:, idx] * width
            coord_y_gt = y_p[:, idx] * height
            ...
            x = torch.arange(0, width, 1, dtype=torch.float32).cuda()
            y = torch.arange(0, height, 1, dtype=torch.float32).cuda()
...
def decode_batch_sa_simdr(output_x, output_y):
    ...
    output[:, :, 0] = torch.squeeze(preds_x) / width
    output[:, :, 1] = torch.squeeze(preds_y) / height
    output = output.cuda()
    return output,prob
