...
from .io import build_cache_file, legacy_track_delta_to_uv, load_or_build_npz_array, read_jsonl, resolve_stored_path
...
def _ellipse_from_payload(payload: dict[str, Any]) -> list[float] | None:
    ellipse = (
        payload.get("pupil_ellipse_xywht_sensor")
        or payload.get("ellipse_sensor_xywht")
        or payload.get("ellipse_xywht")
        or payload.get("ellipse_frame_xywht")
    )
    if ellipse is None:
        return None
...
def _state6_from_payload(payload: dict[str, Any]) -> np.ndarray:
    if payload.get("state_xyabuv") is not None:
        return np.asarray(payload["state_xyabuv"], dtype=np.float32)
    ellipse = (
        payload.get("pupil_ellipse_xywht_sensor")
        or payload.get("ellipse_sensor_xywht")
        or payload.get("ellipse_xywht")
    ...
    if ellipse is None:
        raise KeyError("Payload is missing ellipse/state information")
    return np.asarray(xywht_to_xyabuv(np.asarray(ellipse, dtype=np.float32)), dtype=np.float32)


def _state6_from_row_or_annotation(row: dict[str, Any], annotation: dict[str, Any]) -> np.ndarray:
    if row.get("state6") is not None:
        return np.asarray(row["state6"], dtype=np.float32)
    return _state6_from_payload(annotation)
...
class SwiftHBTXRDataset(Dataset):
    def __init__(
        self,
        manifest_path: str,
        *,
    ...
    @staticmethod
    def _select_existing_path(*candidates: Path | None) -> Path | None:
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        source_frame_path = self._resolve_path(row["frame_path"])
        raw_frame_path = self._resolve_optional_path(row.get("raw_frame_path"))
        interpolated_frame_path = self._resolve_optional_path(row.get("interpolated_frame_path"))
        frame_path = self._select_existing_path(interpolated_frame_path, source_frame_path, raw_frame_path) or source_frame_path
        ...
        cur_state_sensor = _normalize_uv(_state6_from_row_or_annotation(row, annotation))
        prev_state_sensor = _normalize_uv(_state6_from_payload(prev_annotation))
        ...
        open_extent = _safe_float(row.get("open_extent", annotation.get("open_extent", 1.0)), 1.0)
        antiblink_source = str(
            row.get(
                "antiblink_source",
                annotation.get("antiblink_source", annotation.get("annotation_source", row.get("annotation_source", "unknown"))),
            )
        )
        ...
        ellipse_xywht = np.asarray(row.get("ellipse_xywht") or _ellipse_from_payload(annotation) or cur_state_xywht, dtype=np.float32)
        ...
        return {
            ...
            "open_extent": torch.tensor(open_extent, dtype=torch.float32),
            ...
            "ellipse_xywht": torch.from_numpy(ellipse_xywht.astype(np.float32)),
            "state6": torch.from_numpy(cur_state.astype(np.float32)),
            "antiblink_source": antiblink_source,
            "meta": {
                "frame_path": str(frame_path),
                "source_frame_path": str(source_frame_path),
                "raw_frame_path": None if raw_frame_path is None else str(raw_frame_path),
                "interpolated_frame_path": None if interpolated_frame_path is None else str(interpolated_frame_path),
                "interpolated_frame_matched": bool(row.get("interpolated_frame_matched", False)),
                ...
                "ellipse_xywht": ellipse_xywht.tolist(),
                "state6": cur_state.tolist(),
                "open_extent": open_extent,
            },
        }


SWIFTHBTXRDataset = SwiftHBTXRDataset
