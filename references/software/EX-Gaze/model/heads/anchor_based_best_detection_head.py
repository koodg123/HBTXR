...
from mmdet.registry import TASK_UTILS as MMDET_TASK_UTILS,MODELS as MMDET_MODELS
...
class AnchorBasedBestDetectionHead(BaseDenseHead):
    ...
    def __init__(
        ...
        self.bbox_coder = MMDET_TASK_UTILS.build(bbox_coder)
        self.loss_cls = MMDET_MODELS.build(loss_cls)
        self.loss_bbox = MMDET_MODELS.build(loss_bbox)
        self.train_cfg = train_cfg
        self.test_cfg = test_cfg
        if self.train_cfg:
            self.assigner = MMDET_TASK_UTILS.build(self.train_cfg['assigner'])
            if train_cfg.get('sampler', None) is not None:
                self.sampler = MMDET_TASK_UTILS.build(
        ...
        self.prior_generator = MMDET_TASK_UTILS.build(anchor_generator)
