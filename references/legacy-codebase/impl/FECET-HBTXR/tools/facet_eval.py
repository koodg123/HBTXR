...
from facet_common import import_facet_module, load_facet_config, patch_facet_config, resolve_facet_repo_root, trainer_device_kwargs

from fecet_hbtxr.io import write_json


def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate the original FACET EPNet on the FACET reference-compatible dataset")
    ...
    parser.add_argument("--output", type=str, default=str(PROJECT_ROOT / "runs" / "comparison" / "facet" / "facet_eval_summary.json"))
...
def run(args: argparse.Namespace) -> dict:
    ...
    trainer = lightning.Trainer(
        default_root_dir=str(Path(args.output).resolve().parent),
        max_epochs=1,
    ...
    metrics = trainer.validate(model=model, dataloaders=val_loader, ckpt_path=args.checkpoint)
    summary = {
