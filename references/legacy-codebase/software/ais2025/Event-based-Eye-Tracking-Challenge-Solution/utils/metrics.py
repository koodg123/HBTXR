...
# 主要是有overlap的情况下，要对重叠部分计算均值
def process_array(arr, targets, test=False):
    ...
    for index in range(len(sequences1)):
        ...
        # pdb.set_trace()
        actual_lengths.append(sequences2[index][-1, 1])
        pred_lengths.append(len(unique_vals))
    ...
    if test:
        return new_arrays, pred_lengths, actual_lengths
    else:
...
class weighted_MSELoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # loss += 0.2 * ((inputs[:-1] - inputs[1:]) ** 2).sum(dim=2).sqrt().mean()
        # mask = ((targets[:, :-1] - targets[:, 1:]) ** 2).sum(dim=2).sqrt() < 0.005
        # loss += 0.1 * (((inputs[:, :-1] - inputs[:, 1:]) ** 2).sum(dim=2).sqrt()[mask].mean())
