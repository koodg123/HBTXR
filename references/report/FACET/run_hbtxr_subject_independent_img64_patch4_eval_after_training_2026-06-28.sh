...
for SPLIT in val test; do
  OUT_DIR="${REPORT_ROOT}/${RUN_NAME}_${SPLIT}_motion_eval"
...
"${PYTHON}" "${FACET_ROOT}/EvEye/utils/scripts/validate_hbtxr_motion_eval_artifacts.py" \
  ...
  --date "2026-06-28" \
  --splits val test \
  --output-json "${REPORT_ROOT}/${RUN_NAME}_motion_eval_validation_2026-06-28.json" \
  2>&1 | tee -a "${EVAL_LOG}"
...
"${PYTHON}" "${FACET_ROOT}/EvEye/utils/scripts/build_hbtxr_subject_independent_results_report.py" \
  ...
  --config "${CONFIG}" \
  --checkpoint "${CKPT}" \
  --validation-json "${REPORT_ROOT}/${RUN_NAME}_motion_eval_validation_2026-06-28.json" \
  --output "${REPORT_ROOT}/${RUN_NAME}_results_2026-06-28.md" \
  2>&1 | tee -a "${EVAL_LOG}"
