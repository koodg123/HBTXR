...
LOG_FILE="${LOG_DIR}/EPNet_full_unet_gpu0_train_2026-06-26.log"
RUN_ROOT="${FACET_ROOT}/runs/logs/EPNet_full_unet"
...
echo "[$(date --iso-8601=seconds)] manifest found; starting EPNet full training on GPU0" | tee -a "${LOG_FILE}"
cd "${FACET_ROOT}"

export PYTHONPATH=.
export FACET_DEVICES=0
export FACET_DISABLE_CUDNN=1
export PYTHONPYCACHEPREFIX=/tmp/facet_epnet_full_unet_pycache
...
"${PYTHON}" tools/train.py -c DavisEyeEllipse_EPNet_full_unet.yaml 2>&1 | tee -a "${LOG_FILE}"
