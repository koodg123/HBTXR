...
LOG_FILE="${LOG_DIR}/HBTXR_full_unet_effbs32_gpu1_train_2026-06-26.log"
BASELINE_LOG="${LOG_DIR}/HBTXR_full_unet_gpu1_train_2026-06-26.log"
RUN_ROOT="${FACET_ROOT}/runs/logs/HBTXR_full_unet_effbs32"
CONFIG="DavisEyeEllipse_HBTXR_full_unet_effbs32.yaml"

WAIT_INTERVAL_SECONDS="${FACET_EFFBS32_WAIT_INTERVAL_SECONDS:-3600}"
WAIT_BASELINE_COMPLETE="${FACET_EFFBS32_WAIT_BASELINE_COMPLETE:-1}"
WAIT_GPU1_FREE="${FACET_EFFBS32_WAIT_GPU1_FREE:-1}"
...
gpu1_busy() {
  local pids
  pids="$(nvidia-smi -i 1 --query-compute-apps=pid --format=csv,noheader,nounits 2>/dev/null | sed '/^$/d' || true)"
  [ -n "${pids}" ]
}
...
if [ "${WAIT_BASELINE_COMPLETE}" = "1" ]; then
  echo "[$(timestamp)] waiting for baseline HBTXR completion marker in ${BASELINE_LOG}" | tee -a "${LOG_FILE}"
  while ! training_complete "${BASELINE_LOG}"; do
    echo "[$(timestamp)] baseline HBTXR is not complete yet" | tee -a "${LOG_FILE}"
    sleep "${WAIT_INTERVAL_SECONDS}"
  done
fi

if [ "${WAIT_GPU1_FREE}" = "1" ]; then
  echo "[$(timestamp)] waiting for GPU1 to become free" | tee -a "${LOG_FILE}"
  while gpu1_busy; do
    echo "[$(timestamp)] GPU1 still has active compute processes" | tee -a "${LOG_FILE}"
    sleep "${WAIT_INTERVAL_SECONDS}"
  done
fi

echo "[$(timestamp)] starting HBTXR effective-batch-32 full training on GPU1" | tee -a "${LOG_FILE}"
cd "${FACET_ROOT}"
...
export PYTHONPYCACHEPREFIX=/tmp/facet_hbtxr_full_unet_effbs32_pycache
