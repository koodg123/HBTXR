...
mapfile -t routine_scripts < <(
  find "$REPORT_ROOT" -maxdepth 1 -type f -name 'watch_*_2026-06-26.sh' | sort
)
...
if ! rg -q 'check_reproduction_status\.py' "$REPORT_ROOT/run_hourly_status_refresh_guard_2026-06-26.sh"; then
  echo "hourly guard no longer refreshes status" >&2
  exit 1
fi

if ! rg -q 'write_full_training_progress_snapshot\.py' "$REPORT_ROOT/run_hourly_status_refresh_guard_2026-06-26.sh"; then
  echo "hourly guard no longer refreshes progress" >&2
  exit 1
fi

if ! rg -q 'audit_reproduction_completion_2026-06-26\.py' "$REPORT_ROOT/run_hourly_status_refresh_guard_2026-06-26.sh"; then
  echo "hourly guard no longer refreshes completion audit" >&2
  exit 1
fi

if ! rg -q 'summarize_missing_gates_2026-06-26\.py' "$REPORT_ROOT/run_hourly_status_refresh_guard_2026-06-26.sh"; then
