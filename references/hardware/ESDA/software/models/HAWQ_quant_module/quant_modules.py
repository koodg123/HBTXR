...
class QuantLinear(Module):
    ...
    def set_param(self, linear):
        ...
        self.weight = Parameter(linear.linear.weight.data.clone())
        self.register_buffer('weight_integer', torch.zeros_like(self.weight))
        self.register_buffer('bias_integer', torch.zeros_like(linear.linear.bias))
        try:
    ...
    def forward(self, x, prev_act_scaling_factor=None, name="fc", int_folder=""):
        ...
        """
        # metadata, spatial_size = x.metadata, x.spatial_size
        ...
        if not self.full_precision_flag:
            if self.quant_mode == 'symmetric':
                ...
                if self.bias is not None:
                    ...
                    self.bias_integer = torch.zeros(self.out_features).cuda()
            else:
    ...
    def forward(self, x, pre_act_scaling_factor=None, pre_weight_scaling_factor=None, identity=None,
        ...
        if self.training:
            if self.act_percentile == 0:
                x_min = x.features.data.min()
                x_max = x.features.data.max()
            elif self.quant_mode == 'symmetric':
                x_min, x_max = get_percentile_min_max(x.features.detach().view(-1), 100 - self.act_percentile,
            ...
            # the lower_percentile will be set to 0 in order to make sure the final x_min is 0.
                ...
                x_min, x_max = get_percentile_min_max(x.features.detach().view(-1), 0, self.act_percentile, output_tensor=True)
        ...
        if not self.full_precision_flag:
            ...
            if (pre_act_scaling_factor is None) or (self.fixed_point_quantization == True):
                ...
                quant_act_int = self.act_function(x.features, self.activation_bit, self.act_scaling_factor)
            ...
            output = SparseTensor(
                quant_act_int*correct_output_scale,
...
class QuantBnConv2d(Module):
    ...
    def set_param(self, conv, bn):
        self.out_channels = conv.kernel.shape[-1]
        self.register_buffer('convbn_scaling_factor', torch.zeros(self.out_channels))
        self.register_buffer('weight_integer', torch.zeros_like(conv.kernel.data))
        if hasattr(conv, 'bias') and conv.bias is not None:
            self.register_buffer('bias_integer', torch.zeros_like(bn.bias))

        self.conv = conv
        self.out_channels = self.conv.kernel.shape[-1]
        self.bn = bn
        self.bn.momentum = 0.99
        # self.spconv = True if isinstance(conv, scn.SubmanifoldConvolution) else False
    ...
    def forward(self, x, pre_act_scaling_factor=None, name="", int_folder=""):
        ...
        if self.training:
            self.fix_BN = False if self.epoch_counter < self.fix_BN_threshold else True
        else:
            self.fix_BN = True

        # run the forward without folding BN
        if self.fix_BN == False:
            w_transform = self.conv.kernel.data.contiguous().view(-1, self.out_channels)
            w_min = w_transform.min(dim=0).values
            w_max = w_transform.max(dim=0).values

            conv_scaling_factor = symmetric_linear_quantization_params(self.weight_bit, w_min, w_max, self.per_channel)
            weight_integer = self.weight_function(self.conv.kernel, self.weight_bit, conv_scaling_factor, True)

            conv_output = self.conv(x, None, (weight_integer, torch.zeros(self.out_channels).cuda(),
                                              conv_scaling_factor))

            batch_mean = torch.mean(conv_output.features, dim=0)
            batch_var = torch.var(conv_output.features, dim=0)

            # update mean and variance in running stats
            self.bn.bn.running_mean = self.bn.bn.running_mean.detach() * self.bn.bn.momentum + (
                        1 - self.bn.bn.momentum) * batch_mean
            self.bn.bn.running_var = self.bn.bn.running_var.detach() * self.bn.bn.momentum + (1 - self.bn.bn.momentum) \
                                     * batch_var

            output_factor = self.bn.bn.weight.view(1, -1) / torch.sqrt(batch_var + self.bn.bn.eps).view(1, -1)

            output = SparseTensor(
                output_factor * (conv_output.features - batch_mean.view(1, -1)) + self.bn.bn.bias.view(1, -1),
                coordinate_map_key=conv_output.coordinate_map_key,
                coordinate_manager=conv_output.coordinate_manager,
            )

            return (output, conv_scaling_factor.view(-1) * output_factor.view(-1))
        # fold BN and fix running statistics
        else:
            running_std = torch.sqrt(self.bn.bn.running_var.detach() + self.bn.bn.eps)
            scale_factor = self.bn.bn.weight / running_std
            scaled_weight = self.conv.kernel * scale_factor.reshape([-1, self.out_channels])

            if hasattr(self.conv, "bias") and self.conv.bias is not None:
                scaled_bias = self.conv.bias
            else:
                scaled_bias = torch.zeros_like(self.bn.bn.running_mean)
            scaled_bias = (scaled_bias - self.bn.bn.running_mean.detach()) * scale_factor + self.bn.bn.bias

            if not self.full_precision_flag:
                if self.per_channel:
                    w_transform = scaled_weight.data.contiguous().view(-1, self.out_channels)
...
class QuantAveragePool2d(Module):
    ...
    def __init__(self,
        ...
        self.stride = stride
        self.padding = padding
        # self.final_pool = ME.MinkowskiGlobalAvgPooling()
    ...
    def forward(self, x, x_scaling_factor=None):
        ...
        x_int = x.features / correct_scaling_factor
        ...
        # x.features = x_int
        output = self.final_pool(x_int)
        output = SparseTensor(
            transfer_float_averaging_to_int_averaging.apply(output.features) * correct_scaling_factor,
            coordinate_map_key=output.coordinate_map_key,
            coordinate_manager=output.coordinate_manager,
        )

        return output, correct_scaling_factor
