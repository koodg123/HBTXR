...
| 파일 | 내용 |
|---|---|
| `src/lut_calibration.py` | ReQuant, GeLU-ReQuant, LayerNorm RSQRT, Softmax EXP/RECIP LUT와 scalar 생성 |
| `src/quantization_scheme.py` | Group-vector quantization, KL dyadic scale calibration, nonlinear LUT wrapper, hardware index helper |
| `src/cli.py` | group-vector, dyadic calibration, LUT calibration CLI 추가 |
| `src/__init__.py` | 새 quantization/LUT API export |
| `src/paper_equivalence.py` | replacement LUT generator가 원본 QAT equivalence 체크를 잘못 통과하지 않도록 제외 |
...
`src/ops.py`는 HLS quantization kernel을 Python으로 bit-exact하게 재구성한다.
...
python -m py_compile src/quantization_scheme.py src/lut_calibration.py src/cli.py src/__init__.py
