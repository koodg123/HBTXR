...
- `src/artifacts.py`: parse refs and load `type.npy`/`range.npy`.
- `src/ops.py`: bit-exact integer kernels.
- `src/pipeline.py`: discover all supported quantization refs and verify outputs.
- `src/report.py`: write JSON/Markdown evidence reports.
- `src/cli.py`: `list` and `verify` commands.
...
python3 -m src.cli list --limit 12
